...
impl AsyncTestSuite for MySqlIntegrationTestSuite {
    ...
    async fn run(&mut self) -> RunResult {
        ...
        self.test_execute_query("SELECT COUNT(*), status GROUP BY status FROM Orders".to_string())
            .await?;
        ...
        .await?;
        self.test_execute_query(
            "SELECT COUNT(*), status, DATE_TRUNC('month', createdAt) FROM Orders GROUP BY status, DATE_TRUNC('month', createdAt) ORDER BY createdAt".to_string(),
        )
        .await?;
        self.test_execute_query(
            "SELECT COUNT(*), status, DATE_TRUNC('quarter', createdAt) FROM Orders GROUP BY status, DATE_TRUNC('quarter', createdAt) ORDER BY createdAt".to_string(),
        )
        .await?;
