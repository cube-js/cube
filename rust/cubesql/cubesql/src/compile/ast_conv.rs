            ...
            for factor in factors {
                match factor {
                    ...
                    ast::TableFactor::Table { name, alias, .. } => {
                        ...
                        // A CTE is only ever named unqualified, so a qualified
                        // relation is the cube even when a CTE shares its name,
                        // which is how the resolver reads it too
                        let cte = (parts.len() == 1)
                            .then(|| {
                                withs.iter().find_map(|with| {
...
/// How deep a single expression is unwrapped: the parentheses around a
/// filter group, and the truncations or extractions wrapped around the column
/// a predicate filters. Both are peeled in a loop rather than by recursion,
/// and the bound is what keeps a hostile query from turning either walk into
/// an unbounded one.
...
fn apply_action_to_outermost_select(
    ...
    match action {
        ModifyAction::Add(filter) => {
            ...
            let keys = clause_keys.of(kind, match_ctx, clause_mut(select, kind));
        ...
        ModifyAction::Replace { old, new } => {
            ...
            if old_kind == new_kind {
                ...
                let keys = clause_keys.of(new_kind, new_match_ctx, clause_mut(select, new_kind));
                add_expr_to_clause(clause_mut(select, new_kind), new_expr, new_match_ctx, keys);
                Ok(true)
            }
        }
    }
...
/// expression is already present. `keys` holds the keys of the clause's
/// conjuncts, as [`clause_key_set`] builds them, and is extended with the
/// appended expression, so that a batch keys the clause once rather than
/// once per addition.
fn add_expr_to_clause(
    option_clause: &mut Option<ast::Expr>,
    expr: ast::Expr,
    ctx: MatchContext,
    keys: &mut HashSet<FilterKey>,
) -> bool {
    let expr_key = ctx.key(&expr);
    if keys.contains(&expr_key) {
        return false;
    }
    // A group is present when each of its members is
    let members = group_members(&expr);
    if !members.is_empty() && members.iter().all(|member| keys.contains(&ctx.key(member))) {
        return false;
    }
    // The appended expression is a conjunct under both splits, and its
    // members are under the looking-through one
    keys.insert(expr_key);
    for member in members {
        keys.insert(ctx.key(member));
    }
...
#[derive(Debug, Clone, Copy)]
struct MatchContext {
    /// Whether the filter being matched is the only filter the query reports
    /// on its member. One it does report but does not spell out the way this
    /// API writes it - a range the engine merged, a date it computed - is
    /// matched by the column it stands on, which takes every predicate on
    /// that column with it. A filter that isn't reported, or a member
    /// carrying a second reported filter, must never take that path.
    sole_reported: bool,
    ...
    /// Whether a date is the same value written in another form. It is on a
    /// time member, whose values the planner canonicalizes to
    /// `2020-01-01T00:00:00.000Z` whatever the query wrote. On any other
    /// member `'2024-01-01'` and `'2024-01-01T00:00:00.000Z'` are two
    /// different strings, and collapsing them would let a filter name a
    /// predicate that is not the one it stands for.
...
/// A comparison key for a filter expression: its SQL rendering with
/// identifier quoting reduced to a common form, since this API writes a
/// column as `orders_view."status"` where a query may write
/// `orders_view.status`. With `drop_qualifiers` the relation name goes too,
/// for where the outermost FROM leaves only one it could be; with
/// `normalize_dates` a date literal is reduced as well, for a member whose
/// values the planner reports as `2020-01-01T00:00:00.000Z` however the
/// query wrote them.
...
/// The conjunct keys of the outermost clauses, remembered across a batch of
/// actions. Keying a clause renders every conjunct, so a batch of additions
/// that keyed the clause each time would be quadratic in its size; instead
/// the set is built once per clause and per way of keying, extended as
/// expressions are appended, and forgotten when the clause changes in a way
/// that isn't an append.
#[derive(Default)]
struct ClauseKeys {
    // `ignore_qualifier` is fixed for a query, so the way of keying is down to
...
/// Pairs each conjunct with the key worked out for it while the clause was
/// still borrowed, so that a removal or a replacement keys a conjunct once
/// rather than once per pass over the clause. The borrowing and the consuming
/// split walk the clause the same way, so the two line up; were that ever to
/// stop holding, the keys are worked out again rather than paired wrongly.
fn with_keys(
    keys: Vec<FilterKey>,
...
/// needs.
///
/// This only applies to a filter the query reports and reports alone on its
/// member. A filter that isn't reported would take a predicate it never
/// stood for, and a member carrying a second reported filter - a range
/// alongside an `IS NOT NULL`, a `notEquals`, an `EXTRACT` comparison - would
/// lose that one too, silently and with nothing in the response saying so.
/// Where the column cannot be claimed by one filter, no predicate is removed
/// and the no-op a deletion falls back to is the answer.
...
fn replace_expr_in_clause(
    ...
    *option_clause = Some(clause);
    ...
    let mut keys = clause_key_set(option_clause, new_ctx);
    add_expr_to_clause(option_clause, new, new_ctx, &mut keys);
    removed
}

/// Reduces the values of a filter on a time member back to the form they were
/// written in. The planner canonicalizes a date to `2020-01-01T00:00:00.000Z`
/// whatever the query said, and reporting that back would hand callers a value
/// their own query doesn't contain. The member is what decides it rather than
/// the operator: a time member carries a date under every operator, including
/// the `notEquals` a `<>` comparison keeps, while a string member carrying a
/// date-shaped value carries that value and not a date.
fn report_filter(
...
impl ReportedFilters {
    ...
    /// Whether the query reports the filter, and reports nothing else on the
    /// member it stands on. Removing every predicate on a column is only ever
    /// removing this one filter while no other reported filter shares the
    /// column, and a member is how that is decided: the predicates behind a
    /// filter the query does not spell out are not identifiable one by one.
...
/// A filter exactly as it is written, for deciding whether it is the very
/// filter a query reports. Unlike [`filter_key`] this reduces nothing: a
/// value written in another form is another value, which is what the column
/// path has to be sure of before it removes predicates the filter cannot
/// name by value.
...
async fn add_filters_and_verify(
    ...
    let (new_sql, _) = modify_sql_ast_many(&sql, &actions, &meta, &ReportedFilters::none())
