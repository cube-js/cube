...
/// back from a plan has to match the literal it came from.
fn expr_match_key(expr: &ast::Expr) -> String {
...
fn clause_contains_expr(clause: &ast::Expr, needle: &ast::Expr) -> bool {
    ...
    if keys.contains(&expr_match_key(needle)) {
        return true;
    }
    ...
    !members.is_empty()
        && members
            .iter()
            .all(|member| keys.contains(&expr_match_key(member)))
}
...
fn matching_conjuncts(
    ...
    let needle_key = expr_match_key(needle);

    if and_conjuncts_exact(&clause)
        .into_iter()
        .any(|conjunct| expr_match_key(conjunct) == needle_key)
    {
...
fn remove_expr_from_clause(option_clause: &mut Option<ast::Expr>, needle: &ast::Expr) -> usize {
    ...
    let clause = match matching_conjuncts(clause, needle) {
        Ok(conjuncts) => {
            ...
            let kept = conjuncts
                .into_iter()
                .filter(|conjunct| expr_match_key(conjunct) != needle_key)
                .collect::<Vec<_>>();
    ...
    let kept = conjuncts
        .into_iter()
        .filter(|conjunct| !member_keys.contains(&expr_match_key(conjunct)))
        .collect::<Vec<_>>();
    let removed = before - kept.len();
    *option_clause = and_chain(kept);
    removed
...
fn replace_expr_in_clause(
    ...
    let clause = match matching_conjuncts(clause, old) {
        Ok(conjuncts) => {
            let old_key = expr_match_key(old);
            let mut replaced = 0;
            let conjuncts = conjuncts
                .into_iter()
                .map(|conjunct| {
                    if expr_match_key(&conjunct) == old_key {
                        replaced += 1;
                        new.clone()
                    } else {
                        conjunct
...
mod tests {
    ...
    #[tokio::test]
    async fn test_sql_filters_round_trip_reported_values() -> std::result::Result<(), CubeError> {
        use crate::compile::{test::get_test_session, DatabaseProtocol};
