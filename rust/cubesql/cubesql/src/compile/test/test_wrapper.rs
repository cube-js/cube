...
async fn test_wrapper_conditional_aggregation_multi_cte() {
    ...
    // joined to it - swapping the sides would drop unmatched rows
    let from_position = sql.find(r#") AS "t_root""#).expect(&sql);
    let join_position = sql
        .find(r#"LEFT JOIN (SELECT"#)
        .filter(|join_position| {
            sql[*join_position..]
                .contains(r#") AS "t_prop_b" ON ("t_root"."gender" = "t_prop_b"."j_gender")"#)
        })
        .expect(&sql);
    assert!(
