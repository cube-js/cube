...
mod tests {
    ...
    fn build_response_schema_sets_only_marker_when_no_last_refresh_time() {
        let schema = build_schema();
        // No incoming last_refresh_time, but external flag is set — emit only
    ...
    fn convert_transport_response_threads_external_flag_into_schema_metadata() {
        ...
        // path: a V1LoadResponse with `external: true` and a `lastRefreshTime`
        // should produce a RecordBatch whose schema metadata has the marker
