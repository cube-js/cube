...
use super::information_schema::utils::{ExtDataType, OidBuilder, OidType};
...
fn cast_oid_arg(argument: &ArrayRef, name: &str) -> Result<ArrayRef> {
    match argument.data_type() {
        DataType::Int64 => cast(&argument, &ExtDataType::Oid.into()).map_err(|err| err.into()),
        ...
        let cond_array = match condition.data_type() {
            ...
            DataType::Utf8 => {
                ...
                for i in 0..cond_array.len() {
                    result.append_value(!condition.is_null(i))?;
                }
...
pub fn create_pg_datetime_precision_udf() -> ScalarUDF {
    let fun = make_scalar_function(move |args: &[ArrayRef]| {
        let tmp = cast_oid_arg(&args[0], "oid")?;
        let typids = downcast_primitive_arg!(tmp, "typid", OidType);
...
pub fn create_pg_truetypid_udf() -> ScalarUDF {
    let fun = make_scalar_function(move |args: &[ArrayRef]| {
        let atttypids = downcast_primitive_arg!(args[0], "atttypid", OidType);
        let typtypes = downcast_string_arg!(args[1], "typtype", i32);
        let typbasetypes = downcast_primitive_arg!(args[2], "typbasetype", OidType);
        ...
        let result = izip!(atttypids, typtypes, typbasetypes)
            ...
            .collect::<PrimitiveArray<OidType>>();
    ...
    create_udf(
        ...
        vec![
            ExtDataType::Oid.into(),
            DataType::Utf8,
            ExtDataType::Oid.into(),
        ],
        Arc::new(ExtDataType::Oid.into()),
        Volatility::Immutable,
        fun,
    )
}
...
pub fn create_pg_get_expr_udf() -> ScalarUDF {
    ...
    ScalarUDF::new(
        ...
        &Signature::one_of(
            vec![
                TypeSignature::Exact(vec![DataType::Utf8, ExtDataType::Oid.into()]),
                TypeSignature::Exact(vec![
                    DataType::Utf8,
                    ExtDataType::Oid.into(),
                    DataType::Boolean,
...
pub fn create_pg_table_is_visible_udf() -> ScalarUDF {
    ...
    ScalarUDF::new(
        "pg_table_is_visible",
        &Signature::one_of(
            vec![TypeSignature::Exact(vec![ExtDataType::Oid.into()])],
            Volatility::Immutable,
        ),
        &return_type,
        &fun,
...
// TODO: accepts double, not an integer
...
pub fn create_pg_type_is_visible_udf() -> ScalarUDF {
    ...
    ScalarUDF::new(
        "pg_type_is_visible",
        &Signature::exact(vec![ExtDataType::Oid.into()], Volatility::Immutable),
        &return_type,
        &fun,
    )
}
...
pub fn create_pg_get_constraintdef_udf() -> ScalarUDF {
    ...
    ScalarUDF::new(
        ...
        &Signature::one_of(
            vec![
                TypeSignature::Exact(vec![ExtDataType::Oid.into(), DataType::Boolean]),
                TypeSignature::Exact(vec![ExtDataType::Oid.into()]),
            ],
            Volatility::Immutable,
        ),
        &return_type,
...
pub fn create_pg_my_temp_schema() -> ScalarUDF {
    let fun = make_scalar_function(move |_args: &[ArrayRef]| {
        let mut builder = OidBuilder::new(1);
        builder.append_value(0).unwrap();

        Ok(Arc::new(builder.finish()) as ArrayRef)
    });

    let return_type: ReturnTypeFunction = Arc::new(move |_| Ok(Arc::new(ExtDataType::Oid.into())));
...
pub fn create_cube_regclass_cast_udf() -> ScalarUDF {
    let fun = make_scalar_function(move |args: &[ArrayRef]| {
        ...
        match args[0].data_type() {
            ...
            DataType::Int32 => cast(&args[0], &DataType::Int64).map_err(|err| err.into()),
