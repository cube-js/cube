...
impl QueryPlanner {
    ...
    async fn select_to_plan(
        ...
        let rewrite_engine = env::var("CUBESQL_REWRITE_ENGINE")
            ...
            .unwrap_or(self.state.protocol == DatabaseProtocol::PostgreSQL || self.state.protocol == DatabaseProtocol::Redshift);
        if rewrite_engine {
...
WHERE `TABLE_SCHEMA` = '{}'",
    ...
    fn set_variable_to_plan(
        ...
        match self.state.protocol {
            DatabaseProtocol::PostgreSQL | DatabaseProtocol::Redshift => Ok(QueryPlan::MetaOk(flags, CommandCompletion::Set)),
