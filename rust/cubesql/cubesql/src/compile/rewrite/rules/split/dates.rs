...
impl SplitRules {
    ...
    fn transform_date_part_epoch(
        ...
        move |_, egraph, subst| {
            ...
            {
                let date_part: DatePartToken = if let Ok(date_part) = date_part.parse() {
                    date_part
                } else {
                    return false;
                };
                if !matches!(
                    date_part,
                    // Julian sates are very similar to epoch, so we handle it here as well
    ...
    fn transform_date_part(
        ...
        move |_, egraph, subst| {
                ...
                &egraph[subst[date_part_var]].data.constant
            {
                ...
                let trunc_granularity = if let Some(token) = date_part.delta_for_trunc() {
                    token
                } else {
                    return false;
                };
