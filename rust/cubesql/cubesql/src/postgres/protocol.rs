...
pub trait Deserialize {
    async fn deserialize(mut buffer: Cursor<Vec<u8>>) -> Result<Self, Error> where Self: Sized;

    async fn read_from<Reader: AsyncReadExt + Unpin + Send>(mut reader: &mut Reader) -> Result<Self, Error>
    where Self: Sized {
