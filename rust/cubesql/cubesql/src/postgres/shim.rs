...
use crate::{
    ...
    sql_shared::{
        ...
        AuthContext, ConnectionState, QueryResponse,
    },
    CubeError,
};
...
impl AsyncPostgresShim {
    pub async fn run_on
    (socket: TcpStream, server: Arc<ServerManager>, state: Arc<ConnectionState>)
    -> Result<(), Error> {
        let mut shim = Self {
            socket,
    ...
    pub async fn write<Message: protocol::Serialize>(&mut self, message: Message) -> Result<(), Error> {
    ...
    pub async fn process_startup_message(&mut self) -> Result<(), Error> {
        ...
        if
            startup_message.protocol_version.major != 3 ||
            startup_message.protocol_version.minor != 0
        {
            let error_response = protocol::ErrorResponse::new(
                ...
                format!(
                    ...
                    startup_message.protocol_version.major,
                    startup_message.protocol_version.minor,
                ),
            );
            buffer::write_message(&mut self.socket, error_response).await?;
            return Err(Error::new(ErrorKind::Unsupported, "unsupported frontend protocol version"));
        }
        ...
        self.write(protocol::ReadyForQuery::new(protocol::TransactionStatus::Idle)).await?;
        Ok(())
    }
