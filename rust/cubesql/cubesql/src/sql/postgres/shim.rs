...
use crate::sql::statement::{redact_error_log, sql_query_key};
use crate::{
    compile::{
...
impl AsyncPostgresShim {
    ...
    fn new_span_id(&self, sql: String) -> Arc<SpanId> {
        Arc::new(SpanId::new(
            ...
            self.sql_query_key(&sql),
        ))
    }

    /// The `{ "sql": ... }` a load event carries for a statement, redacted per configuration.
    fn sql_query_key(&self, sql: &str) -> serde_json::Value {
        sql_query_key(
            sql,
            self.session
                .session_manager
    ...
    pub async fn handle_connection_error(
        ...
        // The client gets its statement and message back as sent; the log does not
        let (log_message, log_props) = if self
            .session
            .session_manager
            .server
            .config_obj
            .log_redaction()
        {
            redact_error_log(&message, props.clone())
        } else {
            (message.clone(), props.clone())
        };
        ...
        self.logger.error(log_message.as_str(), log_props);
