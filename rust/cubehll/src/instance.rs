...
impl HllInstance {
    pub fn new(num_buckets: u32) -> HllInstance {
        assert!(num_buckets <= MAX_BUCKETS);
        return HllInstance::Sparse(SparseHll::new(index_bit_length(num_buckets)));
    }
...
impl SparseHll {
    ...
    pub fn new(index_bit_len: u8) -> SparseHll {
        SparseHll::validate_prefix_len(index_bit_len);
        return SparseHll {
            index_bit_len,
    ...
    fn validate_prefix_len(index_bit_len: u8) {
        assert!(
            1 <= index_bit_len && index_bit_len <= SparseHll::EXTENDED_PREFIX_BITS,
...
impl DenseHll {
    ...
    pub fn new(index_bit_len: u8) -> DenseHll {
        DenseHll::validate_prefix_len(index_bit_len);
    ...
    pub fn read(data: &[u8]) -> Result<DenseHll> {
        ...
        let index_bit_len = c.read_u8()?;
        DenseHll::validate_prefix_len(index_bit_len); // TODO: show error.
    ...
    fn validate_prefix_len(index_bit_len: u8) {
        assert!(
            1 <= index_bit_len && index_bit_len <= 16,
            "index_bit_len is out of range: {}",
            index_bit_len
        );
    }
