...
impl PrettyPrint for MultiStageMeasureCalculation {
    fn pretty_print(&self, result: &mut PrettyPrintResult, state: &PrettyPrintState) {
        result.println(&format!("Measure Calculation: {}", self.calculation_type.to_string()), state);
        let state = state.new_level();
        let details_state = state.new_level();
        result.println("schema:", &state);
        self.schema.pretty_print(result, &details_state);
        if !self.partition_by.is_empty() {
            result.println(&format!("partition_by: {}", self.partition_by.join(", ")), &state);
        }
        if self.window_function_to_use != MultiStageCalculationWindowFunction::None {
            result.println(&format!("window_function_to_use: {}", self.window_function_to_use.to_string()), &state);
        }
