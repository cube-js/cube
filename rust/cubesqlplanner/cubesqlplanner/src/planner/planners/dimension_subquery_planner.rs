...
impl DimensionSubqueryPlanner {
    ...
    pub fn plan_logical_queries(&self, dimensions: &Vec<Rc<BaseDimension>>) -> Result<Vec<Rc<DimensionSubQuery>>, CubeError> {
        let mut result = Vec::new();
        for subquery_dimension in dimensions.iter() {
            result.push(self.plan_logical_query(subquery_dimension.clone())?)
        }
        Ok(result)
    }

    fn plan_logical_query(&self, subquery_dimension: Rc<BaseDimension>) -> Result<Rc<DimensionSubQuery>, CubeError> {
        ...
        let sub_query = query_planner.plan_logical()?;
        let result = Rc::new(DimensionSubQuery {
            query: sub_query,
            primary_keys_dimensions: primary_keys_dimensions.into_iter().map(|d| d.member_evaluator()).collect(),
