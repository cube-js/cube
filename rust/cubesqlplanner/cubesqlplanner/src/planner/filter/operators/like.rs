...
impl FilterOperationSql for LikeOp {
    fn to_sql(&self, ctx: &FilterSqlContext) -> Result<String, CubeError> {
        let allocated = ctx.allocate_and_cast_values(
            &self.values.iter().map(|v| Some(v.clone())).collect::<Vec<_>>(),
            &self.member_type,
        )?;
        ...
        let like_parts = allocated
            ...
            .map(|v| {
                ctx.plan_templates
                    .ilike(ctx.member_sql, &v, self.start_wild, self.end_wild, self.negated)
            })
        ...
        Ok(format!("({}){}", like_parts.join(logical_symbol), null_check))
    }
}
