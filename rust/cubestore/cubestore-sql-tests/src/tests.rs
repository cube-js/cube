...
pub fn sql_tests(prefix: &str) -> Vec<(&'static str, TestFn)> {
    ...
    let test_list = if prefix == "migration" {
        test_list.into_iter().filter(|(name, _)| !excluded_from_migration_test(name)).collect()
    } else {
        test_list
    };
...
async fn decimal_math(service: Box<dyn SqlClient>) {
    ...
    service.exec_query("CREATE TABLE foo.test_decimal (value Decimal(5, 10))").await.unwrap();
    service.exec_query("INSERT INTO foo.test_decimal (value) VALUES (10), (20), (30), (40), (100), (200), (300)").await.unwrap();
    let r: Arc<DataFrame> = service.exec_query("SELECT value, value / 3 FROM foo.test_decimal").await.unwrap();
    let columns: &Vec<Column> = r.get_columns();
    assert_eq!(columns.len(), 2);
    assert_eq!(columns[0].get_column_type(), &ColumnType::Decimal { scale: 10, precision: 10 });
    assert_eq!(columns[1].get_column_type(), &ColumnType::Decimal { scale: 14, precision: 14 });
    const S10: i128 = 1_00000_00000i128;
    const S14: i128 = 1_0000_00000_00000i128;
    fn mk_row(n: i128) -> Vec<TableValue> {
        vec![TableValue::Decimal(Decimal::new(n * S10)), TableValue::Decimal(Decimal::new(n * S14 / 3))]
    }
    assert_eq!(to_rows(&r), [10, 20, 30, 40, 100, 200, 300].into_iter().map(|n| mk_row(n)).collect::<Vec<_>>());
}
...
async fn date_add(service: Box<dyn SqlClient>) {
    ...
    match (&rows[0][0], &rows[0][1]) {
        (TableValue::Timestamp(tv), TableValue::Timestamp(day_later)) => {
            assert_eq!(day_later.get_time_stamp(), tv.get_time_stamp() + 86400i64 * 1_000_000_000);
        },
...
async fn cache_incr(service: Box<dyn SqlClient>) {
    ...
    assert_eq!(
        r.get_rows(),
        &vec![Row::new(vec![TableValue::String((if !service.is_migration() { "1" } else { "3" }).to_string()),]),]
    );
    ...
    assert_eq!(
        r.get_rows(),
        &vec![Row::new(vec![TableValue::String((if !service.is_migration() { "2" } else { "4" }).to_string()),]),]
    );
}
...
async fn cache_set_get_rm(service: Box<dyn SqlClient>) {
    ...
    let get_response = service
        .exec_query("CACHE GET 'key_to_rm'")
        .await
        .unwrap();
