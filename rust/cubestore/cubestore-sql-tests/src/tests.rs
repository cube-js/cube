...
async fn rolling_window_two_aggregates_ratio(service: Box<dyn SqlClient>) -> Result<(), CubeError> {
    ...
    // gives them positionally, so a missing name has to decline the rewrite rather than drop the
    // aggregate's field: a narrower schema than the enclosing subquery was built with makes
    // `optimize_projections` resolve a field the node no longer has, which panics with an index
    // out of bounds. The multi-aggregate rewrite declines here, so this runs as a plain aggregate
    // over the range join.
