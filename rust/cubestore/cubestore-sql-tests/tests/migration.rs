...
use cubestore::{config::Config, sql::{QueryPlans, SqlQueryContext, SqlService}, store::DataFrame, CubeError};
    ...
    run_sql_tests("migration", vec![], move |test_name, test_fn| {
...

        {
            ...
            if let Err(e) = recursive_copy_directory(&from_dir, &to_dir) {
                panic!("could not copy data directory from {:?} to {:?}: {}", from_dir, to_dir, e);
            }
        }
        {
            ...
            if std::fs::exists(&from_dir).unwrap() {
                ...
                if let Err(e) = recursive_copy_directory(&from_dir, &to_dir) {
                    panic!("could not copy 'remote' directory from {:?} to {:?}: {}", from_dir, to_dir, e);
                }
            }
        }

        r.block_on(Config::run_migration_test(&test_name, |services| async move {
...
impl FilterWritesSqlClient {
    fn new(sql_service: Arc<dyn SqlService>) -> FilterWritesSqlClient {
        FilterWritesSqlClient { tolerate_next_query_flag: std::sync::Mutex::new(NextQueryTreatment::FilterNormally), sql_service }
    }

    fn replace_tolerate_next_query_flag(&self, new_flag_value: NextQueryTreatment) -> NextQueryTreatment {
        let mut guard = self.tolerate_next_query_flag.lock().expect("unpoisoned tolerate_next_query_flag");
...
impl FilterWritesSqlClient {
    fn should_filter(query: &str) -> FilterQueryResult {
        ...
        let should_skip = q.starts_with("insert ")
            || q.starts_with("create ")
            || q.starts_with("cache set ")
            ;
...
impl SqlClient for FilterWritesSqlClient {
    async fn exec_query(&self, query: &str) -> Result<Arc<DataFrame>, CubeError> {
        match self.compute_filter_flag(query) {
            ...
            FilterQueryResult::UnrecognizedQueryType => unimplemented!("FilterWritesSqlClient does not support query prefix for '{}'", query),
        }
    }
    async fn exec_query_with_context(
        ...
        match self.compute_filter_flag(query) {
            FilterQueryResult::RunQuery => self.sql_service.exec_query_with_context(context, query).await,
            FilterQueryResult::Hardcoded(result) => result,
            FilterQueryResult::UnrecognizedQueryType => unimplemented!("FilterWritesSqlClient does not support query prefix for '{}'", query),
        }
    }
    ...
    fn migration_hardcode_next_query(&self, next_result: Result<Arc<DataFrame>, CubeError>) {
        let old_flag = self.replace_tolerate_next_query_flag(NextQueryTreatment::Hardcoded(next_result));
        assert!(matches!(old_flag, NextQueryTreatment::FilterNormally));
    }
}
