...
impl<W: io::Write + Send> AsyncMysqlShim<W> for Backend {
    ...
    async fn on_prepare<'a>(&'a mut self, _query: &'a str, info: StatementMetaWriter<'a, W>) -> Result<(), Self::Error> {
        info.reply(42, &[], &[])
    }

    async fn on_execute<'a>(&'a mut self, _id: u32, _params: ParamParser<'a>, results: QueryResultWriter<'a, W>) -> Result<(), Self::Error> {
    ...
    async fn on_query<'a>(&'a mut self, query: &'a str, results: QueryResultWriter<'a, W>) -> Result<(), Self::Error> {
        ...
        let columns = data_frame.get_columns().iter().map(|c| Column {
    ...
    pub async fn listen(address: String, sql_service: Arc<dyn SqlService>) -> Result<(), CubeError> {
        ...
        loop {
            ...
            tokio::spawn(async move {
                if let Err(e) = AsyncMysqlIntermediary::run_on(Backend {
