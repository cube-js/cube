...
impl SchedulerImpl {
    ...
    async fn schedule_compaction_if_needed(
        ...
        if check_row_counts && chunk_sizes > self.config.compaction_chunks_total_size_threshold()
            ...
            // TODO config
            || in_memory_chunks.len() > self.config.compaction_in_memory_chunks_count_threshold()
            || min_in_memory_created_at.map(|min| Utc::now().signed_duration_since(min).num_seconds() > self.config.compaction_in_memory_chunks_lifetime_threshold()).unwrap_or(false)
            || max_created_at.map(|min| Utc::now().signed_duration_since(min).num_seconds() > self.config.compaction_chunks_idle_threshold()).unwrap_or(false)
        {
