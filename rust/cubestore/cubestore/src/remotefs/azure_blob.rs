...
use object_store::{Error as ObjectStoreError, ObjectMeta, ObjectStore, PutPayload};
use std::env;
use std::fmt;
...
impl AzureBlobRemoteFs {
    ...
    async fn head_if_exists(
        &self,
        path: &object_store::path::Path,
        remote_prefix: &str,
    ) -> Result<Option<ObjectMeta>, CubeError> {
        match self.store.head(path).await {
            Ok(meta) => Ok(Some(meta)),
            Err(ObjectStoreError::NotFound { .. }) => Ok(None),
            Err(e) => Err(CubeError::internal(format!(
                "Azure Blob head error for {}: {}",
                remote_prefix, e
            ))),
        }
    }
...
impl RemoteFs for AzureBlobRemoteFs {
    ...
    async fn upload_file(
        ...
        // Capture the uploaded byte count BEFORE moving `data` into the
        // payload. Some cubestore callers (notably RocksDB WAL files and
        // `*-current` pointer files under `cachestore` / `metastore`) keep
        // writing to `temp_upload_path` while the upload is in flight, so
        // re-reading `fs::metadata(&temp_upload_path)` after `put` would
        // return a newer, larger size and trip `check_upload_file` with
        // "File sizes for X don't match after upload. Expected N but got M"
        // even though the blob correctly contains the original N bytes.
        let expected_size = data.len() as u64;
        let payload = PutPayload::from(data);

        self.store.put(&path, payload).await.map_err(|e| {
            CubeError::internal(format!(
                "Azure Blob upload error for {}: {}",
                remote_path, e
            ))
        })?;

        self.check_upload_file(remote_path.clone(), expected_size)
            .await?;
    ...
    async fn list(&self, remote_prefix: String) -> Result<Vec<String>, CubeError> {
        ...
        while let Some(item) = stream.next().await {
            let meta =
                item.map_err(|e| CubeError::internal(format!("Azure Blob list error: {}", e)))?;
        ...
        // Same head() fallback as list_with_metadata — see comment there.
        if result.is_empty() {
            if let Some(meta) = self.head_if_exists(&prefix, &remote_prefix).await? {
                result.push(self.strip_sub_path(&meta.location.to_string()));
            }
        }
    ...
    async fn list_with_metadata(
        ...
        // The object_store crate's list(Some(&prefix)) returns an empty
        // stream when the prefix is an exact blob name that contains a
        // path separator (it is interpreted as a directory path). Fall
        // back to head() so callers that pass an exact filename — e.g.
        // import/mod.rs resolving `temp://foo` to `temp-uploads/foo` —
        // still find the blob.
        if result.is_empty() {
            if let Some(meta) = self.head_if_exists(&prefix, &remote_prefix).await? {
                result.push(RemoteFile {
