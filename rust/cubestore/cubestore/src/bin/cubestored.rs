...
fn main() {
    ...
    let version = package_json
        ...
        .unwrap()
        .to_string();
    let metrics_mode = match std::env::var("CUBESTORE_METRICS_COMPATIBILITY") {
        Ok(s) if s == "statsd" => metrics::Compatibility::StatsD,
        Ok(s) if s == "dogstatsd" => metrics::Compatibility::DogStatsD,
        Ok(s) => panic!(
            "CUBESTORE_METRICS_COMPATIBILITY must be 'statsd' or 'dogstatsd', got '{}'",
            s
        ),
    ...
    init_metrics("127.0.0.1:0",  metrics_server_address, metrics_mode);
