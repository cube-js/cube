...
use datafusion::logical_expr::{
    AggregateUDF, Expr, ScalarUDF, ScalarUDFImpl, Signature, TableSource, TypeSignature, Volatility, Window, WindowUDF
};
...
impl TopicTableProvider {
    pub fn new(topic: String, columns: &Vec<Column>) -> Self {
        ...
        udfs.push(Arc::new(ScalarUDF::new_from_impl(ParseTimestampFunc::new())));
        udfs.push(Arc::new(ScalarUDF::new_from_impl(ConvertTzFunc::new())));
        udfs.push(Arc::new(ScalarUDF::new_from_impl(FormatTimestampFunc::new())));

        let udfs = udfs.into_iter().map(|udf| (udf.name().to_owned(), udf)).collect();

        let mut udafs = SessionStateDefaults::default_aggregate_functions();
        udafs.append(&mut registerable_arc_aggregate_udfs());

        let udafs = udafs.into_iter().map(|udaf| (udaf.name().to_owned(), udaf)).collect();

        let udwfs = SessionStateDefaults::default_window_functions();
        let udwfs = udwfs.into_iter().map(|udwf| (udwf.name().to_owned(), udwf)).collect();
        Self {
            topic,
            schema,
