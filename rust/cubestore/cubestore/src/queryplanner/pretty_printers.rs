            ...
            match plan {
                ...
                LogicalPlan::Extension(Extension { node }) => {
                    ...
                    } else if let Some(topk) = node.as_any().downcast_ref::<ClusterAggregateTopKUpper>()
                    {
                        ...
                        let lower_node: Option<&ClusterAggregateTopKLower> = match topk.input.as_ref() {
                            LogicalPlan::Extension(Extension { node }) => {
                                if let Some(lower_node) = node.as_any().downcast_ref::<ClusterAggregateTopKLower>() {
                                    Some(lower_node)
                                } else {
                                    None
                                }
                        ...
                        if let Some(lower_node) = lower_node {
                            if self.opts.show_aggregations {
                                self.output += &format!(", aggs: {}", pp_exprs(&lower_node.aggregate_expr))
                            }
                            if self.opts.show_sort_by {
                                self.output += &format!(
                                    ", sortBy: {}",
                        ...
                        self.expecting_topk_lower = true;
                    } else if let Some(topk) = node.as_any().downcast_ref::<ClusterAggregateTopKLower>()
                    {
                        if !was_expecting_topk_lower {
                            self.output += &format!("ClusterAggregateTopKLower (ERROR: unexpected)");
                        } else {
