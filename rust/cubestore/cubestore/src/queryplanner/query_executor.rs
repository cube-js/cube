...
use datafusion::physical_plan::{
    collect, DisplayAs, DisplayFormatType, ExecutionPlan, ExecutionPlanProperties,
    ...
    async fn execute_router_plan(
        ...
        if execution_time.as_millis() > 200 {
            ...
            debug!(
                ...
                execution_time,
                pp_phys_plan_ext(split_plan.as_ref(), &PPOptions{ show_metrics: true, ..PPOptions::none() }),
            );
        }
        if results.is_err() {
            error!(
    ...
    async fn execute_worker_plan(
        ...
        if execution_time.elapsed()?.as_millis() > 200 || results.is_err() {
            ...
            debug!(
                ...
                execution_time.elapsed()?,
                pp_phys_plan_ext(worker_plan.as_ref(), &PPOptions{ show_metrics: true, ..PPOptions::none() }),
            );
        }
        if results.is_err() {
            error!(
    ...
    fn async_scan(
        ...
        let plan: Arc<dyn ExecutionPlan> = if let Some(key_columns) = unique_key_columns {
            ...
            let join_columns = join_columns
                ...
                .collect::<Result<Vec<_>, _>>()?;
            Arc::new(SortPreservingMergeExec::new(LexOrdering::new(join_columns), read_data))
        } else {
