...
use datafusion::{
    ...
    common::utils::proxy::VecAllocExt, error::{DataFusionError, Result},
    logical_expr::{function::{AccumulatorArgs, StateFieldsArgs}, utils::format_state_name, AggregateUDFImpl, Signature, TypeSignature, Volatility},
...
impl AggregateUDFImpl for XirrUDF {
    ...
    fn state_fields(&self, args: StateFieldsArgs) -> Result<Vec<Field>> {
        Ok(vec![
            Field::new(format_state_name(args.name, "payments"), DataType::List(Arc::new(Field::new_list_field(DataType::Float64, true))), false),
            Field::new(format_state_name(args.name, "dates"), DataType::List(Arc::new(Field::new_list_field(DataType::Date32, true))), false),
            Field::new(format_state_name(args.name, "initial_guess"), DataType::List(Arc::new(Field::new_list_field(DataType::Float64, true))), false),
            Field::new(format_state_name(args.name, "on_error"), DataType::List(Arc::new(Field::new_list_field(DataType::Float64, true))), false),
        ])
    }
}
...
impl XirrAccumulator {
    ...
    fn allocated_size(&self) -> usize {
        let XirrAccumulator { pairs, initial_guess, on_error } = self;
...
impl Accumulator for XirrAccumulator {
    ...
    fn peek_state(&self) -> Result<Vec<ScalarValue>> {
        let (payments_vec, dates_vec): (Vec<_>, Vec<_>) = self.pairs.iter().copied::<(f64, i32)>().unzip();

        let payments_list = single_element_listarray::<Float64Type, _>(payments_vec.into_iter().map(|p| Some(p)));
        let dates_list = single_element_listarray::<Date32Type, _>(dates_vec.into_iter().map(|p| Some(p)));

        let initial_guess_list = match self.initial_guess {
            ValueState::Unset => single_element_listarray::<Float64Type, _>(([] as [Option<f64>; 0]).into_iter()),
            ValueState::Set(initial_guess) => single_element_listarray::<Float64Type, _>(([initial_guess] as [Option<f64>; 1]).into_iter()),
        };
        let on_error_list = match self.on_error {
            ValueState::Unset => single_element_listarray::<Float64Type, _>(([] as [Option<f64>; 0]).into_iter()),
            ValueState::Set(on_error) => single_element_listarray::<Float64Type, _>(([on_error] as [Option<f64>; 1]).into_iter()),
        };
        Ok(vec![
