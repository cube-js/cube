...
pub fn registerable_aggregate_udfs() -> Vec<AggregateUDF> {
    vec![AggregateUDF::new_from_impl(HllMergeUDF::new()), AggregateUDF::new_from_impl(XirrUDF::new())]
}
...
impl ScalarUDFImpl for DateAddSub {
    ...
    fn return_type(&self, arg_types: &[DataType]) -> Result<DataType, DataFusionError> {
        if arg_types.len() != 2 {
            return Err(DataFusionError::Internal(format!("DateAddSub return_type expects 2 arguments, got {:?}", arg_types)));
        }
        match (&arg_types[0], &arg_types[1]) {
            (ts@DataType::Timestamp(_, _), DataType::Interval(_)) => Ok(ts.clone()),
            _ => Err(DataFusionError::Internal(format!("DateAddSub return_type expects Timestamp and Interval arguments, got {:?}", arg_types))),
        }
    }
    fn invoke(&self, inputs: &[ColumnarValue]) -> Result<ColumnarValue, DataFusionError> {
