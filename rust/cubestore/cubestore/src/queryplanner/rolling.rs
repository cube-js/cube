...
use datafusion::physical_expr::{
    EquivalenceProperties, GroupsAccumulatorAdapter, LexOrdering, LexRequirement, Partitioning, PhysicalExpr, PhysicalSortExpr, PhysicalSortRequirement
};
// TODO upgrade DF
...
use datafusion::physical_plan::{
    collect, ColumnarValue, DisplayAs, DisplayFormatType, ExecutionPlan,
    PlanProperties,
};
...
impl PartialOrd for RollingWindowAggregate {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        ...
        let RollingWindowAggregate {
            schema, input, dimension, dimension_alias, from, to, every, partition_by, rolling_aggs, rolling_aggs_alias, group_by_dimension, aggs, lower_bound, upper_bound, offset_to_end
        } = self;
    ...
    async fn plan_extension(
        ...
        let rolling_aggs = node
            ...
            .map(|e| -> Result<_, DataFusionError> {
                match e {
                    Expr::AggregateFunction(AggregateFunction { func, params: AggregateFunctionParams { args, .. } }) => {
