...
#[tokio::test(flavor = "multi_thread")]
async fn test_two_different_agg() {
    let ctx = create_context();
...
#[tokio::test(flavor = "multi_thread")]
async fn test_two_different_windows() {
    let ctx = create_context();

    let query = indoc! {r#"
        measures:
          - orders.rolling_sum_trailing_7d
          - orders.rolling_count_3d
        time_dimensions:
          - dimension: orders.created_at
            granularity: day
            dateRange:
...
#[tokio::test(flavor = "multi_thread")]
async fn test_rolling_and_to_date() {
    let ctx = create_context();
