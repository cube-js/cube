...
impl FilterOperationSql for ToDateRollingWindowOp {
    fn to_sql(&self, ctx: &FilterSqlContext) -> Result<String, CubeError> {
        // The span the window reads is derived while planning wherever the
        // series and the period both follow from interval math, and both its
        // ends render as literal parameters.
        if let Some((from, to)) = ctx.date_range_literals(&self.window_range)? {
            let date_field = ctx.convert_tz(ctx.member_sql())?;
            return ctx.plan_templates.time_range_filter(date_field, from, to);
        }
