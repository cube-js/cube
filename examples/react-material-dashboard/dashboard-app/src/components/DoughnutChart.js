...
const DoughnutChart = props => {
  ...
  if (resultSet) {
    const COLORS_SERIES = [theme.palette.secondary.light, theme.palette.secondary.lighten, theme.palette.secondary.main];
    const data = {
        ...
        <CardContent>
          ...
          <div className={classes.stats}>
            {resultSet.series()[0].series.map(status => (
              ...
              key={status.category}
              >
              <Typography variant="body1" className={classes.title}>{status.category}</Typography>
              <Typography
              variant="h2"
              >
