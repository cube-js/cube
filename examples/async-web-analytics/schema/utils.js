...
export const transformSqlProps = (definitions) => {
  return Object.keys(definitions).reduce((result, key) => {
    const definition = definitions[key];
    return {
      ...result,
      [key]: convertStringPropToFunction(
        ['sql', 'drillMembers'],
        definition,
      ),
    };
...
export const transformSchema = (cubeJsonSchema) => {
  return {
    sql: () => cubeJsonSchema.sql,
    joins: transformSqlProps(cubeJsonSchema.joins),
    dimensions: transformSqlProps(cubeJsonSchema.dimensions),
    measures: transformSqlProps(cubeJsonSchema.measures),
  };
};

export const getEventsSchema = async () => {
...
//
export const getPageViewsSchema = async () => {
  return Promise.resolve({
    title: 'PageViews',
    extends: 'Events',
    // eslint-disable-next-line no-template-curly-in-string
    sql: (Events) => `SELECT * FROM ${Events.sql()} events WHERE events.platform = 'web' AND events.event = 'page_view'`,
