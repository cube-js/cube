...
# Enable query result cache (default: true, can be overridden)
export CUBESQL_QUERY_CACHE_ENABLED="${CUBESQL_QUERY_CACHE_ENABLED:-true}"
export CUBESQL_QUERY_CACHE_MAX_ENTRIES="${CUBESQL_QUERY_CACHE_MAX_ENTRIES:-1000}"
export CUBESQL_QUERY_CACHE_TTL="${CUBESQL_QUERY_CACHE_TTL:-3600}"
...
echo -e "  Query Cache: ${CUBESQL_QUERY_CACHE_ENABLED} (max: ${CUBESQL_QUERY_CACHE_MAX_ENTRIES}, ttl: ${CUBESQL_QUERY_CACHE_TTL}s)"
echo ""
echo -e "${YELLOW}To test the connections:${NC}"
echo -e "  PostgreSQL: psql -h 127.0.0.1 -p ${CUBESQL_PG_PORT} -U root"
echo -e "  Arrow Native: Use ADBC driver with connection_mode=native"
echo ""
echo -e "${YELLOW}Example clients:${NC}"
echo -e "  Python: python arrow_ipc_client.py"
echo -e "  JavaScript: node arrow_ipc_client.js"
echo -e "  R: Rscript arrow_ipc_client.R"
echo ""
