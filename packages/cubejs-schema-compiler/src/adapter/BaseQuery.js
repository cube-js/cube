...
export class BaseQuery {
  ...
  /**
   * Refresh key options letting the orchestrator evaluate this key from its own
   * clock instead of running the SQL. Returns nothing unless the feature is on,
   * which keeps the emitted tuples unchanged by default.
   *
   * @protected
   * @param {Object} refreshKey
      ...
      () => {
        ...
        if (preAggregation.refreshKey) {
          ...
          if (preAggregation.refreshKey.every || preAggregation.refreshKey.incremental) {
            ...
            // renewalThresholdOutsideUpdateWindow shortening for freshly sealed partitions.
            // Neither is reproducible from a time interval, so opt out of local evaluation.
