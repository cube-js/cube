...
describe('SQL Generation', () => {
  ...
  it('calculated join', async () => {
    ...
    const query = new PostgresQuery(
      { joinGraph, cubeEvaluator, compiler },
      {
        measures: ['visitor_checkins.revenue_per_checkin'],
        timeDimensions: [],
        timezone: 'America/Los_Angeles',
      }
    );
    ...
    return dbRunner.testQuery(query.buildSqlAndParams()).then((res) => {
      console.log(JSON.stringify(res));
      expect(res).toEqual([{ visitor_checkins__revenue_per_checkin: '50' }]);
    });
  });
  ...
  it('filter join not multiplied', async () => {
    ...
    const query = new PostgresQuery(
      { joinGraph, cubeEvaluator, compiler },
      {
        measures: ['visitor_checkins.google_sourced_checkins'],
        timeDimensions: [],
        filters: [{ dimension: 'cards.id', operator: 'equals', values: ['3'] }],
        timezone: 'America/Los_Angeles',
      }
    );
  ...
  it('subquery rolling', async () => {
    ...
    const query = new PostgresQuery(
      ...
      {
        measures: ['visitors.visitor_count'],
        dimensions: ['visitors.checkinsRolling'],
        timeDimensions: [
          {
            dimension: 'visitors.created_at',
            granularity: 'day',
  ...
  it('subquery with propagated filters', async () => {
    ...
    const query = new PostgresQuery(
      ...
      {
        measures: ['visitors.visitor_count'],
        dimensions: ['visitors.checkinsWithPropagation'],
        timeDimensions: [
          {
            dimension: 'visitors.created_at',
            granularity: 'day',
  ...
  it('average subquery', async () => {
    ...
    const query = new PostgresQuery(
      { joinGraph, cubeEvaluator, compiler },
      {
        measures: ['visitors.averageCheckins'],
        timeDimensions: [
          {
            dimension: 'visitors.created_at',
            granularity: 'day',
  ...
  it('join rollup pre-aggregation', async () => {
    ...
    const query = new PostgresQuery(
      { joinGraph, cubeEvaluator, compiler },
      {
        measures: ['visitors.per_visitor_revenue'],
    ...
    return dbRunner
      .testQueries(
        preAggregationsDescription.invalidateKeyQueries.concat([
          [
  ...
  it('security context', async () => {
    ...
    return dbRunner.testQuery(query.buildSqlAndParams()).then((res) => {
      console.log(JSON.stringify(res));
      expect(res).toEqual([{ visitor_checkins__revenue_per_checkin: '60' }]);
    });
  });
