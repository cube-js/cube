...
use cubesql::compile::sql_filters::{self, SqlFiltersUpdate};
...
async fn handle_get_sql_filters(
    ...
    with_session(&services, native_auth_ctx.clone(), |session| async move {
        ...
        match sql_filters::get_sql_filters(&sql_query, meta_context, session).await {
...
/// The caller's failures are answered in-band, which the gateway maps to a 400;
/// internal ones are thrown, so they reach its error handler as a 500.
...
async fn handle_add_sql_filters(
    ...
    handle_update(services, native_auth_ctx, move |meta, session| async move {
        sql_filters::add_sql_filters(&sql_query, &filters, meta, session).await
    })
    .await
}
...
async fn handle_set_sql_filters(
    ...
    handle_update(services, native_auth_ctx, move |meta, session| async move {
        sql_filters::set_sql_filters(&sql_query, &filters, meta, session).await
    })
    .await
}
...
async fn handle_delete_sql_filters(
    ...
    handle_update(services, native_auth_ctx, move |meta, session| async move {
        sql_filters::delete_sql_filters(&sql_query, &filters, meta, session).await
    })
    .await
}
...
async fn handle_replace_sql_filters(
    ...
    handle_update(services, native_auth_ctx, move |meta, session| async move {
        sql_filters::replace_sql_filters(&sql_query, &old_filters, &new_filters, meta, session).await
    })
    .await
}
