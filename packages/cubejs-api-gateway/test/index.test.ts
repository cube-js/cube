...
describe("API Gateway", () => {
  ...
  test("query transform with checkAuth", async () => {
    ...
    const res = await request(app)
      ...
      .set(
        "Authorization",
        "Authorization: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1aWQiOjUsImlhdCI6MTYxMTg1NzcwNSwiZXhwIjoyNDc1ODU3NzA1fQ.tTieqdIcxDLG8fHv8YWwfvg_rPVe1XpZKUvrCdzVn3g"
      )
      .expect(200);
  ...
  describe("/cubejs-system/v1", () => {
    ...
    const scheduledRefreshTimeZonesFactory = () => [
      "UTC",
      "America/Los_Angeles",
    ];
    ...
    const notAllowedTestFactory =
      ({ route, method = "get" }) =>
      async () => {
        const { app } = appPrepareFactory();
        return request(app)
          [method](`/cubejs-system/v1/${route}`)
          .set("Content-type", "application/json")
          .expect(403);
      };

    const notAllowedWithUserTokenTestFactory =
      ({ route, method = "get" }) =>
      async () => {
    ...
    const successTestFactory =
      ({ route, method = "get", successBody = {}, successResult }) =>
      async () => {
    ...
    const wrongPayloadsTestFactory =
      ({
        route,
        wrongPayloads,
      }: {
    ...
    const testConfigs = [
      ...
      {
        route: "pre-aggregations",
        successResult: { preAggregations: preAggregationsResultFactory() },
      },
      {
        route: "pre-aggregations/security-contexts",
        successResult: {
          securityContexts: scheduledRefreshContextsFactory().map(
            (obj) => obj.securityContext
          ),
        },
      },
      {
        route: "pre-aggregations/timezones",
        successResult: { timezones: scheduledRefreshTimeZonesFactory() },
      },
      {
        ...
        successResult: {
          preAggregationPartitions: preAggregationPartitionsResultFactory(),
        },
      },
      {
        route: "sql-runner",
      ...
      },
      {
        ...
        successResult: {
          data: ["line_items_count_by_states"],
        },
      },
      {
        route: "db-schema",
    ...
    testConfigs.forEach((config) => {
      describe(`/cubejs-system/v1/${config.route}`, () => {
        ...
        test(
          "not allowed with user token",
          notAllowedWithUserTokenTestFactory(config)
        );
        test(
          "not route (works only with playgroundAuthSecret)",
          notExistsTestFactory(config)
        );
  ...
  describe("healtchecks", () => {
    ...
    test("livez (standalone) partial outage", async () => {
      ...
      const { app, dataSourceStorage } = createApiGateway(
        new AdapterApiMock(),
        new DataSourceStorageUnhealthyMock()
      );
