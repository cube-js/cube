...
class ApiGateway {
  ...
  protected async modifySqlFilters({
    ...
    try {
      ...
      // Walked with a stack of its own rather than by recursion: the nesting
      // is whatever the request carries, and a body deep enough to exhaust
      // the call stack would fail inside the handler and be answered as a
      // server error instead of a rejected request. Counting stops once the
      // limit is passed, since nothing beyond it changes the answer.
      const countFilters = (filters: unknown[]): number => {
        const pending: any[] = [...filters];
        let total = 0;

        while (pending.length > 0 && total <= MAX_SQL_FILTERS) {
          ...
          // reject rather than something to descend into
          ...
          if (Array.isArray(group) && group.length > 0) {
            ...
            // A group holding nothing is still one filter to answer for
            total += 1;
          }
        }
