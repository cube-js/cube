...
class ApiGateway {
  ...
  protected async modifySqlFilters({
    ...
    try {
      ...
      // A filter group nests any number of leaves inside a single array
      // entry, and the leaves are what the rewrite engine pays for, so the
      // leaves are what is bounded - iteratively, since the depth is the
      // caller's to choose.
