...
/**
 * Masking tests with Tesseract DISABLED (CUBESQL_SQL_PUSH_DOWN=false).
 * Verifies the BaseQuery JS path handles masking correctly.
 */
describe('Cube RBAC Engine [masking without tesseract]', () => {
  jest.setTimeout(60 * 5 * 1000);
  let db: StartedTestContainer;
  let birdbox: BirdBox;
  ...
  describe('Masking via REST API (no tesseract)', () => {
    let maskingViewerClient: CubeApi;
    let maskingFullClient: CubeApi;
    let maskingPartialClient: CubeApi;
    ...
    test('cube: dynamic SQL mask works without tesseract', async () => {
      const result = await maskingViewerClient.load({
        dimensions: ['masking_test.secret_string'],
        limit: 5,
      });
      const rows = result.rawData();
      expect(rows.length).toBeGreaterThan(0);
      for (const row of rows) {
        expect(row['masking_test.secret_string']).toMatch(/^\*\*\*.{1,2}$/);
      }
    });
