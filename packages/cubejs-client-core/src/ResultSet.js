...
const TIME_SERIES = {
  day: (range) => Array.from(range.by('day')).map((d) => d.format('YYYY-MM-DDT00:00:00.000')),
  month: (range) => Array.from(range.snapTo('month').by('month')).map((d) => d.format('YYYY-MM-01T00:00:00.000')),
  year: (range) => Array.from(range.snapTo('year').by('year')).map((d) => d.format('YYYY-01-01T00:00:00.000')),
  hour: (range) => Array.from(range.by('hour')).map((d) => d.format('YYYY-MM-DDTHH:00:00.000')),
  minute: (range) => Array.from(range.by('minute')).map((d) => d.format('YYYY-MM-DDTHH:mm:00.000')),
  second: (range) => Array.from(range.by('second')).map((d) => d.format('YYYY-MM-DDTHH:mm:ss.000')),
  week: (range) =>
    Array.from(range.snapTo('isoweek').by('week')).map((d) => d.startOf('isoweek').format('YYYY-MM-DDT00:00:00.000')),
};
...
class ResultSet {
  ...
  drillDown(drillDownLocator, pivotConfig) {
    ...
    values
      .filter(([member]) => member !== 'measures')
      .forEach(([member, value]) => {
        const [cubeName, dimension, granularity] = member.split('.');

        if (granularity !== undefined) {
          const range = moment.range(value, value).snapTo(granularity);

          timeDimensions.push({
            dimension: [cubeName, dimension].join('.'),
            dateRange: [range.start, range.end].map((dt) => dt.format(moment.HTML5_FMT.DATETIME_LOCAL_MS)),
          });
        } else if (value == null) {
          filters.push({
            member,
  ...
  static getNormalizedPivotConfig(query, pivotConfig = null) {
    ...
    const substituteTimeDimensionMembers = (axis) =>
      axis.map((subDim) =>
        timeDimensions.find((td) => td.dimension === subDim) && !dimensions.find((d) => d === subDim)
    ...
    pivotConfig.x = pivotConfig.x
      .concat(allDimensions.filter((d) => !allIncludedDimensions.includes(d)))
  ...
  timeSeries(timeDimension) {
    ...
    if (!dateRange) {
      const dates = pipe(
        map(
          (row) =>
            row[ResultSet.timeDimensionMember(timeDimension)] &&
  ...
  pivot(pivotConfig) {
    ...
    const pivotImpl = (resultIndex = 0) => {
      ...
      if (
        ...
        equals(
          pivotConfig.x,
          (query.timeDimensions || []).filter((td) => !!td.granularity).map((td) => ResultSet.timeDimensionMember(td))
        )
      ) {
        const series = this.loadResponses.map((loadResponse) => this.timeSeries(loadResponse.query.timeDimensions[0]));
        ...
        return {
          ...
          yValuesArray: unnest(
            allYValues.map((yValues) => {
              ...
              return (yGrouped[this.axisValuesString(yValues)] || [{ row: {} }]).map(({ row }) => [
                yValues,
                measureValue(row, measure),
              ]);
            })
          ),
        };
  ...
  mergePivots(pivots, joinDateRange) {
    const minLengthPivot = pivots.reduce(
      (memo, current) => (memo != null && current.length >= memo.length ? memo : current),
      null
    );
  ...
  chartPivot(pivotConfig) {
    ...
    const aliasSeries = (yValues, i) => {
      if (pivotConfig && pivotConfig.seriesAlias && pivotConfig.seriesAlias[i]) {
        return [pivotConfig.seriesAlias[i], ...yValues];
      }
      return [yValues];
    };
    ...
    return this.pivot(normalizedPivotConfig).map(({ xValues, yValuesArray }) =>
      fromPairs(
        normalizedPivotConfig.x
          ...
          .concat(
            isMeasuresPresent
              ? yValuesArray.map(([yValues, measure]) => [yValues.length ? yValues.join() : 'value', measure])
              : []
          )
      )
    );
  ...
  seriesNames(pivotConfig) {
    ...
    const aliasSeries = (axisValues, i) => {
      if (pivotConfig && pivotConfig.seriesAlias && pivotConfig.seriesAlias[i]) {
        return [pivotConfig.seriesAlias[i], ...axisValues];
      }
      return [axisValues];
    };
    ...
    const seriesNames = unnest(
      this.loadResponses.map((_, index) =>
        pipe(map(this.axisValues(pivotConfig.y, index)), unnest, uniq)(this.timeDimensionBackwardCompatibleData(index))
      )
    );
    // const aliasedAxis = aliasSeries(seriesNames);
      ...
      this.backwardCompatibleData[resultIndex] = data.map((row) => ({
        ...
        ...fromPairs(
          Object.keys(row)
            .filter(
              (field) =>
                timeDimensions.find((d) => d.dimension === field) &&
