...
export class RedisQueueDriverConnection {
  ...
  addToQueue(keyScore, queryKey, time, queryHandler, query, priority, options) {
    return this.redisClient.multi()
      ...
      .exec()
      .then((arr) => { return arr ? arr.map((skipFirst) => skipFirst[1]) : arr });
  }
  ...
  async getQueryStageState(onlyKeys) {
    ...
    const [active, toProcess, allQueryDefs] = await request.exec().then((arr) => { return arr ? arr.map((skipFirst) => skipFirst[1]) : arr });
  ...
  async freeProcessingLock(queryKey, processingId, activated) {
    try {
      ...
      if (currentProcessId === processingId) {
        ...
        await removeCommand.exec().then((arr) => { return arr ? arr.map((skipFirst) => skipFirst[1]) : arr });
        return null;
      } else {
