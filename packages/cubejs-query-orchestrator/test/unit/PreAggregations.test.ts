...
describe('PreAggregations', () => {
  ...
  describe('refresh key memoization', () => {
    ...
    test('hasKeyQueryResult ignores the options element of the refresh key tuple', async () => {
      ...
      // Callers legitimately hold differing options objects: replacePartitionSqlAndParams rebuilds
      // that element on every call, with a clock-dependent renewalThreshold for incremental keys.
      ...
      expect(loadCache.hasKeyQueryResult([defaultCacheKeyQuery[0], [], { renewalThreshold: 1 }])).toBe(true);
    ...
    test('both refresh key paths store the same renewal key', async () => {
      const redisKey = queryCache!.queryRedisKey(defaultCacheKeyQuery.slice(0, 2));
      const storedRenewalKey = async () => (await queryCache!.getCacheDriver().get(redisKey)).renewalKey;
