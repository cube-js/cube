...
import { equals, toPairs, fromPairs, indexBy, prop, uniqBy } from 'ramda';
...
function moveKeyAtIndex(object$$1, key, atIndex) {
  ...
  while (j < keys.length) {
    if (entries.length === atIndex) {
      entries.push([key, object$$1[key]]);
      j++;
    } else {
      if (keys[index] !== key) {
        entries.push([keys[index], object$$1[keys[index]]]);
        j++;
      }
