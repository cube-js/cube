/**
 * Configurable class declaration.
 * @copyright Cube Dev
 * @license Apache 2.0
 */
...
import { lookupDriverClass } from './DriverResolver';
...
export type ServerCoreInitializedOptions = Required<
  InitializedOptions,
  'dbType' | 'apiSecret' | 'devServer' | 'telemetry' | 'dashboardAppPath' |
  'dashboardAppPort' | 'driverFactory' | 'dialectFactory' |
  'externalDriverFactory' | 'externalDialectFactory' | 'scheduledRefreshContexts'
>;
...
export class Configurable extends Loggable {
  ...
  public constructor(
    ...
    this.options = this.handleConfiguration(options);
  }
  ...
  private async getDriverConcurrency(
    ...
    const type = await this.options.dbType(ctx);
  ...
  /**
   * Determines whether current instance is configured enough to process query
   * or not.
   */
  ...
  /**
   * Returns instance configuration object.
   */
  protected handleConfiguration(
    opts: InitializedOptions
    ...
    const definedExtDBVariables = skipOnEnv.filter(
    ...
    const externalDbType =
      ...
      (getEnv('devMode') ||
      definedExtDBVariables.length > 0) && 'cubestore' ||
      undefined;
    ...
    if (externalDbType === 'cubestore' && devServer && !opts.serverless) {
      if (!definedExtDBVariables.length) {
        // TODO (buntarb): does it makes any sense to keep this require here?
        // eslint-disable-next-line global-require,no-return-assign,import/no-extraneous-dependencies
    ...
    if (!options.devServer || this.isReadyForQueryProcessing()) {
      ...
      const invalidFields = fieldsForValidation.filter(
   ...
   */
  protected getOrchestratorInitializedOptions(
    context: RequestContext,
