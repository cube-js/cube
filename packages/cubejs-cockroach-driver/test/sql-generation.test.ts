...
import { prepareCompiler as originalPrepareCompiler, UserError } from '@cubejs-backend/schema-compiler';
import { CockroachQuery } from '../src/CockroachQuery';
import { dbRunner } from './CockroachDBRunner';
...
describe('SQL Generation', () => {
  ...
  it('having and where filter in same operator OR', async () => {
    ...
    try {
      ...
      new CockroachQuery({ joinGraph, cubeEvaluator, compiler }, {
        measures: [
    ...
    return dbRunner.testQuery(query.buildSqlAndParams()).then(res => {
      ...
      expect(res).toEqual(
        [{
          ...
          visitors__average_checkins: '2.0000000000000000000'
        }, {
          cards__count: '2',
          visitors__source: 'some',
          visitors__visitor_count: '1',
          visitors__average_checkins: '6.0000000000000000000'
        }]
      );
    });
  });
  ...
  it('where filter with operators OR & AND', async () => {
    ...
    console.log('olololo', query.buildSqlAndParams());
  ...
  it('long named aliased cubes doesn\'t throws', async () => {
    ...
    const aliasedQuery = new CockroachQuery(aliasedCubesCompilers, {
      dimensions: [
