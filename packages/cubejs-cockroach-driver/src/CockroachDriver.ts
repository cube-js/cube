...
import {
  ...
  GenericDataBaseType, IndexesSQL, TableStructure, QueryOptions,
} from '@cubejs-backend/query-orchestrator';

const GenericTypeToCockroach: Record<GenericDataBaseType, string> = {
  double: 'decimal',
...
export class CockroachDriver<Config extends PostgresDriverConfiguration = PostgresDriverConfiguration>
  ...
  protected getTypeParser = (dataTypeID: TypeId, format: TypeFormat | undefined) => {
    ...
    const typeName = this.getCockroachTypeForField(dataTypeID);
    if (typeName === 'hll') {
  ...
  protected getCockroachTypeForField(dataTypeID: number): string | null {
  ...
  protected mapFields(fields: FieldDef[]) {
    return fields.map((f) => {
      const postgresType = this.getCockroachTypeForField(f.dataTypeID);
      if (!postgresType) {
        throw new Error(
  ...
  public fromGenericType(columnType: string) {
    return GenericTypeToCockroach[columnType] || super.fromGenericType(columnType);
  }
}
