...
export function keyByDataSource(origin: string, dataSource?: string): string {
  ...
  if (!isMultipleDataSources() || dataSource === "default") {
    ...
    if (s.length === 2) {
      ...
      throw new Error(
        `The ${origin} environment variable can not be converted for the ${
          dataSource
        } data source.`,
      );
    }
...
const variables: Record<string, (...args: any) => any> = {
  devMode: () => get("CUBEJS_DEV_MODE").default("false").asBoolStrict(),
  port: () => asPortOrSocket(process.env.PORT || "4000", "PORT"),
  tls: () => get("CUBEJS_ENABLE_TLS").default("false").asBoolStrict(),
  webSockets: () => get("CUBEJS_WEB_SOCKETS").default("false").asBoolStrict(),
  serverHeadersTimeout: () => get("CUBEJS_SERVER_HEADERS_TIMEOUT").asInt(),
  serverKeepAliveTimeout: () => get("CUBEJS_SERVER_KEEP_ALIVE_TIMEOUT").asInt(),
  rollupOnlyMode: () =>
    get("CUBEJS_ROLLUP_ONLY").default("false").asBoolStrict(),
  schemaPath: () => get("CUBEJS_SCHEMA_PATH").default("model").asString(),
  refreshWorkerMode: () => {
    ...
    if (process.env.CUBEJS_SCHEDULED_REFRESH_TIMER) {
      return asBoolOrTime(
        process.env.CUBEJS_SCHEDULED_REFRESH_TIMER,
  ...
  refreshWorkerConcurrency: () =>
    get("CUBEJS_REFRESH_WORKER_CONCURRENCY").asIntPositive(),
  ...
  gracefulShutdown: () => get("CUBEJS_GRACEFUL_SHUTDOWN").asIntPositive(),
  dockerImageVersion: () => get("CUBEJS_DOCKER_IMAGE_VERSION").asString(),
  concurrency: ({ dataSource }: { dataSource: string }) =>
    get(keyByDataSource("CUBEJS_CONCURRENCY", dataSource)).asInt(),
  // It's only excepted for CI, nothing else.
  internalExceptions: () =>
    get("INTERNAL_EXCEPTIONS_YOU_WILL_BE_FIRED")
      .default("false")
      .asEnum(["exit", "log", "false"]),
  preAggregationsSchema: () => get("CUBEJS_PRE_AGGREGATIONS_SCHEMA").asString(),
  maxPartitionsPerCube: () =>
    get("CUBEJS_MAX_PARTITIONS_PER_CUBE").default("10000").asInt(),
  scheduledRefreshBatchSize: () =>
    get("CUBEJS_SCHEDULED_REFRESH_BATCH_SIZE").default("1").asInt(),
  nativeSqlPlanner: () =>
    get("CUBEJS_TESSERACT_SQL_PLANNER").default("false").asBool(),
  nativeSqlPlannerPreAggregations: () =>
    get("CUBEJS_TESSERACT_PRE_AGGREGATIONS").default("false").asBool(),
  nativeOrchestrator: () =>
    get("CUBEJS_TESSERACT_ORCHESTRATOR").default("true").asBoolStrict(),
  ...
  allowNonStrictDateRangeMatching: () =>
    get("CUBEJS_PRE_AGGREGATIONS_ALLOW_NON_STRICT_DATE_RANGE_MATCH")
      .default("true")
      .asBoolStrict(),
  transpilationWorkerThreadsCount: () =>
    get("CUBEJS_TRANSPILATION_WORKER_THREADS_COUNT").default("0").asInt(),
  // This one takes precedence over CUBEJS_TRANSPILATION_WORKER_THREADS
  transpilationNative: () =>
    get("CUBEJS_TRANSPILATION_NATIVE").default("false").asBoolStrict(),
  nestedFoldersDelimiter: () =>
    get("CUBEJS_NESTED_FOLDERS_DELIMITER").default("").asString(),
  defaultTimezone: () =>
    get("CUBEJS_DEFAULT_TIMEZONE").default("UTC").asString(),
  preciseDecimalInCubestore: () =>
    get("CUBEJS_DB_PRECISE_DECIMAL_IN_CUBESTORE")
      .default("false")
      .asBoolStrict(),
   ...
   */
  dbSsl: ({ dataSource }: { dataSource: string }) => {
    const val =
      process.env[keyByDataSource("CUBEJS_DB_SSL", dataSource)] || "false";
   ...
   */
  dbSslRejectUnauthorized: ({ dataSource }: { dataSource: string }) => {
    const val =
      process.env[
   ...
   * Database URL.
   */
  dbUrl: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_URL", dataSource)],

  /**
   * Database host.
   */
  dbHost: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_HOST", dataSource)],
   ...
   */
  dbUseSelectTestConnection: ({ dataSource }: { dataSource: string }) => {
    const val =
      process.env[
   ...
   */
  dbKafkaUseSsl: ({ dataSource }: { dataSource: string }) =>
   ...
   * Database domain.
   */
  dbDomain: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_DOMAIN", dataSource)],

  /**
   * Database port.
   */
  dbPort: ({ dataSource }: { dataSource: string }) =>
  ...
  dbSchema: ({
    ...
    console.warn(
      `The ${keyByDataSource(
        "CUBEJS_DB_SCHEMA",
        dataSource,
      )} is deprecated. Please, use the ${keyByDataSource(
        "CUBEJS_DB_NAME",
        dataSource,
      )} instead.`,
    );
   ...
   */
  dbMaxPoolSize: ({ dataSource }: { dataSource: string }) =>
   ...
   * TODO: clarify this env.
   */
  dbPollTimeout: ({ dataSource }: { dataSource: string }) => {
   ...
   */
  dbQueryLimit: (): number =>
    get("CUBEJS_DB_QUERY_LIMIT").default(50000).asInt(),

  /**
   * Query limit wich will be used in the query to the data source if
   * limit property was not specified in the query.
   */
  dbQueryDefaultLimit: (): number =>
    get("CUBEJS_DB_QUERY_DEFAULT_LIMIT").default(10000).asInt(),

  /**
   * Query stream `highWaterMark` value.
   */
  dbQueryStreamHighWaterMark: (): number =>
    get("CUBEJS_DB_QUERY_STREAM_HIGH_WATER_MARK").default(8192).asInt(),

  /**
   * Max number of elements
   */
  usedPreAggregationCacheMaxCount: (): number =>
    get("CUBEJS_USED_PRE_AGG_CACHE_MAX_COUNT").default(8192).asInt(),

  /**
   * Max number of elements
   */
  touchPreAggregationCacheMaxCount: (): number =>
    get("CUBEJS_TOUCH_PRE_AGG_CACHE_MAX_COUNT").default(8192).asInt(),
   ...
   */
  touchPreAggregationTimeout: (): number =>
    get("CUBEJS_TOUCH_PRE_AGG_TIMEOUT")
      .default(60 * 60 * 24)
      .asIntPositive(),
  ...
  dropPreAggregationsWithoutTouch: (): boolean =>
    get("CUBEJS_DROP_PRE_AGG_WITHOUT_TOUCH").default("true").asBoolStrict(),
   ...
   */
  fetchColumnsByOrdinalPosition: (): boolean =>
    get("CUBEJS_DB_FETCH_COLUMNS_BY_ORDINAL_POSITION")
      .default("true")
      .asBoolStrict(),
   ...
   * JDBC URL.
   */
  jdbcUrl: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_JDBC_URL", dataSource)],

  /**
   * JDBC driver.
   */
  jdbcDriver: ({ dataSource }: { dataSource: string }) =>
   ...
   */
  dbExportBucketCsvEscapeSymbol: ({ dataSource }: { dataSource: string }) =>
    process.env[
      keyByDataSource("CUBEJS_DB_EXPORT_BUCKET_CSV_ESCAPE_SYMBOL", dataSource)
    ],
  ...
  dbExportBucketType: ({
    ...
    const val =
      process.env[keyByDataSource("CUBEJS_DB_EXPORT_BUCKET_TYPE", dataSource)];
    if (
      val &&
      supported &&
  ...
  dbExportBucketAwsKey: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_EXPORT_BUCKET_AWS_KEY", dataSource)],
   ...
   */
  dbExportAzureSasToken: ({ dataSource }: { dataSource: string }) =>
    process.env[
   ...
   */
  dbExportBucketAzureClientId: ({ dataSource }: { dataSource: string }) =>
    process.env[
   ...
   */
  dbExportBucketAzureClientSecret: ({ dataSource }: { dataSource: string }) =>
    process.env[
   ...
   */
  dbExportBucketAzureTokenFilePAth: ({ dataSource }: { dataSource: string }) =>
    process.env[
      keyByDataSource(
   ...
   */
  dbExportBucketAzureTenantId: ({ dataSource }: { dataSource: string }) =>
    process.env[
  ...
  dbExportIntegration: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_EXPORT_INTEGRATION", dataSource)],
  ...
  mysqlUseNamedTimezones: ({ dataSource }: { dataSource: string }) => {
    const val =
      process.env[
        keyByDataSource("CUBEJS_DB_MYSQL_USE_NAMED_TIMEZONES", dataSource)
      ];
   ...
   */
  databricksUrl: ({ dataSource }: { dataSource: string }) => {
    const val =
      process.env[keyByDataSource("CUBEJS_DB_DATABRICKS_URL", dataSource)];
    if (!val) {
      throw new Error(
        `The ${keyByDataSource(
   ...
   */
  databricksToken: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_DATABRICKS_TOKEN", dataSource)],

  /**
   * Databricks catalog name.
   * https://www.databricks.com/product/unity-catalog
   */
  databricksCatalog: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_DATABRICKS_CATALOG", dataSource)],

  /**
   * Databricks OAuth Client ID (Same as the service principal UUID)
   */
  databricksOAuthClientId: ({ dataSource }: { dataSource: string }) =>
    process.env[
   ...
   */
  databricksOAuthClientSecret: ({ dataSource }: { dataSource: string }) =>
    process.env[
   ...
   * Athena AWS key.
   */
  athenaAwsKey: ({ dataSource }: { dataSource: string }) =>
   ...
   * Athena AWS secret.
   */
  athenaAwsSecret: ({ dataSource }: { dataSource: string }) =>
    // TODO (buntarb): this name is a common. Deprecate and replace?
   ...
   * Athena AWS region.
   */
  athenaAwsRegion: ({ dataSource }: { dataSource: string }) =>
    // TODO (buntarb): this name is a common. Deprecate and replace?
   ...
   */
  athenaAwsS3OutputLocation: ({ dataSource }: { dataSource: string }) =>
    // TODO (buntarb): this name is a common. Deprecate and replace?
   ...
   */
  athenaAwsWorkgroup: ({ dataSource }: { dataSource: string }) =>
   ...
   * Athena AWS Catalog.
   */
  athenaAwsCatalog: ({ dataSource }: { dataSource: string }) =>
    // TODO (buntarb): Deprecate and replace?
   ...
   */
  athenaAwsAssumeRoleArn: ({ dataSource }: { dataSource: string }) =>
    process.env[
   ...
   */
  athenaAwsAssumeRoleExternalId: ({ dataSource }: { dataSource: string }) =>
    process.env[
   ...
   * BigQuery project ID.
   */
  bigqueryProjectId: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_BQ_PROJECT_ID", dataSource)],

  /**
   * BigQuery Key file.
   */
  bigqueryKeyFile: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_BQ_KEY_FILE", dataSource)],

  /**
   * BigQuery credentials.
   */
  bigqueryCredentials: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_BQ_CREDENTIALS", dataSource)],

  /**
   * BigQuery location.
   */
  bigqueryLocation: ({ dataSource }: { dataSource: string }) =>
   ...
   * @deprecated
   */
  bigqueryExportBucket: ({ dataSource }: { dataSource: string }) => {
    console.warn(
   ...
   */
  clickhouseReadOnly: ({ dataSource }: { dataSource: string }) =>
   ...
   */
  clickhouseCompression: ({ dataSource }: { dataSource: string }) =>
   ...
   */
  elasticApiId: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_ELASTIC_APIKEY_ID", dataSource)],

  /**
   * ElasticSearch API Key.
   */
  elasticApiKey: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_ELASTIC_APIKEY_KEY", dataSource)],

  /**
   * ElasticSearch OpenDistro flag.
   */
  elasticOpenDistro: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_ELASTIC_OPENDISTRO", dataSource)],

  /**
   * ElasticSearch query format.
   */
  elasticQueryFormat: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_ELASTIC_QUERY_FORMAT", dataSource)],
   ...
   */
  fireboltApiEndpoint: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_FIREBOLT_API_ENDPOINT", dataSource)],

  /**
   * Firebolt engine name.
   */
  fireboltEngineName: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_FIREBOLT_ENGINE_NAME", dataSource)],

  /**
   * Firebolt engine endpoint.
   */
  fireboltEngineEndpoint: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_FIREBOLT_ENGINE_ENDPOINT", dataSource)],

  /**
   * Firebolt account name.
   */
  fireboltAccount: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_FIREBOLT_ACCOUNT", dataSource)],
   ...
   * Hive type.
   */
  hiveType: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_HIVE_TYPE", dataSource)],

  /**
   * Hive version.
   */
  hiveVer: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_HIVE_VER", dataSource)],

  /**
   * Hive thrift version.
   */
  hiveThriftVer: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_HIVE_THRIFT_VER", dataSource)],

  /**
   * Hive CDH version.
   */
  hiveCdhVer: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_HIVE_CDH_VER", dataSource)],
   ...
   * Aurora secret ARN.
   */
  auroraSecretArn: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DATABASE_SECRET_ARN", dataSource)],

  /**
   * Aurora cluster ARN.
   */
  auroraClusterArn: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DATABASE_CLUSTER_ARN", dataSource)],
   ...
   */
  redshiftUnloadArn: ({ dataSource }: { dataSource: string }) =>
    process.env[
   ...
   * Materialize cluster.
   */
  materializeCluster: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_MATERIALIZE_CLUSTER", dataSource)],
   ...
   * Snowflake account.
   */
  snowflakeAccount: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_SNOWFLAKE_ACCOUNT", dataSource)],

  /**
   * Snowflake region.
   */
  snowflakeRegion: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_SNOWFLAKE_REGION", dataSource)],

  /**
   * Snowflake warehouse.
   */
  snowflakeWarehouse: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_SNOWFLAKE_WAREHOUSE", dataSource)],

  /**
   * Snowflake role.
   */
  snowflakeRole: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_SNOWFLAKE_ROLE", dataSource)],

  /**
   * Snowflake session keep alive flag.
   */
  snowflakeSessionKeepAlive: ({ dataSource }: { dataSource: string }) => {
    const val =
      process.env[
        keyByDataSource(
   ...
   */
  snowflakeAuthenticator: ({ dataSource }: { dataSource: string }) =>
    process.env[
   ...
   */
  snowflakeOAuthToken: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_SNOWFLAKE_OAUTH_TOKEN", dataSource)],

  /**
   * Snowflake OAuth token path.
   */
  snowflakeOAuthTokenPath: ({ dataSource }: { dataSource: string }) =>
    process.env[
   ...
   * Snowflake host.
   */
  snowflakeHost: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_SNOWFLAKE_HOST", dataSource)],

  /**
   * Snowflake private key.
   */
  snowflakePrivateKey: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_SNOWFLAKE_PRIVATE_KEY", dataSource)],

  /**
   * Snowflake private key path.
   */
  snowflakePrivateKeyPath: ({ dataSource }: { dataSource: string }) =>
    process.env[
   ...
   */
  snowflakePrivateKeyPass: ({ dataSource }: { dataSource: string }) =>
    process.env[
   ...
   */
  snowflakeQuotedIdentIgnoreCase: ({ dataSource }: { dataSource: string }) => {
    const val =
      process.env[
        keyByDataSource(
   ...
   * Presto catalog.
   */
  dbCatalog: ({ dataSource }: { dataSource: string }) => {
    console.warn(
  ...
  duckdbMotherDuckToken: ({ dataSource }: { dataSource: string }) =>
    process.env[
      keyByDataSource("CUBEJS_DB_DUCKDB_MOTHERDUCK_TOKEN", dataSource)
    ],

  duckdbDatabasePath: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_DUCKDB_DATABASE_PATH", dataSource)],

  duckdbS3Region: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_DUCKDB_S3_REGION", dataSource)],

  duckdbS3AccessKeyId: ({ dataSource }: { dataSource: string }) =>
    process.env[
      keyByDataSource("CUBEJS_DB_DUCKDB_S3_ACCESS_KEY_ID", dataSource)
    ],

  duckdbS3SecretAccessKeyId: ({ dataSource }: { dataSource: string }) =>
    process.env[
      keyByDataSource("CUBEJS_DB_DUCKDB_S3_SECRET_ACCESS_KEY", dataSource)
    ],

  duckdbS3Endpoint: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_DUCKDB_S3_ENDPOINT", dataSource)],

  duckdbMemoryLimit: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_DUCKDB_MEMORY_LIMIT", dataSource)],

  duckdbSchema: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_DUCKDB_SCHEMA", dataSource)],

  duckdbS3UseSsl: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_DUCKDB_S3_USE_SSL", dataSource)],

  duckdbS3UrlStyle: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_DUCKDB_S3_URL_STYLE", dataSource)],

  duckdbS3SessionToken: ({ dataSource }: { dataSource: string }) =>
    process.env[
      keyByDataSource("CUBEJS_DB_DUCKDB_S3_SESSION_TOKEN", dataSource)
    ],

  duckdbExtensions: ({ dataSource }: { dataSource: string }) => {
    const extensions =
      process.env[keyByDataSource("CUBEJS_DB_DUCKDB_EXTENSIONS", dataSource)];
    if (extensions) {
  ...
  duckdbCommunityExtensions: ({ dataSource }: { dataSource: string }) => {
    const extensions =
      process.env[
  ...
  duckdbS3UseCredentialChain: ({ dataSource }: { dataSource: string }) => {
    const val =
      process.env[
   ...
   * Presto catalog.
   */
  prestoCatalog: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_PRESTO_CATALOG", dataSource)],

  /**
   * Presto/Trino Auth Token
   */
  prestoAuthToken: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_PRESTO_AUTH_TOKEN", dataSource)],
   ...
   */
  pinotAuthToken: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_PINOT_AUTH_TOKEN", dataSource)],
  ...
  pinotNullHandling: ({ dataSource }: { dataSource: string }) => {
    const val =
      process.env[keyByDataSource("CUBEJS_DB_PINOT_NULL_HANDLING", dataSource)];
   ...
   * Dremio Auth Token
   */
  dremioAuthToken: ({ dataSource }: { dataSource: string }) =>
    process.env[keyByDataSource("CUBEJS_DB_DREMIO_AUTH_TOKEN", dataSource)],

  /** ****************************************************************
   * Cube Store Driver                                               *
   ***************************************************************** */

  cubeStoreHost: () => get("CUBEJS_CUBESTORE_HOST").asString(),
  cubeStorePort: () => get("CUBEJS_CUBESTORE_PORT").asPortNumber(),
  cubeStoreUser: () => get("CUBEJS_CUBESTORE_USER").asString(),
  cubeStorePass: () => get("CUBEJS_CUBESTORE_PASS").asString(),
  cubeStoreMaxConnectRetries: () =>
    get("CUBEJS_CUBESTORE_MAX_CONNECT_RETRIES").default("20").asInt(),
  cubeStoreNoHeartBeatTimeout: () =>
    get("CUBEJS_CUBESTORE_NO_HEART_BEAT_TIMEOUT").default("30").asInt(),
  cubeStoreRollingWindowJoin: () =>
    get("CUBEJS_CUBESTORE_ROLLING_WINDOW_JOIN").default("false").asBoolStrict(),
  allowUngroupedWithoutPrimaryKey: () =>
    get("CUBEJS_ALLOW_UNGROUPED_WITHOUT_PRIMARY_KEY")
      .default(get("CUBESQL_SQL_PUSH_DOWN").default("true").asString())
      .asBoolStrict(),
  nodeEnv: () => get("NODE_ENV").asString(),
  cacheAndQueueDriver: () => get("CUBEJS_CACHE_AND_QUEUE_DRIVER").asString(),
  defaultApiScope: () => get("CUBEJS_DEFAULT_API_SCOPES").asArray(","),
  jwkUrl: () => get("CUBEJS_JWK_URL").asString(),
  jwtKey: () => get("CUBEJS_JWT_KEY").asString(),
  jwtAlgorithms: () => get("CUBEJS_JWT_ALGS").asArray(","),
  jwtAudience: () => get("CUBEJS_JWT_AUDIENCE").asString(),
  jwtIssuer: () => get("CUBEJS_JWT_ISSUER").asArray(","),
  jwtSubject: () => get("CUBEJS_JWT_SUBJECT").asString(),
  jwtClaimsNamespace: () => get("CUBEJS_JWT_CLAIMS_NAMESPACE").asString(),
  playgroundAuthSecret: () => get("CUBEJS_PLAYGROUND_AUTH_SECRET").asString(),
  agentFrameSize: () => get("CUBEJS_AGENT_FRAME_SIZE").default("200").asInt(),
  agentEndpointUrl: () => get("CUBEJS_AGENT_ENDPOINT_URL").asString(),
  agentFlushInterval: () =>
    get("CUBEJS_AGENT_FLUSH_INTERVAL").default(1000).asInt(),
  agentMaxSockets: () => get("CUBEJS_AGENT_MAX_SOCKETS").default(100).asInt(),
  instanceId: () => get("CUBEJS_INSTANCE_ID").asString(),
  telemetry: () => get("CUBEJS_TELEMETRY").default("true").asBool(),
  // SQL Interface
  sqlPort: () => {
    const port = asFalseOrPort(
      process.env.CUBEJS_SQL_PORT || "false",
      "CUBEJS_SQL_PORT",
    );
    if (port) {
      return port;
  ...
  nativeApiGatewayPort: () => {
    ...
    const port = asFalseOrPort(
      process.env.CUBEJS_NATIVE_API_GATEWAY_PORT || "false",
      "CUBEJS_NATIVE_API_GATEWAY_PORT",
    );
    if (port) {
      return port;
    }
  ...
  pgSqlPort: () => {
    ...
    const isDevMode = get("CUBEJS_DEV_MODE").default("false").asBoolStrict();
  ...
  nativeApiGateway: () => get("CUBE_JS_NATIVE_API_GATEWAY_INTERNAL").asBool(),
  // Experiments & Preview flags
  livePreview: () => get("CUBEJS_LIVE_PREVIEW").default("true").asBoolStrict(),
  externalDefault: () =>
    get("CUBEJS_EXTERNAL_DEFAULT").default("true").asBoolStrict(),
  scheduledRefreshDefault: () =>
    get("CUBEJS_SCHEDULED_REFRESH_DEFAULT").default("true").asBoolStrict(),
  previewFeatures: () =>
    get("CUBEJS_PREVIEW_FEATURES").default("false").asBoolStrict(),
  batchingRowSplitCount: () =>
    get("CUBEJS_BATCHING_ROW_SPLIT_COUNT")
      .default(256 * 1024)
      .asInt(),
  maxSourceRowLimit: () =>
    get("CUBEJS_MAX_SOURCE_ROW_LIMIT").default(200000).asInt(),
  convertTzForRawTimeDimension: () =>
    get("CUBESQL_SQL_PUSH_DOWN").default("true").asBoolStrict(),
  // Deprecated section
  ...
  redisUseIORedis: () =>
    get("CUBEJS_REDIS_USE_IOREDIS").default("false").asBoolStrict(),
  // TODO(ovr): Remove in after 1.0 + LTS
  redisUrl: () => {
    const redisUrl = get("CUBEJS_REDIS_URL").asString();
    if (redisUrl) {
      return redisUrl;
    }

    const legacyRedisUrl = get("REDIS_URL").asString();
    if (legacyRedisUrl) {
  ...
  fastReload: () =>
    get("CUBEJS_FAST_RELOAD_ENABLED").default("false").asBoolStrict(),
};

type Vars = typeof variables;

export function getEnv<T extends keyof Vars>(
  key: T,
  opts?: Parameters<Vars[T]>,
): ReturnType<Vars[T]> {
  if (key in variables) {
