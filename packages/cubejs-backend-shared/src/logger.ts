...
export type LoggerFn = (msg: string, params?: LogParams) => void;
...
export function devLogger(level: string) {
  return (msg: string, params: LogParams = {}) => {
    ...
    const logWarning = () => console.log(
      `${withColor(msg, colors.yellow)}: ${format({ ...rest, allSqlLines: true, showRestParams: true })} \n${withColor(warning, colors.yellow)}`
    );
    const logError = () => console.log(`${withColor(msg, colors.red)}: ${format({ ...rest, allSqlLines: true, showRestParams: true })} \n${error?.message} \n${error?.stack}`);
    const logDetails = (showRestParams?: boolean) => console.log(`${withColor(msg)}: ${format({ ...rest, showRestParams })}`);
