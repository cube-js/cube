...
description: Read deployment secrets and tunables from environment variables inside Python config, JavaScript config, and data model code.
---
...
```yamltitle="model/cubes/my_cube.yml"
cubes:
  - name: my_cube
...
file](#cube-py-file), you can access environment variables either via the `getenv`
...
```javascripttitle="model/cubes/my_cube.js"
