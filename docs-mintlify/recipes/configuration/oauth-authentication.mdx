...
[Databricks][ref-databricks-jdbc] and [Snowflake][ref-snowflake].
...
1. Change the credential key in `userCredentials` to match your data source
   (e.g., `userCredentials.databricks`, `userCredentials.snowflake`)
2. Update the `driver_factory` return value with the correct `type` and
   driver-specific options for your data source
3. Keep `context_to_app_id` and `context_to_orchestrator_id` as-is — they
   are data-source-agnostic

See the [data sources reference][ref-data-sources] for driver-specific
