...
<Warning>
  ...
  across data sources, it can only join two rollups, neither rollup can have
  more than one partition (so at least one side must be under ~1M rows), and
  it is ephemeral — set freshness controls on the referenced rollups rather
  than on the `rollup_join` itself.
</Warning>
