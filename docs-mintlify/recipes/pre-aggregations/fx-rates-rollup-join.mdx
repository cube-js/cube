...
to be refreshed every few minutes to stay accurate.
...
small (one row per currency) and cheap to rebuild, so we set a fast
`refresh_key` — for example, every minute:
...
(refreshed every minute). The heavy aggregation never rebuilds when FX rates
