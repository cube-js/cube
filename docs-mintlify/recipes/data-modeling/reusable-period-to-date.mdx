...
dimension the query groups by. Unlike a default name, it has nothing to fall back to: the
query fails with `Rolling window granularity '<name>' is not found in time dimension`
rather than silently reporting Gregorian periods.
