...
The `sql` argument is emitted as a [literal string][ref-jinja-literal] (`|-`) rather than
...
expressions that contain one, such as `{CUBE}."amount"`. The literal form takes all of
them, and — unlike the folded form (`>-`), which folds newlines into spaces — it keeps
line breaks, so a leading `--` comment stays on its own line instead of swallowing the
expression that follows it.

Jinja's [`indent`][link-jinja-indent] filter is what makes a multi-line expression work. A
block scalar ends at the first line indented less than its opening, so without it the
second line of a `CASE` expression closes the block and is read as a mapping key.

Apply `indent` *before* `safe`, not after. Cube's Jinja engine escapes values as JSON
unless they are marked safe, and `indent` returns a fresh, unmarked string — so
`sql | safe | indent(10)` throws the marker away and the SQL arrives quoted, as
`"{CUBE}.amount"`. Marking the result of `indent` keeps the expression raw.
...
The orders cube holds 30 in revenue in December 2024 and nothing earlier. 1 January
...
[link-jinja-indent]: https://jinja.palletsprojects.com/en/stable/templates/#jinja-filters.indent
...
[ref-jinja-literal]: /docs/data-modeling/dynamic/jinja#folded-and-literal-strings
