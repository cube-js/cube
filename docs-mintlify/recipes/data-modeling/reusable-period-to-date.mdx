...
{#- Period-to-date measures for one base measure. -#}
...
Apply the [`safe`][ref-jinja-escaping] filter to every value that is concatenated with
other text rather than substituted on its own. Cube escapes substituted values as JSON
strings, which is harmless when the value is the whole of a YAML value — `type: {{ type }}`
renders as `type: "sum"` and compiles — but breaks as soon as anything is appended to it.
Here, `{{ name }}_{{ period }}_to_date` renders as `"revenue"_"week"_to_date` and the model
fails with `bad indentation of a mapping entry`. The `description` line mixes values into
prose and has the same problem. The macro applies `safe` throughout for consistency.
...
Both mistakes surface as a YAML parse error far from the macro that caused them. Render
the model first — see [previewing YAML][ref-jinja-preview].

</Warning>

<Note>

The `sql` argument is emitted as a [folded string][ref-jinja-folded] (`>-`) rather than
inline. A SQL expression is arbitrary text, and inline it has to avoid everything YAML
reads as syntax: `{CUBE}.amount` starts a flow mapping, `amount # note` truncates at the
comment, and wrapping the whole thing in double quotes only moves the problem to
expressions that contain one, such as `{CUBE}."amount"`. The folded form takes all of
them.
...
The orders cube holds 30 in revenue in December 2024, shown in the monthly table below,
and nothing earlier. 1 January falls in the ISO week that began on 30 December, so WTD
opens with that 30 already counted, while MTD, QTD, and YTD all start fresh:
...
the generated measures pick that up: `{{ ptd.to_date_measures("revenue", "amount") }}`
already emits `granularity: week` and `granularity: month`, so a five-week retail month
accumulates over all five of its weeks.

Pass only the periods the calendar cube actually overrides. A calendar cube that defines
`week`, `month`, and `year` but not `quarter` still accepts `granularity: quarter` — it
falls back to `DATE_TRUNC` and reports Gregorian quarters next to retail months, with no
error. Restrict the list to match:
...
`retail_month` does not compile. See [custom calendars][ref-custom-calendar] for the
calendar cube itself, including its [pre-aggregation
...
```yaml
{{ ptd.to_date_measures("revenue", "amount", "sum", ["fiscal_year"]) }}
```
...
default. Querying a `to_date` window over an overridden granularity also requires
v1.7.32 or later.
...
[ref-jinja-preview]: /docs/data-modeling/dynamic/jinja#previewing-yaml
[ref-jinja-folded]: /docs/data-modeling/dynamic/jinja#folded-and-literal-strings
