...
- Connectors are managed in the admin panel under **MCP Connectors** and apply across the
  organization, so every agent can use the tools you enable — except agents using the
  [`chassis` runtime](/admin/ai#runtime), which runs a reduced toolset that does not
  include MCP connector tools.
