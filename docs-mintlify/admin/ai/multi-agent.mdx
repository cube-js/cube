...
<Info>
  `agents/config.yml` is the source of truth for spaces and agents: you declare them in YAML and Cube creates them for you when you [reconcile](#reconciliation). Agents can't be created in the UI, and their settings are read-only there; a space can be, but not linked to a `spaces:` entry — see [Space scope](#space-scope). Entries are matched to what already exists by their `name` field.
</Info>
...
An agent's `space` is the exception: the link is made when the agent is created, so changing it in YAML doesn't move an existing agent. The agent's page flags the mismatch between the space its config names and the space it is linked to. If the newly named space has no record yet, it appears under **Pending Configurations**, and **Create All** creates it and moves the agent onto it in the same action. If that space already exists for the deployment, nothing is pending and there is no **Create All** to press — delete the agent so its config reads as pending again, then **Create All** recreates it in the space the config names.

<Warning>
  Changing an entry's `name` reads as a new entry: reconciling creates a new space or agent, and the record created from the old name keeps what it stored — an agent's chats, a space's memories — and is flagged **(misconfigured)** because its config no longer exists. Delete it from the **Agents** or **Spaces** page once you no longer need it.
</Warning>
