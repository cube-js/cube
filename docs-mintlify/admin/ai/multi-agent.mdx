...
<Info>
  `agents/config.yml` is the source of truth for spaces and agents: you declare them in YAML and Cube creates them for you when you [reconcile](#reconciliation). Agents can't be created in the UI, and their settings are read-only there. A space can be created by hand, but it isn't linked to any `spaces:` entry — declare it in YAML and reconcile instead, so the entry matches. Entries are matched to what already exists by their `name` field.
</Info>
