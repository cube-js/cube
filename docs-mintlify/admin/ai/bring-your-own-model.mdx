...
- You can add custom HTTP headers sent with every request. A header value
  may include placeholders that are resolved per request from the caller's
  identity: `{{userId}}`, `{{userExternalId}}`, `{{email}}`, `{{username}}`,
  `{{tenantId}}`, `{{tenantName}}`, `{{tenantUrl}}`, `{{deploymentId}}`, and
  `{{agentId}}`. If a placeholder has no value for a given request (for
  example, `{{userExternalId}}` for a user who did not arrive through
  signed embedding), it is left in the header value as literal text rather
