...
- BYOM requests appear in the **AI Tokens Usage** tab in billing settings
  with their token counts, at no cost. They're also recorded in the **AI
  Calls & Cost** view; no pre-built dashboard uses that view, so
  administrators need to build one on it in [Usage
  Analytics][ref-usage-analytics]
