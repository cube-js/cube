...
- No Cube token quota is deducted for BYOM chat requests, and BYOM calls are
  never charged against your Cube AI token balance
- Per-call token counts for BYOM requests do appear in usage views (for
  visibility into what you're sending your own provider), but always with no
  cost attached, since Cube doesn't bill for them
