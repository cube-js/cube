...
Rules can also shape how the agent presents its responses on different surfaces. For example, to suppress charts in Slack unless the user explicitly asks for one:
...
type: always
---
When responding in Slack, do not include charts in the response unless the user has explicitly asked for them.
```

The agent is aware of the surface it's responding on (for example, Slack DMs), so rules like this take effect only when the conversation originates from that surface.
