...
Comments in a certified query file are handled differently depending on where they appear, which lets you control whether the agent sees them:

- **Markdown comments** (`<!-- ... -->`) in the body of the file **are included** in the content the agent reads. Use them to add context, caveats, or hints that should influence the agent's behavior.
...
<!-- agents/certified_queries/quarterly-revenue.md -->
---
# Internal note: reviewed by the analytics team on 2025-01-15 — hidden from the agent
description: "Get revenue by fiscal quarter"
user_request: "What is the revenue by quarter?"
---
...
<Tip>
If you want the agent to pick up on guidance, put it in a Markdown comment or in the `description` field. Keep YAML comments for notes you want to leave for other developers.
</Tip>
