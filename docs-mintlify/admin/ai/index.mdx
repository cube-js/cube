...
<Warning>
  `reasoning` and `chassis` are **experimental**. They may be unstable and are not yet feature-complete. Stick with `plain` unless you have a specific need for extended reasoning or the `chassis` toolset.
</Warning>
