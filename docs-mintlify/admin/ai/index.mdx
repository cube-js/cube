...
| `runtime`           | string           | `plain`                  | [Runtime mode](#runtime) — `plain`, `reasoning`, or `chassis`.                                                          |
...
| `chassis`    | Optimized for semantic-model exploration. Runs a reduced toolset, and query results appear inline in the answer rather than as separate tool-call results. |

<Warning>
  `reasoning` and `chassis` are **experimental**. They may be unstable and are not yet feature-complete. Stick with `plain` unless you specifically need extended thinking or the leaner semantic-model toolset.
</Warning>

Under `chassis`, some agent features are unavailable, including [MCP connector
tools](/admin/ai/mcp-connectors) and parts of [workbook
authoring](/docs/explore-analyze/workbooks/workbook-agent).

The `runtime` property applies to chat surfaces such as [Analytics
Chat](/docs/explore-analyze/analytics-chat), the [Workbook
Agent](/docs/explore-analyze/workbooks/workbook-agent), and the [Dashboard
Agent](/docs/explore-analyze/dashboards/dashboard-agent). [AI summary
widgets](/docs/explore-analyze/dashboards/widgets/ai-summary) always use their own runtime,
