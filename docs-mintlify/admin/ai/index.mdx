...
├── knowledge/                     # Read-only reference files for the agent
│   └── examples.md
└── cube.py
```
...
Files under a top-level `knowledge/` directory in your project are available to the agent
as read-only reference material — examples, playbooks, or other documents — independent of
data-model access. The agent can list and read them to inform its answers, but can't modify
them; edit them in your project through git or the data-model editor.
