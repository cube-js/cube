...
| `chassis`    | Optimized for semantic-model exploration with a reduced toolset — some agent features, including MCP connector tools and workbook creation, are unavailable. Query results appear inline in the answer rather than as separate tool-call results. |

<Warning>
  `reasoning` and `chassis` are **experimental**. They may be unstable and are not yet feature-complete. Stick with `plain` unless you specifically need extended thinking or query results returned inline in the answer.
</Warning>
