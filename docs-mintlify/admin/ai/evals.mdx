...
The Cube CLI can run the same eval suite as a pull-request check. Authenticate
with `CUBE_API_URL` and `CUBE_API_KEY`, then pass the branch under review:
...
- name: Run Cube agent evals
  ...
  run: cube evals run "$DEPLOYMENT_ID" --branch "$GITHUB_HEAD_REF" --wait
```

The command waits for completion and exits non-zero when the eval run fails,
when it produces no graded questions, or when any question has a verdict other than
`pass`. It also fails closed if the API does not return a complete result set. Add
`--agent NAME` to test a configured agent or
