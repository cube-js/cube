...
can run on demand from [Analytics Chat](/docs/explore-analyze/analytics-chat). Instead
of re-typing the same multi-step request ("produce a weekly revenue report, broken down
by region, with week-over-week trends…"), a user picks a skill and the agent follows the
workflow you've defined.

Skills are configured as code in your [data model repository](/admin/ai#agent-configuration),
alongside your cubes and views. There is no separate UI or database object — a skill is
versioned, reviewed, and deployed like the rest of your Cube project.
...
| `instructions`  | string | No       | The skill instructions. Falls back to the Markdown body if omitted.              |
...
Inline skills accept the same properties as Markdown ones — `name`, `title` (required),
`description` (required), and `instructions` (required).
...
see [Agent skills in Analytics Chat](/docs/explore-analyze/skills).
