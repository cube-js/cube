...
Cube ships a small number of built-in skills alongside any you define. They appear in the
same catalog — the agent's skill list, the `loadSkill` tool, and the `/` menu — with no
configuration required. To customize a built-in skill, define a skill with the same `name`
under `agents/skills/`; your version replaces the built-in one for your deployment.

Built-in skills don't appear as buttons in the chat empty state — those buttons are your
own deployment's pitch — but they're reachable from the `/` menu like any other skill.
