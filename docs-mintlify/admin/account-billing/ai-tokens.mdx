...
against your Cube AI token balance, and you're billed directly by your model
provider. Token counts for BYOM calls still appear in the **AI Tokens
Usage** tab above, at zero cost. They're also recorded in the **AI Calls &
Cost** view in [Usage Analytics][ref-usage-analytics]; no pre-built
dashboard uses that view, so administrators need to build one on it.
