...
Automatic top-up is currently in preview, and the user experience may still change. Reach
out to the [Cube support team](/admin/account-billing/support) to activate this feature for
your account.

</Warning>

Once activated, an admin can turn on automatic top-up on the AI Tokens balance section of
the Billing page. When the balance falls below a threshold, Cube automatically purchases a
credit package so AI features keep working without anyone noticing the balance ran low.

- Pick a top-up amount from the presets ($50/$100/$200/$500) or enter a custom amount.
  Setting the amount to $0 turns automatic top-up off.
- The balance threshold that triggers a purchase is configured by Cube on a per-account
  basis (defaults to $5).
- Only account admins can see or change this setting.
...
Token counts include [BYOM][ref-byom] calls, which are recorded at zero cost
and do not count against your allocation.

On paying accounts, **Billing → Cost and Usage** — a separate tab from **AI
Tokens Usage** above — also shows a balance chip for each credit package the
account holds: the base AI token allowance, then any compute (CCU), additional
AI token, or professional services package, each with its own type, expiry,
and a meter showing how much of it is left. Packages are never summed into a
single total, so promotional or rollover packages stay distinguishable from
...
requests bypass the token quota system entirely — no tokens are consumed
against your Cube AI token balance, and you're billed directly by your
model provider. Token counts for BYOM calls still appear in the **AI Tokens
Usage** tab above. They're also recorded in the **AI Calls & Cost** view in
[Usage Analytics][ref-usage-analytics]; no pre-built dashboard uses that
view, so administrators need to build one on it.
