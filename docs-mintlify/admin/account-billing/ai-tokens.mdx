...
On paying accounts, the **Cost and Usage** tab also shows a balance chip for
each credit package the account holds — the AI token allowance, then any
compute (CCU), AI token, or professional services packages — each with its
own type, expiry, and a meter showing how much of it is left. Packages are
never summed into a single total, so promotional or rollover packages stay
distinguishable from the rest of the balance.
