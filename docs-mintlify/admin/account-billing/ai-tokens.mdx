...
against your Cube AI token balance. You are billed directly by your model
provider. BYOM requests are recorded in the **AI Calls & Cost** view with
their token counts but no cost attached; no pre-built dashboard uses that
view, so administrators need to build one on it in [Usage
