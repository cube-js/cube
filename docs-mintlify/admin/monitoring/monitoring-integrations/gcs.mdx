...
authentication takes two options that work together:

| Option | Description |
| --- | --- |
| `credentials_base64` | The base64-encoded service account key, or a reference to an environment variable holding it (e.g. `$CUBE_CLOUD_MONITORING_GCS_CREDENTIALS`). Cube decodes it and mounts it into the Vector agent at `/etc/credentials/<sink-name>.json`. |
| `credentials_path` | Where the sink reads the key from. Set it to the same `/etc/credentials/<sink-name>.json` path. |
...
start with `Healthcheck endpoint forbidden` even though it could write objects
successfully.
