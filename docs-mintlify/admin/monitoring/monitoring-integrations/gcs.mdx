...
The `gcp_cloud_storage` sink reads its service account key from a file, so
authentication takes two options that work together — one handled by Cube, one by
Vector:

| Option | Handled by | Description |
| --- | --- | --- |
| `credentials_base64` | Cube | The base64-encoded service account key, or a reference to an environment variable holding it (e.g. `$CUBE_CLOUD_MONITORING_GCS_CREDENTIALS`). Cube decodes it, mounts it into the Vector agent at `/etc/credentials/<sink-name>.json`, and removes the option from the configuration it passes on — which is why it does not appear in [Vector's own sink reference][vector-docs-sinks-gcs]. |
| `credentials_path` | Vector | Where the sink reads the key from. Set it to the same `/etc/credentials/<sink-name>.json` path. |

<Warning>

The sink name becomes the name of a Kubernetes object, so it must be lowercase
alphanumeric characters and dashes only — **no underscores**. A sink named
`query_history_gcs` will not get a credentials file; name it
`query-history-gcs` instead.
