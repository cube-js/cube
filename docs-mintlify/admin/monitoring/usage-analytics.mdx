...
| **AI Calls & Cost** | Token consumption and cost of every individual LLM call — finer-grained than AI Usage, which rolls calls up per conversation — broken down by conversation, embedded end user, model, and billing line item. Calls served by a model you bring yourself ([BYOM](/admin/ai/bring-your-own-model)) appear in AI Usage but consume no Cube tokens, so they carry no cost here. |
