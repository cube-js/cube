...
and billing views, which no pre-built dashboard uses. Anything you build
...
| **AI Calls & Cost** | Token consumption and cost of every individual LLM call — a finer grain than AI Usage's per-run rows — broken down by conversation, embedded end user, model, and billing line item. Runs on a model you bring yourself ([BYOM](/admin/ai/bring-your-own-model)) still appear in AI Usage, but consume no Cube tokens, so they have no cost to attribute here. Per-call detail is retained for about 120 days. |
