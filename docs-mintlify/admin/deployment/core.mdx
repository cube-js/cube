...
If you'd like a quick single-instance deployment for testing or small
workloads, [Easypanel](https://easypanel.io) has a one-click Cube template:
[easypanel.io/templates/cubejs](https://easypanel.io/templates/cubejs). It
deploys the `cubejs/cube` image alone (no Cube Store cluster), so it's not a
substitute for the production topology described below.
