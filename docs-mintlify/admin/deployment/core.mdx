...
`CUBEJS_DEV_MODE` defaults to `false`, and the official Docker images run `cubejs server`,
so leaving the flag unset is enough there. Set it to `false` explicitly if you start Cube
with `cubejs dev-server` or the `cubejs-dev-server` bin, which turn development mode on
for themselves when the variable is unset. `NODE_ENV` no longer has any say in it.

</Info>

```dotenv
# Keep development mode off. Setting this to `true` switches off JWT
