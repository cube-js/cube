...
pass its branch name as `branchName` when generating the session and the embedded
content queries that branch's data model. The environment has to be active at that
moment, so leaving the toggle on is what makes a branch embeddable at any time.
