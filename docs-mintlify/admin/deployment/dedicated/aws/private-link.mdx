...
- **Endpoint Service Region** (only if different from your Cube Region): if
  your Endpoint Service is published in an AWS region other than the one
  hosting your Dedicated Infrastructure, share that region too
...
## Multiple subdomains in one DNS zone

If several endpoints should resolve inside the same private hosted zone,
list each additional subdomain along with the DNS name you share. The Cube
team creates a matching record for each one, plus an optional wildcard
fallback record for any subdomain you didn't list explicitly.
