...
- **Endpoint Service Region**: only if your Endpoint Service is published in an
  AWS region other than your Cube Region
...
team creates a matching record for each one. If you also need subdomains you
can't enumerate up front, ask the Cube team to add a wildcard fallback record
too.
