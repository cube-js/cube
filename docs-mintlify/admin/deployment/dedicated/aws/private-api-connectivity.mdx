...
- Links inside the Cube UI (used by **browsers** when rendering dashboards
  and the schema editor — every `/load`, `/meta`, `/sql`, `/run-scheduled-refresh`,
  and context call the UI issues goes to this hostname directly from the
  user's machine).
...
This is almost always a network-path problem on the client side, not a
Cube configuration problem. Use the browser's developer tools to isolate
which leg of the path is broken:
