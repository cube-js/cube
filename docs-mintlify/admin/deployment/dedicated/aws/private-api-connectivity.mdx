...
— never touching the public internet. All traffic to the Cube data plane flows
through the same HTTP and SQL ingress controllers, so this includes AI traffic
to the [Chat API][chat-api] and other AI endpoints — both from Cube's
data-plane-hosted AI Engineer and from external agents that call the Chat API
as a tool. When private connectivity is in place, the public API endpoints can
be disabled completely on request.
