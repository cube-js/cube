...
[Region][cube-region], can be PrivateLink/PSC-connected to your own networks,
and can optionally expose Cube's APIs to your network so that no Cube traffic
ever crosses the public internet.
...
- **Backend connectivity** — traffic that flows **from Cube into your network**.
  ...
  Cube data plane has to reach. PrivateLink, Private Link, and Private Service
  Connect on the provider pages below all configure backend connectivity.
...
PrivateLink/PSC into the customer's data network, plus a frontend
