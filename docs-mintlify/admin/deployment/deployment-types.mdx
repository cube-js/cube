...
one Cube Store router. With **Multi-router Cube Store** enabled, each Cube Store
router id gets its own router, spreading pre-aggregation storage and query load
...
A deployment uses at most as many routers as its **Number of clusters**. Once a
Cube API instance has seen more distinct router ids than that, requests with
additional ids are served by the shared Cube Store router. To keep every tenant
on a dedicated router, map tenants onto a fixed set of router ids no larger
...
module.exports = {
  ...
  contextToCubeStoreRouterId: ({ securityContext }) => {
    const hash = crypto.createHash('md5').update(`${securityContext.tenant_id}`).digest()
    return `CUBE_ROUTER_${hash.readUInt32BE(0) % NUMBER_OF_CLUSTERS}`
  }
}
```
...
Router ids should be stable for each tenant. Pre-aggregations are stored per
router, so a tenant whose router id changes has its pre-aggregations built
again on the new router.
