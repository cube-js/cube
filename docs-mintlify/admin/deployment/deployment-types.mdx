...
Cube Store querying performance. See [Multi-router Cube Store](#multi-router-cube-store).
...
### Multi-router Cube Store {#multi-router-cube-store}

By default, all Dedicated deployments within a Multi-cluster deployment share
one Cube Store router. With **Multi-router Cube Store** enabled, distinct Cube
Store router ids are spread across separate routers, up to the deployment's
**Number of clusters**. This spreads pre-aggregation storage and query load
across Cube Store in your region.

To enable it, navigate to **Settings → Configuration**, turn on
**Multi-router Cube Store** in the **Configuration flags** section, and click
**Apply**. The option is only available on Multi-cluster deployments.

Each request's router id comes from
[`context_to_cube_store_router_id`][ref-ctx-to-cube-store-router-id], or from
[`context_to_orchestrator_id`][ref-ctx-to-orchestrator-id] if it's not defined.
Without either, all requests resolve to the same id and share one router.

Keep the number of distinct router ids at or below **Number of clusters**. To
serve many tenants, map them onto a fixed set of router ids:
...
def context_to_cube_store_router_id(ctx: dict) -> str:
  tenant_id = str(ctx['securityContext']['tenant_id'])
  return f"CUBE_ROUTER_{zlib.crc32(tenant_id.encode()) % NUMBER_OF_CLUSTERS}"
```

```js title="JavaScript"
const zlib = require('zlib')

const NUMBER_OF_CLUSTERS = 3
...
module.exports = {
  ...
  contextToCubeStoreRouterId: ({ securityContext }) => {
    ...
    return `CUBE_ROUTER_${zlib.crc32(tenantId) % NUMBER_OF_CLUSTERS}`
  }
}
```
...
Pre-aggregations are stored per router, so a tenant whose router changes has
its pre-aggregations built again on the new router. Keep router ids stable for
each tenant.

Once a Cube API instance has seen more distinct router ids than **Number of
clusters**, it connects to the shared Cube Store router until it restarts,
including for ids that had their own router before. Instances fall back
independently, so the same tenant can be served by different routers.
...
[ref-ctx-to-orchestrator-id]: /reference/configuration/config#context_to_orchestrator_id
[ref-ctx-to-cube-store-router-id]: /reference/configuration/config#context_to_cube_store_router_id
