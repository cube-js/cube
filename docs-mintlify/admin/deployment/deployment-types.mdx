...
* Optionally, enable **Multi-router Cube Store** to run a dedicated Cube
Store router per cluster instead of sharing a single router across all
clusters, allowing for increased Cube Store querying performance.
...
To give each cluster its own Cube Store router, navigate to
**Settings → Configuration** and enable **Multi-router Cube Store**. The
number of routers follows **Number of clusters**.
