...
`cubejs-cli` is deprecated. Use the new [Cube CLI][ref-cli] instead. In CI,
authenticate with an API key rather than the deploy token — set `CUBE_API_URL`
and `CUBE_API_KEY` (or run `cube login --api-key <KEY> --url <URL>`), then run
`cube deploy <deployment-id> --directory .` (find the id with
`cube deployments list`). To silence the deprecation warning `cubejs-cli`
prints to stderr, set `CUBEJS_CLI_NO_DEPRECATION_WARNING=true`.
