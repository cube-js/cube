...
[cubes](/reference/data-modeling/cube) reference data from different
databases.
...
Declare the list of data sources using the [`CUBEJS_DATASOURCES`](/reference/configuration/environment-variables#cubejs_datasources) environment
variable, then use
...
For example, using the `datasource1` data source, [`CUBEJS_DB_TYPE`](/reference/configuration/environment-variables#cubejs_db_type) could be
decorated as:
...
Use the [`data_source`](/reference/data-modeling/cube#data_source)
