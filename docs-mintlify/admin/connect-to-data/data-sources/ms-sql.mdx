...
In SQL API pushdown, use explicit comparisons for boolean dimensions backed by
`BIT` columns:
...
Bare `WHERE completed`, `NOT completed`, and `CASE WHEN completed THEN ...` are
unsupported for these dimensions. Both comparisons preserve unknown for `NULL`.
Existing conditions on predicate-backed dimensions, such as `amount > 50`, remain
supported.
A predicate-backed boolean dimension such as `big` cannot be used as a scalar
value: `SELECT big` and `COUNT(DISTINCT big)` are unsupported.
