...
description: Connect Cube to SQL Server with database host credentials, optional domain-scoped login, and SSL or pool tuning as needed.
---
...
## SQL API boolean expressions

SQL Server distinguishes scalar `BIT` values from predicates such as `amount > 50`.
A dimension declared as `type: boolean` can contain either SQL shape. When the
SQL API pushes an expression into a Cube member expression, it leaves a raw
boolean dimension reference unchanged rather than guessing its SQL shape.

For a dimension backed by a `BIT` column, use explicit comparisons:
...
Use `completed = FALSE` instead of `NOT completed`. Both comparisons preserve
unknown for a `NULL` input. Bare `WHERE completed`, `NOT completed`, and
`CASE WHEN completed THEN ...` are not supported for raw `BIT` dimensions on this
pushdown path.

For a boolean dimension whose model SQL is a predicate, such as `amount > 50`,
existing conditions like `WHERE big`, `NOT big`, and `CASE WHEN big THEN ...`
retain their predicate SQL. This does not enable using that raw dimension as a
scalar, for example `SELECT big` or `big = TRUE`.

Comparisons written directly in the SQL API query, such as `SELECT amount > 50`
or `SELECT COUNT(*) > 0`, can be converted to nullable `BIT` values because the
renderer can see the comparison. Segment references are also recognized as
predicates. No model opt-in setting is required.
