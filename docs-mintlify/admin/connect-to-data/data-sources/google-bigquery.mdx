...
description: Grant a Google Cloud service account the right BigQuery IAM roles, then point Cube at your project, region, and key material.
---
...
BigQuery is stateless: a [temporary UDF][bq-docs-temp-udf] exists only for the
duration of a single query, so there is no connection to install one on ahead of
time. Set [`CUBEJS_DB_SQL_PREAMBLE`](/reference/configuration/environment-variables#cubejs_db_sql_preamble)
and Cube prepends it to each query it sends, putting the function in scope for
the query that calls it:

```dotenv
CUBEJS_DB_SQL_PREAMBLE=CREATE TEMP FUNCTION median(x ARRAY<FLOAT64>) AS (x[OFFSET(ARRAY_LENGTH(x) DIV 2)])
```

The same value can be set as the `sqlPreamble` driver option from a custom
[`driver_factory`][ref-driver-factory], which takes precedence over the
environment variable.

<Warning>

BigQuery treats a request containing several statements as a
[script][bq-docs-multi-statement], and a script ignores job configuration such as
the destination table. The one exception is `CREATE TEMP FUNCTION` statements
followed by a single query, which BigQuery still runs as a normal query.

Because pre-aggregation builds set a destination table, Cube rejects a build
whose preamble falls outside that exempt shape rather than letting it report
success while writing an empty table. A preamble that only defines temporary
functions is unaffected.

</Warning>

[bq-docs-temp-udf]: https://cloud.google.com/bigquery/docs/user-defined-functions#temporary-udf-syntax
[bq-docs-multi-statement]: https://cloud.google.com/bigquery/docs/multi-statement-queries
...
| [`CUBEJS_DB_SQL_PREAMBLE`](/reference/configuration/environment-variables#cubejs_db_sql_preamble) | SQL run in the context of every query, typically to define temporary UDFs | A valid SQL statement or batch |    ❌    |
