...
provision or rotate. Set the `OAUTH` authenticator and point the driver at
the token file Cube maintains, omitting `CUBEJS_DB_USER` /
`CUBEJS_DB_PASS`:
...
CUBEJS_DB_SNOWFLAKE_OAUTH_TOKEN_PATH=/var/cube/<tenant-id>/<deployment-id>/cube_cloud_token_snowflake
```
