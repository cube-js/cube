...
- User name and password or an RSA private key for the Snowflake account.
  ...
  identity][ref-oidc-snowflake] instead — the same role grants apply to the
  mapped service user.
...
to Snowflake with [OIDC workload identity][ref-oidc-snowflake]: a Snowflake
External OAuth integration trusts Cube's OIDC issuer, and the driver
presents a short-lived Cube-minted token — no long-lived secrets to
provision or rotate. Set the `OAUTH` authenticator and omit
`CUBEJS_DB_USER` / `CUBEJS_DB_PASS`:

```dotenv
CUBEJS_DB_TYPE=snowflake
CUBEJS_DB_SNOWFLAKE_ACCOUNT=acme-analytics
CUBEJS_DB_SNOWFLAKE_WAREHOUSE=MY_SNOWFLAKE_WAREHOUSE
CUBEJS_DB_NAME=my_snowflake_database
...
Cube sets `CUBEJS_DB_SNOWFLAKE_OAUTH_TOKEN_PATH` automatically via the
token config's **Target Env Var** — the token file lives at a different
path in deployed pods, dev mode, and test connection, so it is never
written by hand.

See the [Snowflake OIDC guide][ref-oidc-snowflake] for the security
integration, service user, and token config setup.
...
<sup>1</sup> Required when using password-based authentication. Not required with `SNOWFLAKE_JWT` (key pair) or with `OAUTH` — including [OIDC workload identity][ref-oidc-snowflake] in Cube Cloud, where the driver reads a Cube-minted token from [`CUBEJS_DB_SNOWFLAKE_OAUTH_TOKEN_PATH`](/reference/configuration/environment-variables#cubejs_db_snowflake_oauth_token_path).
...
[ref-oidc-snowflake]: /admin/deployment/oidc/snowflake
