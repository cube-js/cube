  ...
  - name: order_events_stream
    ...
    pre_aggregations:
      - name: stream
        ...
        granularity: second
        partition_granularity: day
        # ksqlDB does not support NOW(), DATE_SUB, DATE_ADD, or date_trunc.
        # Use UNIX_TIMESTAMP() (current epoch ms) with FROM_UNIXTIME() instead.
        build_range_start:
          sql: "SELECT FROM_UNIXTIME(UNIX_TIMESTAMP() - 5 * 60 * 60 * 1000)"
        build_range_end:
          sql: "SELECT FROM_UNIXTIME(UNIX_TIMESTAMP() + 15 * 60 * 1000)"
        refresh_key:
          every: 1 minute
