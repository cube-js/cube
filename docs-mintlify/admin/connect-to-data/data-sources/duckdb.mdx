...
or with the `sqlPreamble` driver option, runs on each connection before the query
that uses it — including streamed queries, which DuckDB serves on their own
connection.

<Info>

The `initSql` driver option is deprecated in favour of `sqlPreamble` and will be
removed in a future release. It keeps its existing behaviour while it lasts: a
statement that fails is logged and skipped, whereas a failing `sqlPreamble`
surfaces the error, so a preamble meant to define a function does not fail later
as a missing function instead.
