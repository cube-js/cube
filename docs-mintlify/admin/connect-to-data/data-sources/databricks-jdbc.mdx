...
SQL set with [`CUBEJS_DB_SQL_PREAMBLE`](/reference/configuration/environment-variables#cubejs_db_sql_preamble),
or with the `sqlPreamble` driver option, runs on each pooled connection before the
query that uses it, on both the query and streaming paths.

<Info>

The `prepareConnectionQueries` driver option is deprecated in favour of
`sqlPreamble` and will be removed in a future release. Databricks ships no
built-in connection statements, so the two options behave identically here; on
drivers that do have them, `sqlPreamble` is appended after them while
`prepareConnectionQueries` replaces them, as it always has.
