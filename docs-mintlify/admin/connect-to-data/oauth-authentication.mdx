...
Rebuilding the connection when the configuration changes requires Cube
v1.7.20 or later. On earlier versions the connection resolved by the first
request is kept until the deployment restarts, so a rotated token is only
picked up by a redeploy — including the token itself in the orchestrator ID
is then the only way to pick one up without restarting, at the cost of a
fresh orchestrator per rotation.
...
def _access_token(ctx: dict):
    ...
    # No expiry advertised, so there is nothing to compare against and `status`
    # is all there is to go on. Requiring an expiry here would send every user
    # of a provider that omits it to the service account.
...
function accessToken(securityContext) {
  ...
  // No expiry advertised, so there is nothing to compare against and `status` is
  // all there is to go on. Requiring an expiry here would send every user of a
  // provider that omits it to the service account.
