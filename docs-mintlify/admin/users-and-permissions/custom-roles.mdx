...
In the role builder, every custom role has exactly one **Base Role**. It determines:

- The user's **license tier** — Cube infers the tier from the highest Base Role across all of the user's roles.
- What that tier **licenses** — the kind of work its holder may do.
...
With all three actions checked, `Org Viewer` gives the same tier and the same deployment access as the built-in Viewer. What makes it worth writing as a custom role is what you change from here: leave **Download data** off, narrow **Scope** to specific deployments, or add a [Global permission](#global-permissions) the built-in does not carry.
