...
1. **Base Role** (required in the role builder) — Viewer, Explorer, or Developer. Determines the user's license tier and the account-wide capabilities that tier carries; deployment access comes from the role's deployment policies.
...
- The **inherited** capabilities that tier carries. These are account-wide capabilities only — the Base Role grants no deployment-scoped access on its own, even where a built-in role of the same name does; that comes from the role's own deployment policies.

| Base Role  | Access the built-in role of that name has                    |
| ---------- | ------------------------------------------------------------ |
...
Those descriptions split in two where a custom role is concerned. The account-wide capabilities in them — viewing dashboards and chats, creating and editing workbooks, running queries — are gated on the tier alone, so a custom role's Base Role carries them. The deployment-scoped ones — editing the data model, managing a deployment — the built-in role grants through its own deployment policy, and a custom role needs a deployment policy of its own.
...
The descriptions are what the built-in role of that name can do. As a custom role's Base Role, the action sets the license tier and nothing deployment-scoped — see [Base Role](#base-role).

| Internal name   | Label     | The built-in role of that name                             |
| --------------- | --------- | ---------------------------------------------------------- |
