...
Four deployment-scoped actions can be checked at every tier, because checking one does not force the Base Role upward: **Access deployment** (`DeploymentRead`), **Access AI agents** (`DeploymentAgentRead`), **Download data** (`DownloadData`) and **Access chat history** (`ChatHistoryRead`). They still have to be granted by a deployment policy — the tier confers none of them. Check any other action — for example **Edit deployment** or **Edit data model** — and the Base Role is automatically forced to **Developer**, with the Viewer and Explorer options disabled behind a tooltip:
...
<Steps>
  ...
  <Step title="Add a deployment policy">
    Click **Add deployment policy**, leave **Scope** on **All deployments**, and check **Access deployment**, **Access AI agents** for Analytics Chat, and **Download data** if they should be able to export query results. The Base Role carries none of them, and none [forces the Base Role to Developer](#auto-bump-to-developer), so the role stays a Viewer.
  </Step>
  <Step title="Save">
    Click **Create**.
  </Step>
