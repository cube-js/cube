...
Those are the built-in roles' own policies at work — mostly deployment-scoped, though Developer's account-wide deployment management is a [Global permission](#global-permissions). A custom role with Base Role **Viewer** and no deployment policy licenses the holder for viewer-level work but reaches no deployment.
...
As a custom role's Base Role, the action sets the license tier and nothing deployment-scoped — see [Base Role](#base-role) for what the tier licenses.
