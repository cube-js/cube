...
As written, `Org Viewer` reproduces the built-in Viewer account-wide — assigning that role does the same thing. What makes it worth writing as a custom role is what you change from here: narrow **Scope** to specific deployments, drop an action the built-in grants, or add a [Global permission](#global-permissions) the built-in does not carry.
