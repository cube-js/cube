...
Four deployment-scoped actions sit at every tier: **Access deployment** (`DeploymentRead`), **Access AI agents** (`DeploymentAgentRead`), **Download data** (`DownloadData`) and **Access chat history** (`ChatHistoryRead`). Opening a deployment, using Analytics Chat and downloading results are viewer-level work. Chat history is exempt too, though it shows other users' conversations, so grant it deliberately. Check any other action — for example **Edit deployment** or **Edit data model** — and the Base Role is automatically forced to **Developer**, with the Viewer and Explorer options disabled behind a tooltip:
...
| Monitoring  | Access chat history                 | View other users' chat conversations for the deployment, including Analytics Chat and the Excel/Google Sheets add-in panes (**Admin → Chats History**). It does not [force the Base Role to Developer](#auto-bump-to-developer), so a Viewer-tier role can hold it — grant it only to roles that should see other people's conversations. |
...
<Steps>
  ...
  <Step title="Add a deployment policy">
    Click **Add deployment policy**, leave **Scope** on **All deployments**, and check **Access deployment**, **Access AI agents** for Analytics Chat, and **Download data** if they should be able to export dashboards and results. The Base Role carries none of them, and none [forces the Base Role to Developer](#auto-bump-to-developer), so the role stays a Viewer.
  </Step>
  <Step title="Save">
    Click **Create**.
  </Step>
