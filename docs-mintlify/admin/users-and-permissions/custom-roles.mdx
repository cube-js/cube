...
1. **Base Role** (required in the role builder) — Viewer, Explorer, or Developer. Determines the user's license tier — the kind of work the holder is licensed for; access to any deployment comes from the role's deployment policies.
...
Those descriptions are the built-in roles', and a custom role's Base Role carries only half of each. The tier licenses the kind of work — viewing dashboards and chats, creating and editing workbooks, running queries, editing the data model — but reaching the deployment where any of it happens comes from the role's own deployment policies, which the built-in roles carry and a Base Role does not. So a custom role with Base Role **Viewer** and no deployment policy grants nothing at all.
