...
Everything above is also available over the [REST API][ref-api-roles], so a team that provisions deployments from CI can grant access to the new deployment in the same pipeline instead of finishing in the console.
...
<Steps>
  ...
  <Step title="Create a role scoped to it">
    `POST /api/v1/roles` with a `Deployment` policy whose `resources` carry that id **as a string**:
...
`GET`, `PUT` and `DELETE /api/v1/roles/{roleId}` read, update and remove a role; `PUT` replaces `policies` outright rather than merging, so read the role first if you mean to add a policy. All of these require administrator privileges, and they cover your custom roles only — the built-in roles are not listed or editable, though a built-in role id can still be assigned.
