...
Each description mixes the tier with access that comes from elsewhere, and a custom role's Base Role supplies only the tier:

- **The tier licenses the kind of work** — viewing dashboards and chats, creating and editing workbooks, running queries, editing the data model. That is what the Base Role carries; whether the holder may do it on a given deployment is a separate question.
