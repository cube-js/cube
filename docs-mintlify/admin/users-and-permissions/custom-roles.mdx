...
These are the **role builder's** rules, enforced client-side before it submits; the API validates differently — see [Managing roles through the API](#managing-roles-through-the-api) below, where a policy with no actions is rejected rather than dropped.
