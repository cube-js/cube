...
Those are the built-in roles' own deployment policies at work. A custom role with Base Role **Viewer** and no deployment policy licenses the holder for viewer-level work but reaches no deployment.
