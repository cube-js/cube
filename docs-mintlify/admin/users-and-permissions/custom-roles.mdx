...
1. **Base Role** (required in the role builder) — Viewer, Explorer, or Developer. Sets that role's license tier — the kind of work it licenses the holder for.
...
In the role builder, every custom role has exactly one **Base Role**. It sets that role's **license tier**; the user's tier is the highest Base Role across all of their roles.
...
The built-in roles of the same names carry deployment policies of their own. Viewer and Explorer get `DeploymentRead`, `DeploymentAgentRead` and `DownloadData` over every deployment; Developer gets every deployment action, data-model ones included, except **Access chat history**, plus account-wide deployment management as a [Global permission](#global-permissions). A custom role starts with none of it. A role with Base Role **Viewer** and no deployment policy is licensed for viewer-level work and reaches no deployment; a role meant to work like Developer has to grant the deployment actions itself. **Full access** (`All`) is the bulk shortcut, and unlike the built-in Developer it takes chat history as well.
...
Four deployment-scoped actions can be checked at every tier: **Access deployment** (`DeploymentRead`), **Access AI agents** (`DeploymentAgentRead`), **Download data** (`DownloadData`) and **Access chat history** (`ChatHistoryRead`). Check any other action — for example **Edit deployment** or **Edit data model** — and the Base Role is automatically forced to **Developer**, with the Viewer and Explorer options disabled behind a tooltip:
...
| Group       | Permission                          | What it grants                                                                                                                                                            |
| ----------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| General     | Access deployment                   | Foundation permission required to access the deployment at all.                                                                      |
...
| Monitoring  | Access chat history                 | View other users' chat conversations for the deployment, including Analytics Chat and the Excel/Google Sheets add-in panes (**Admin → Chats History**). |
| Data Export | Download data                       | Download query results as CSV from workbooks, Analytics Chat, and published dashboards. See [Data download controls][ref-data-download-controls]. |
...
### Create a Viewer for all deployments
...
<Steps>
  ...
  <Step title="Add a deployment policy">
    Click **Add deployment policy**, leave **Scope** on **All deployments**, and check **Access deployment**, **Access AI agents** for Analytics Chat, and **Download data** if they should be able to export query results. None of them [forces the Base Role to Developer](#auto-bump-to-developer), so the role stays a Viewer.
  </Step>
  <Step title="Save">
    Click **Create**.
  </Step>
...
With all three actions checked, `Org Viewer` gives the same tier and the same deployment access as the built-in Viewer. What makes it worth writing as a custom role is what you change from here: narrow **Scope** to specific deployments, or add a [Global permission](#global-permissions) the built-in does not carry.
...
<Steps>
  ...
  <Step title="Pick actions">
    Check **Access deployment** so the user can open the deployment, **Access AI agents** for Analytics Chat, and **Download data** if they should be able to export query results — none of them [forces the Base Role to Developer](#auto-bump-to-developer). **Access chat history** does not either, but it shows other users' conversations, so leave it off unless the role should see them. Leave the rest alone — **Access query history**, for instance, does force the bump, and that costs you the Explorer this walkthrough is for.
  </Step>
  <Step title="Save">
    Click **Create**.
  </Step>
...
This section lists every action the role builder exposes. The internal action names match the names shown in the [audit log][ref-audit-log]. The API is wider in two ways: it accepts actions the builder does not render, and policies over resource types it has no card for — `Report`, `Workbook`, `Agent` and `ChatThread` among them. See the `actions` and `resourceType` enums on [`POST /api/v1/roles`][ref-api-create-role] for both full lists.

[ref-audit-log]: /admin/monitoring/audit-log

### Base Role actions (Global)

As a custom role's Base Role, the action sets the license tier — see [Base Role](#base-role) for what each tier licenses.
