...
The built-in roles of the same names carry deployment policies of their own: `DeploymentRead`, `DeploymentAgentRead` and `DownloadData` over every deployment for Viewer and Explorer, every deployment action but **Access chat history** for Developer, including the data-model ones, plus — for Developer — account-wide deployment management as a [Global permission](#global-permissions). A custom role starts with none of it. Base Role **Viewer** and no deployment policy is licensed for viewer-level work and reaches no deployment; a role meant to work like Developer has to grant the deployment actions itself. **Full access** (`All`) is the bulk shortcut, and it takes chat history too.
