...
1. **Base Role** (required in the role builder) — Viewer, Explorer, or Developer. Determines the user's license tier — the kind of work the holder is licensed for.
...
In the role builder, every custom role has exactly one **Base Role**. It determines the user's **license tier**, which Cube infers from the highest Base Role across all of the user's roles.
