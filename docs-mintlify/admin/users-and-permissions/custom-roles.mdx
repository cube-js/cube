...
Those descriptions are the built-in roles' own policies at work, not the tier's: Viewer and Explorer carry `DeploymentRead`, `DeploymentAgentRead` and `DownloadData` over every deployment, Developer's policy carries nearly every deployment action including the data-model ones, and Developer's account-wide deployment management is a [Global permission](#global-permissions). A custom role's Base Role carries none of it — with Base Role **Viewer** and no deployment policy, the holder is licensed for viewer-level work and reaches no deployment.
