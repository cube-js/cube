...
<Steps>
  ...
  <Step title="Pick actions">
    Check **Access deployment** so the user can open the deployment. Leave the rest alone: **Access query history** and everything else outside the four exempt actions [force the Base Role to Developer](#auto-bump-to-developer), so adding one here costs you the Explorer this walkthrough is for.
  </Step>
  <Step title="Save">
    Click **Create**.
  </Step>
