...
| --- | --- | --- |
| Language | Set the display language for the Cube Cloud console and transactional emails | English (US) |
...
The **Language** preference sets the display language for the entire Cube Cloud
console UI. It is **per-user** — each user picks their own language, and the choice
does not affect anyone else in the account. The same language also determines the
locale of the transactional emails you receive (for example,
[budget alerts][ref-budgets]).

Cube supports the same 12 languages listed for
[embedded surfaces](/embedding/iframe/localization#supported-languages), with
English (US) as the default. You can also switch languages from the sign-in,
sign-up, and forgot-password pages using the language switcher before you log in.

<Note>

Setting a language for [embedded surfaces](/embedding/iframe/localization) —
dashboards, Analytics Chat, and Creator Mode — is configured separately and is
