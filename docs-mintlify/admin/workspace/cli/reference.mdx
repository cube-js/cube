...
descriptiCommand-by-command reference for cubejs-cli covering app scaffolding, model generation, validation, JWT creation, and Cloud deploy.loy.
---
...
[`CUBEJS_API_SECRET`](/reference/configuration/environment-variables#cubejs_api_secret) environment variable or provided value with `-s` flag. You
...
| -s, --secret           | Cube app secret. Also can be set via environment variable [`CUBEJS_API_SECRET`](/reference/configuration/environment-variables#cubejs_api_secret) | -                          |
