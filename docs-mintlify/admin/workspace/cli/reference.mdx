...
description: Command-by-command reference for `cubejs-cli` covering app scaffolding, model generation, validation, JWT creation, and Cloud deploy.
---
...
an active [database connection](/docs/configuration/data-sources).
...
[`CUBEJS_API_SECRET`](/docs/configuration/reference/environment-variables#cubejs_api_secret) environment variable or provided value with `-s` flag. You
...
| -s, --secret           | Cube app secret. Also can be set via environment variable [`CUBEJS_API_SECRET`](/docs/configuration/reference/environment-variables#cubejs_api_secret) | -                          |
...
[ref-ci]: /docs/deployment/cloud/continuous-deployment#deploy-with-cli