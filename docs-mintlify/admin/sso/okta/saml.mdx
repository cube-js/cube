...
auto-provisioned** on first login. Each matching group adds its mapped
role to the new user, on top of the
[default role](#default-role-for-new-users). Existing users are not
modified on subsequent logins.
