...
uses — without driving a browser through the console. Requires the
`WorkbookManage` permission.

Rendering happens asynchronously, since a capture waits for every widget on
the dashboard to finish querying: submit a job, poll it until it completes,
...
[ref-api-reference]: /api-reference/introduction
