...
Send a `GET` request to export [audit log][ref-audit-log] events for your
Cube Cloud account. This endpoint allows you to programmatically retrieve
security-related events for compliance reviews, external log aggregation,
or integration with SIEM tools.

<Info>

Requires [Audit Log][ref-audit-log] to be enabled on the
[Enterprise Premier plan](https://cube.dev/pricing).
...
curl \
  -H "Authorization: Api-Key YOUR_API_KEY" \
  "https://YOUR_CUBE_CLOUD_HOST/api/v1/audit-log/events"
```
