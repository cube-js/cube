...
uses — without driving a browser through the console. Identify the dashboard
with either `dashboardId` (numeric) or `dashboardPublicId` (string) — exactly
one, not both. Requires **manage** access to the dashboard's workbook, the
same permission the console's whole-dashboard download requires.
...
A tenant administrator can also render the dashboard as if a specific other
...
per person, and only the identity that submitted a job can poll or download
it.

Submitting returns `404` for a dashboard that doesn't belong to the
deployment, a request that supplies both ids or neither, or a deactivated
