...
A `cube.ts` configuration file is no longer supported — use `cube.js` or
`cube.py` instead. A server that finds only `cube.ts` fails to start with
`Configuration in cube.ts is no longer supported`.
...
The example maps tenants onto a fixed set of router ids. In Cube, multiple Cube
Store routers are available on Multi-cluster deployments with
[Multi-router Cube Store][ref-multi-router-cube-store] enabled, and the number
of distinct ids should stay at or below **Number of clusters**.
...
Cube's own drivers can also be loaded with a named ESM import instead of `require`:
...
In the Cube cloud platform, all [environments][ref-environments] of a deployment
share the same pre-aggregations schema and Cube Store. This allows reusing
pre-aggregations across environments, saving both time and cost. See [Deployment
environments → Pre-aggregations][ref-environments-preaggs] for what to expect
when a branch changes a pre-aggregation.
...
In Cube Core, it's **strongly** recommended to use different pre-aggregation
schemas in development and production environments to avoid pre-aggregation
table clashes.
...
Filter values, SQL parameters, and string literals in SQL API statements in `params`
arrive as `redacted` when
[`CUBEJS_LOG_REDACTION`](/reference/configuration/environment-variables#cubejs_log_redaction)
is on (the default outside development mode); error messages from a data source arrive as
returned, and a SQL API error that quotes the statement has it redacted.
...
[ref-multi-router-cube-store]: /admin/deployment/deployment-types#multi-router-cube-store
...
[ref-environments-preaggs]: /admin/deployment/environments#pre-aggregations
