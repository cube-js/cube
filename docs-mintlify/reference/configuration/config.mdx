...
You can pass this object to set advanced options for the query orchestrator. It can also
be set to a function of the request context (`Callable[[RequestContext], Dict]` in Python,
`(context) => object` in JavaScript) to vary
the options per tenant — with the caveat that only the object form is validated. Bounds
documented below, such as the maximum for `continueWaitTimeout`, are not enforced on the
dict a function returns, and nothing clamps the values downstream.
