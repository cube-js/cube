...
In Cube, multiple Cube Store routers are available on Multi-cluster deployments
with [Multi-router Cube Store][ref-multi-router-cube-store] enabled.
...
On Multi-cluster deployments, keep the number of distinct ids at or below
**Number of clusters**. See [Multi-router Cube Store][ref-multi-router-cube-store]
for an example that maps many tenants onto a fixed set of ids.
