...
`context_to_app_id` enables multitenancy for data model compilation only.
Querying and caching stay shared across app ids: database connections, query
queues, the query results cache, and pre-aggregations, including their tables in
the pre-aggregations schema. To isolate those per tenant as well, define
[`context_to_orchestrator_id`][self-orchestrator-id] and
[`pre_aggregations_schema`][self-pre-aggregations-schema]. See
[multitenancy][ref-multitenancy] for details.
