...
driver cannot move a deadline the factory keeps re-asserting. So is one less
than 30 seconds away — replacements are rate-limited to that interval, so a
shorter lifetime could only ever be honoured by replacing the connection on
every window. A credential rotating that fast is picked up by its configuration
changing instead.
