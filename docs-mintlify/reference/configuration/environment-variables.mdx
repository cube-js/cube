...
Postgres, MySQL and the JDBC-based drivers it runs on each pooled connection, and
on BigQuery it is prepended to the query itself. In every case the query executes
in the preamble's context, streamed queries included.
