...
The maximum number of SQL queries that Cube Store may build a logical plan for
at the same time. Query planning runs before the result cache, so a burst of
queries can otherwise enter it all at once, oversubscribing the available
cores and holding every in-flight plan's intermediate state in memory. Extra
queries beyond the limit wait for a free slot, up to
[`CUBESTORE_MAX_QUEUED_QUERY_PLANS`](#cubestore_max_queued_query_plans).

Set to `0` to disable the limit. `auto` (the default) sizes the limit to
twice the CPU cores available to the process, with a floor of `4`.
...
The maximum number of SQL queries that may wait for a query-planning slot once
