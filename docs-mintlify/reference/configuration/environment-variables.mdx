...
Keep this option enabled. With `false`, orphaned table collection only removes
*older versions* of each pre-aggregation table; it always keeps the most recent
version of every table present in the pre-aggregation schema. Tables that the
current data model no longer builds are therefore never dropped, e.g., partitions
that have rotated out of the [build
range](/reference/data-modeling/pre-aggregations#build_range_start-and-build_range_end),
or partitions left behind after a rollup is renamed or removed or its partition
granularity is changed.

These partitions accumulate in Cube Store indefinitely and inflate its metastore.
A large metastore slows down metastore operations, which degrades the performance
of every query served from a pre-aggregation and can eventually bring down the
Cube Store cluster.

If partitions that are still in use are being dropped, address the missing touches
rather than disabling this option: raise
[`CUBEJS_TOUCH_PRE_AGG_TIMEOUT`](/reference/configuration/environment-variables#cubejs_touch_pre_agg_timeout)
and/or add Refresh Worker capacity so every partition is refreshed or freshness-checked
within the timeout. For externally refreshed pre-aggregations, make sure each one is
refreshed at least once within that timeout.
