...
If `true`, enables [development mode](/docs/data-modeling/dev-mode). Otherwise
development mode is off, and `NODE_ENV` is ignored — except that the `cubejs dev-server`
command and the `cubejs-dev-server` bin turn it on for themselves when this variable is
unset, and code that embeds `@cubejs-backend/server-core` directly can do the same with
the `devServer` option it passes to `CubejsServerCore`. Setting this variable to `false`
overrides both commands; the `devServer` option takes precedence over it.
...
`NODE_ENV` used to enable development mode whenever it was anything but `production`,
which put an instance with no `NODE_ENV` at all into development mode. That is
deprecated and no longer happens; Cube prints a warning if it sees a non-production
`NODE_ENV` with `CUBEJS_DEV_MODE` unset. Set `CUBEJS_DEV_MODE=true` instead.
...
It is off by default in development mode — whether that comes from
[`CUBEJS_DEV_MODE`](#cubejs_dev_mode), from a dev server command, or from the
`devServer` option an embedder passes to `CubejsServerCore` — so runnable SQL
statements can be read from the console.
...
When this variable is unset, Cube writes the schema it resolved into it at startup, so
that drivers reading the variable directly use the same schema as the rest of the
instance. A value you set yourself is never overwritten.
