...
The maximum length of a chain of [multi-stage][ref-multi-stage-calculations] members reachable from
any single queried member. Past this depth, the query is refused with an error naming
the depth reached, the budget, and this variable, instead of aborting the process.

| Possible Values | Default in Development | Default in Production |
| --------------- | ---------------------- | --------------------- |
| A valid number  | `32`                   | `32`                   |
...
SQL queries stop entirely, including the ones issued by scheduled refresh.
...
honoured: when set, the locally computed value is cached and renewed exactly like the SQL
result, and scheduled refresh warms it without querying the database. Without an
override, local keys are computed from the current time on every check.
...
Required when using an AWS instance role. In web identity mode, the default is a
30-second poll, any override is capped at 5 minutes, and `0` disables the refresh
(STS credentials then expire after one session).
...
before they expire or when the token file changes.
...
The thread stack size, in bytes, Cube Store's main process reserves for its async
runtime, including blocking threads. Raise it if a deeply nested query plan aborts the
process with a stack overflow before [`CUBESTORE_MAX_QUERY_PLAN_DEPTH`](#cubestore_max_query_plan_depth)
would refuse it; the reservation is virtual memory, committed lazily, so raising it
does not increase resident memory use.

| Possible Values          | Default in Development | Default in Production |
| ------------------------ | ---------------------- | --------------------- |
| A valid number of bytes | `8388608` (8 MiB)        | `8388608` (8 MiB)     |

## `CUBESTORE_MAX_QUERY_PLAN_DEPTH`

The maximum nesting depth of a query plan Cube Store will serialize and execute. Past
this depth, the query is refused with an error naming the depth reached, the budget, and
this variable, instead of failing during plan decoding.

| Possible Values | Default in Development | Default in Production |
| --------------- | ---------------------- | --------------------- |
| A valid number  | `150`                  | `150`                  |
...
The maximum nesting depth Cube Store's SQL parser allows in a single query, applied to
both the router's own queries and streaming sources. Past this depth, parsing is refused
with an error naming the depth reached, the budget, and this variable, instead of an
opaque internal error.

| Possible Values | Default in Development | Default in Production |
| --------------- | ---------------------- | --------------------- |
| A valid number  | `128`                  | `128`                  |
