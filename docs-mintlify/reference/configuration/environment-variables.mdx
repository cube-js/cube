...
statements are replaced with `redacted` before a log event reaches the logger, so
personal data passed in queries does not end up in log storage. Member names, numeric
