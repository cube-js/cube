...
Storage. If not set, Cube Store falls back to `CUBESTORE_AZURE_SAS_TOKEN` and
then to workload identity or managed identity (see
[`CUBESTORE_AZURE_CONTAINER`](#cubestore_azure_container)).
...
Storage as persistent storage; setting it selects the Azure backend.

When neither `CUBESTORE_AZURE_ACCESS_KEY` nor `CUBESTORE_AZURE_SAS_TOKEN` is
set, Cube Store authenticates with workload identity using the standard
`AZURE_CLIENT_ID`, `AZURE_TENANT_ID` and `AZURE_FEDERATED_TOKEN_FILE`
environment variables, or with the managed identity of the host (IMDS) if
those are not present either.
...
the SAS token is set, workload identity or managed identity is used.
