...
Supported by the BigQuery, Snowflake, Postgres, CrateDB, Materialize, MySQL,
DuckDB and JDBC-based (including Databricks) drivers.
...
every pre-aggregation on that data source. Cube logs a warning when it detects
this.
...
Postgres, CrateDB, Materialize, MySQL, DuckDB and the JDBC-based drivers it runs
on each pooled connection, and on BigQuery it is prepended to the query itself.
In every case the query executes in the preamble's context, streamed queries
included.
