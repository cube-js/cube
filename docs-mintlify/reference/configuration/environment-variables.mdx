...
If `true`, enables [development mode](/docs/data-modeling/dev-mode). Development mode
is off unless this variable is set to `true`, and `NODE_ENV` is ignored. Code that
embeds `@cubejs-backend/server-core` directly can also turn it on with the `devServer`
option it passes to `CubejsServerCore`, which takes precedence over this variable.
