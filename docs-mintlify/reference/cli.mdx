...
outlast a single attempt. The compile and SQL generation ahead of it are unbounded and
add to that, which on a cold first query is where the two could still cross 95s. The next
attempt finds the compile already done, so what this looks like in a log is a first
attempt aborting at `--max-time` with `no answer from the API (000)` on a gate that then
passes — not a network fault. That cap applies to an `orchestrator_options` **object** — the function form used
for multi-tenancy is validated as a function, so the options it returns are never checked
and nothing clamps them downstream. If yours returns something larger, `--max-time` has
to clear it.
