...
trigger `--ref` gets an empty string, which is why the run stops there. The one exception
is `data-model put --branch`, which reads an empty value as "not specified" instead: its
fallback is your own active dev-mode branch, so an unnamed target there is neither shared
nor production, and nothing is sent. Every flag this recipe uses refuses one.
