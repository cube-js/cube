    ...
    run: |
      ...
      for _ in $(seq 30); do
        RES=$(curl -sG --fail-with-body "$API/dev-mode/$DEV_BRANCH/cubejs-api/v1/load" \
          ...
          --data-urlencode 'query={"measures":["dbt_fct_orders.count"]}') \
          || { echo "$RES"; exit 1; }
        if jq -e '.data' <<<"$RES"; then OK=1; break; fi
        jq -e '.error == "Continue wait"' <<<"$RES" >/dev/null || { echo "$RES"; exit 1; }
        sleep 5
      done
      [ -n "$OK" ] || { echo "query never returned data: $RES"; exit 1; }
...
are expected to retry until a result appears. A first query against a cold dev-mode
environment and an uncached warehouse is exactly when that happens, so treating it as a
failure would fail the gate on a model that is fine. `--fail-with-body` rather than `-f`
keeps the diagnostic on a 4xx — with `-f` the body is discarded, and naming a measure
the sync didn't generate fails with an empty log instead of *"Cube 'nope' not found for
path 'nope.count'"*.
