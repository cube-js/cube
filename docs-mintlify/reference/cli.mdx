...
Every branch argument this recipe passes refuses an empty value rather than letting it
mean "whichever branch the server would have picked" — `dbt sync --ref`, `data-model
dev-mode`, and `deployments build-status --branch`. So a gate that gets this wrong fails
instead of reporting green, and it fails at the source rather than later. Note that
...
`branchName`, which leaves the server to decide what one means. Pass such a flag only
with a name in hand, or leave it off and take the documented default.
