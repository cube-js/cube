...
steps:
  ...
  - name: Compile and query the generated model
    ...
    run: |
      ...
      OK=
      for _ in $(seq 30); do
        CODE=$(curl -sSG -o res.json -w '%{http_code}' \
      ...
      [ -n "$OK" ] || { echo "query never returned data (last status $CODE):"; cat res.json; exit 1; }
...
JSON at all. So a body carrying `error` is Cube answering — the gate fails immediately
and prints it, which for a mistyped measure means *"Cube 'nope' not found for path
'nope.count'"* in one second rather than after two minutes of retries. Anything else —
a reset, a DNS failure, an ingress error page — is retried, and `-sS` keeps curl's
message visible instead of leaving a blank line where the reason should be. Exhausting
the retries fails too, so a query that never answers can't pass quietly.
