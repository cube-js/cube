...
Check that `$DEV` is set before the two lines that use it. An interactive shell has no
...
two lines later does refuse it, so the sequence fails eventually, but only after `commit`
has already pushed. In a script, `set -o pipefail` and a `[ -n "$DEV" ]` guard cover it.
...
steps:
  - name: Sync the dbt branch under review
    shell: bash   # the `case` below is a bash range, not one inherited by luck
    run: |
      # `dbt sync` can only verify a ref whose first character it recognises, so a
      # convention it can't check would run this gate unverified. Reject it here, before
      # a sync spends minutes creating a branch this step would then abandon. LC_ALL=C
      # keeps the range ASCII: bash matches it by collation until 5.0, and the macOS
      # runners still ship 3.2.
      export LC_ALL=C
      case "$GITHUB_HEAD_REF" in
        [A-Za-z0-9_.-]*) ;;
        *) echo "this gate needs a branch name starting with a letter, digit, _, . or -: $GITHUB_HEAD_REF"; exit 1;;
      esac
      cube dbt sync "$DEPLOYMENT_ID" --ref "$GITHUB_HEAD_REF" --wait --json > sync.json
      # Read the branch name BEFORE the assertion below: a sync that reaches this point
      # has already created a branch, and a failing assertion exits the step before
      # $GITHUB_ENV is written, which skips the prune step. This message is then the only
      # place the job says which branch to sweep.
      BRANCH=$(jq -r .branchName sync.json)
      # The gate only means anything if the sync used the ref, so refuse to continue
      # when `dbt sync` could not confirm it — a ref starting with `#` or `@`, say.
      jq -e '.refVerified == true' sync.json >/dev/null || {
        echo "sync not verified to have used $GITHUB_HEAD_REF"
        echo "prune the branch it created: cube data-model delete-branch $DEPLOYMENT_ID '$BRANCH'"
        exit 1
      }
...
`continueWaitTimeout` [orchestrator option][ref-conf-orchestrator-opts] returns [`Continue
...
95 is deliberate: `continueWaitTimeout` — the only part of a request Cube bounds — is
capped at 90 where Cube validates it, so no accepted value can make the long poll itself
outlast a single attempt, and the two numbers never need keeping in sync. That cap applies
to an `orchestrator_options` **object**: the function form used for multi-tenancy is
validated as a function, so the options it returns are never checked and nothing clamps
them downstream. If yours returns something larger, `--max-time` has to clear it.
...
for what is really two timeouts disagreeing. Raising it instead multiplies the budget by the
attempt count, which is why the attempts came down from 30 as the cap went up. With the
default `continueWaitTimeout` of 10s, a query that keeps long-polling gets about five
minutes before the loop gives up.

The compile and SQL generation ahead of the long poll are unbounded and add to it, so on a
cold first query the two can still cross 95s even at an accepted value. The next attempt
finds the compile already done, so in a log that shows up as a first attempt aborting at
`--max-time` on a gate that then passes — not a network fault. That is why the loop keeps
curl's exit status rather than flattening every failure to one code: an abort logs
`no answer from the API (curl-28)`, a name-resolution failure `(curl-6)`, a reset
`(curl-56)`, so the line itself tells you which you are looking at. (`-sS` also leaves
curl's own message on stderr, but stderr interleaving in CI logs is not something to rely
on.)

The body is deleted before each attempt because curl leaves the previous file in place when
it fails before the transfer starts (`--remove-on-error` does not change that), and
without the `rm` a later transport failure would re-read an earlier body — making a stale
`Continue wait` look like work still in progress.
...
**`--ref` is verified, not assumed.** A deployment that predates `ref` support drops the
field silently and syncs the branch saved on the dbt integration, which would compile the
wrong code and still pass. The response doesn't say which ref was used, so `dbt sync`
checks the branch the server created: the requested ref appears in it (`--ref main` gives
`dbt-sync/main-…`), and a sync that ignored the ref doesn't mention it. When it doesn't
match, the command fails instead of reporting a result for code it can't confirm was
compiled, and names the sync so you can cancel it. Passing `--branch` skips the check,
because then the branch name is yours and carries no ref to look for.

A few legal ref names can't be checked this way — `#1234-fix` and `@release` begin with
characters whose treatment isn't known, so there's nothing to compare. Those syncs run,
with a warning on stderr and `"refVerified": false` in the JSON document — from `--wait`
or from a bare `--json` start, since a pipeline that follows the sync in a later step has
no `--ref` left to check. A verified run reports `true`, and `null` means
there was nothing to verify. The recipe above rejects an uncheckable branch name *before* syncing, because whether a
ref can be checked depends only on the ref — so paying for a sync it is going to reject
buys nothing, and leaves a branch behind. (The assertion after the sync names that branch
so it can still be swept, but not creating it is better.) Its character set mirrors the CLI's, which
is the source of truth: if that set grows, the shell guard is the copy that goes stale, and
the assertion after the sync is what still holds either way. It then still asserts `refVerified == true`
rather than trusting it, which also fails on `null`: that recipe always passes `--ref` and
never `--branch`, so `null` would mean something changed underneath it. If you're choosing a
branch convention for a gate, one starting with a letter or digit is the one that gets
checked.
...
All four branch arguments this recipe passes refuse an empty value rather than letting it
mean "whichever branch the server would have picked" — `dbt sync --ref`, `data-model
dev-mode`, `data-model delete-branch`, and `deployments build-status --branch`. So a gate
that gets this wrong fails instead of reporting green, and it fails at the source rather
than later. Note that `$GITHUB_HEAD_REF` is only set on `pull_request` events; on any
other trigger `--ref` gets an empty string, which is why the run stops there.

Optional `--branch` flags outside that set take an empty value and send it as an empty
field, which leaves the server to decide what one means. (The field is `branchName` on
most endpoints and `branch` on `github connect`.) Pass such a flag only with a name in
hand, or leave it off and take the documented default.

The API key needs more than the sync itself: `SchemaUpdate` to start or cancel a sync,
enter dev mode and delete branches, `SchemaRead` to follow a sync and read the data model,
and `DeploymentRead` to read build status, resolve the deployment's URL and mint a Core
Data APIs token. `SchemaUpdateDevBranches` exists as a separate action for dev-branch
edits, and the gate's middle steps are dev-branch operations — whether `SchemaUpdate`
subsumes it isn't something this page has verified, so include it if your role separates
the two. `All` avoids the question at the cost of granting everything else. Its
scope must include this deployment — see [API keys][ref-api-keys] and [custom
roles][ref-custom-roles]. Any failure — a failed sync, a failed compile, a
...
[ref-conf-orchestrator-opts]: /reference/configuration/config#orchestrator_options
