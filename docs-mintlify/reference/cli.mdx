...
with a warning on stderr and `"refVerified": false` in the `--wait --json` document; a
verified run reports `true`, and `null` means there was nothing to verify. If you're
choosing a branch convention for a gate, one starting with a letter or digit is the one
that gets checked.
