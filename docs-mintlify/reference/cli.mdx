...
steps:
  ...
  - name: Release the dev-mode session
    if: always()
    run: cube data-model exit-dev-mode "$DEPLOYMENT_ID" || true
```
...
at once will re-point each other's. The `if: always()` step above releases it even
when the compile or query fails.
