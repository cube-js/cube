...
steps:
  ...
  - name: Compile and query the generated model
    ...
    run: |
      ...
      curl -sfG "$API/dev-mode/$DEV_BRANCH/cubejs-api/v1/load" \
        -H "Authorization: $TOKEN" \
        --data-urlencode 'query={"measures":["dbt_fct_orders.count"]}' | jq -e '.data'
...
and an HTML page, which `curl -sf` accepts happily. Piping to `jq -e '.data'` is what
turns "some 200 came back" into "the model answered a query", so the gate fails on an
error body instead of passing. Name a measure the sync generated, not one that already
existed, or the step passes without testing the new model at all.
