...
95 is deliberate: `continueWaitTimeout` is capped at 90 where Cube validates it, so no
accepted value can outlast a single attempt and the two numbers never need keeping in
sync. That cap applies to an `orchestrator_options` **object** — the function form used
