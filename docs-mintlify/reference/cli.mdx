...
there was nothing to verify. The recipe above asserts `refVerified == true` rather than
trusting it, which also fails on `null`: that recipe always passes `--ref` and never
`--branch`, so `null` would mean something changed underneath it. If you're choosing a
