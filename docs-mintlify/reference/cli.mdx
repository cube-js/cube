...
| `dbt` | dbt sync and cube generation — see [dbt commands](#dbt-commands) |
...
| Command | Description |
| --- | --- |
| `dbt sync` | Start a dbt sync (`--ref` or `--manifest`, `--wait`) |
| `dbt generate` | Generate cubes from a dbt manifest and write them locally, committing nothing (`--manifest`, `--out`, `--check`) — see [Generate cubes without committing them](#generate-cubes-without-committing-them) |
