...
steps:
  - name: Sync the dbt branch under review
    run: |
      # `dbt sync` can only verify a ref whose first character it has measured, so a
      # convention it can't check would run this gate unverified. Reject it here, before
      # a sync spends minutes creating a branch this step would then abandon.
      case "$GITHUB_HEAD_REF" in
        [A-Za-z0-9]*) ;;
        *) echo "branch names for this gate must start with a letter or digit: $GITHUB_HEAD_REF"; exit 1;;
      esac
...
behind that the failed step can no longer name. It then still asserts `refVerified == true`
