...
steps:
  ...
  - name: Prune the branch the sync created
    run: cube data-model delete-branch "$DEPLOYMENT_ID" "$BRANCH"
```
...
Each sync creates a branch, so a gate that runs per pull request would accumulate
them — hence the prune step. It deliberately has no `if: always()`: on a **failure**
the branch is the evidence, holding the generated cubes that broke the build, so it
is left for someone to open. On success it has served its purpose.
