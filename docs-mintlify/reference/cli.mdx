...
steps:
  ...
  - name: Compile and query the generated model
    ...
    run: |
      ...
      for _ in $(seq 30); do
        rm -f res.json
        CODE=$(curl -sSG -o res.json -w '%{http_code}' --max-time 60 \
...
The loop exists because not every `200` is an answer: a query that takes longer than
`CUBEJS_CONTINUE_WAIT_TIMEOUT` returns [`Continue wait`][ref-rest-api-continue-wait]
...
The step's own budget is `--max-time 60` per attempt, 30 attempts, 5s apart: about half
an hour at worst, which matters because without a per-attempt cap a host that accepts the
connection and never answers would hang until the *job* timeout (six hours by default) —
a gate that never returns is worse than one that fails.
`CUBEJS_CONTINUE_WAIT_TIMEOUT` decides how many of those attempts a cold first query
spends — but only while it stays below `--max-time`. It defaults to 10s, so 60s is
comfortably clear of it; if your deployment has raised it past 60s, raise `--max-time`
too, or curl aborts every long poll before Cube can answer and the loop reports a network
failure for what is really two timeouts disagreeing. The body is deleted before each attempt because curl leaves the previous file in
