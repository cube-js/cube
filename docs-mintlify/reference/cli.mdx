...
`shell: bash` is there for `pipefail`. Actions' default `run:` shell is `bash -e`
**without** it, so in `cube … | jq -r .branchName` the pipeline takes `jq`'s exit
status: a failed `cube` call would pass silently and leave the variable empty. The
CLI refuses an empty `--branch` rather than letting it mean "whichever branch the
deployment is on", but failing at the source beats failing three steps later.
