...
partially supports the MySQL protocol and accepts a set of SQL commands beyond
`SELECT`. They fall into three tiers:

- [Diagnostics](#diagnostics) — inspect pre-aggregations and query plans. Safe
  to run at any time.
...
You can still run the [diagnostic commands](#diagnostics) there: in the SQL
Runner, pick the `cache` data source and run them against Cube Store. Only
read-only statements are accepted on that path, so the [cache and
queue](#cache-and-queue) and [store maintenance and
recovery](#store-maintenance-and-recovery) commands below are rejected. If you
need one of those, contact support.

</Note>

## Diagnostics

These commands only read state and are safe to run on a healthy cluster.
...
```sql
QUEUE ADD [EXCLUSIVE] [PRIORITY priority] [ORPHANED timeout] key 'value';
QUEUE GET key;
...
teardown begins the previous state cannot be restored. Use it only when the
cachestore is unrecoverable by other means.
