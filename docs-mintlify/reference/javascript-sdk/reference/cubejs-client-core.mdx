...
Allows you to fetch data and receive updates over time. See [Real-Time Data Fetch](/reference/recipes/real-time-data-fetch)
...
------ | ------ | ------ | --- |
`cache` | [`CacheMode`](#types-cache-mode) | ✅ Yes | Server-side cache policy for query execution. Does not control client-side caching. See [cache control][ref-cache-control] for details on available strategies. |
