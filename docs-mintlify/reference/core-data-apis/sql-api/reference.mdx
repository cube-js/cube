...
| `DATE_TRUNC` | Truncate a timestamp to specified precision. Only [default granularities][ref-granularities] are supported, see below | ✅ Yes | <nobr>✅ Outer</nobr><br/><nobr>✅ Inner (selections)</nobr><br/><nobr>✅ Inner (projections)</nobr> |
...
via the [REST API][ref-rest-api] and the [GraphQL API][ref-graphql-api]. Using a
custom granularity name results in an error:
