...
| ----------- | --------------------------------------------------------------------------------------------------------------------- | --- |
| `query`     | Either a single URL encoded Cube [Query](/reference/core-data-apis/rest-api/query-format), or an array of queries    | ✅ Yes |
...
| `responseFormat` | Output format of the result `data` payload: `default` (row-oriented), `compact` (a `{ members, dataset }` object with rows of primitive arrays), or `columnar` (a `{ members, columns }` object with one primitive array per member). `default` by default | ❌ No |
...
| --- | --- | --- |
| `format`, `string` | Query format:<br/>`sql` for [SQL API][ref-sql-api] queries,<br/>`rest` for [REST (JSON) API][ref-rest-api] queries (default) | ❌ No |
...
| `dataSource`, `string` | The name of the [data source][ref-datasources] the generated query targets |
...
In Cube Cloud, consider using the [Metadata API][self-metadata-api] instead.
...
per chunk is set by
[`CUBESQL_CUBE_SCAN_MAX_BATCH_ROWS`](/reference/configuration/environment-variables#cubesql_cube_scan_max_batch_rows).
...
[`/v1/load`](#base_path/v1/load) or [`/v1/cubesql`](#base_path/v1/cubesql).
...
[Control Plane API][ref-control-plane-api].
...
| Parameter | Description | Required |
| --- | --- | --- |
| `offset` | Pagination offset. Defaults to `0` | No |
| `limit` | Maximum number of items to return. Defaults to all items | No |
...
Example response. `data` is an array of entities; `pagination` reports the
...
- `table_references` - Data lineage to source tables, each with `data_source`,
  `table`, and, if the table is qualified, `schema`
- `cube_references` - Relationships to other cubes, each as an object with
  `type` (currently always `upstream`) and `cube`
...
If neither `cubes` nor `views` is provided, all cubes and views are returned.
...
Example response. `data` is an array of entities, each in the same format as the
      ...
      "measures": [
        {
          ...
          "member_references": [
            { "cube": "orders", "member": "total_amount" }
          ],
          "column_references": [
            {
              "data_source": "default",
...
[ref-basepath]: /reference/core-data-apis/rest-api#base-path
[ref-datasources]: /admin/connect-to-data/multiple-data-sources
...
[ref-query-wpd]: /reference/core-data-apis/queries#query-with-pushdown
[ref-sql-api]: /reference/core-data-apis/sql-api
