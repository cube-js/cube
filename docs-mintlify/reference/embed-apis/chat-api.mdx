...
| `sessionSettings.timezone` | string | No | IANA time zone the agent runs its queries in for this turn (e.g. `America/New_York`), so relative filters and time buckets land on the viewer's day. Also accepted nested as `sessionSettings.settings.timezone`, the shape the [Generate Session API](/reference/embed-apis/generate-session#session-settings) uses — the flat field wins if both are sent. Falls back to the deployment's default time zone if omitted. |
...
- **`sessionSettings`** (object, required for API key authentication; ignored
  ...
  - **`timezone`** (string): IANA time zone the agent runs its queries in for this turn. Also accepted nested as `sessionSettings.settings.timezone`, for parity with the Generate Session API's `settings` object — the flat field wins if both are sent.
