---
title: Chat API
description: The Chat API enables real-time conversations with AI agents for analytics and data exploration.
---
...
| `sessionSettings.allowChatWorkspaceAuthoring` | boolean | No | Restricts this session from creating or modifying persistent workspace content when set to `false`. Ad-hoc analysis and inline tables/charts remain available, but the agent cannot save or update standalone explorations/reports, create or modify workbooks, or create or publish dashboards. Omitted or `true` preserves the user's role-derived authoring capabilities and never grants more. |
| `sessionSettings.timezone` | string | No | IANA time zone of this session's user, e.g. `America/New_York`. Queries the agent runs are executed in it. Must be set explicitly — never inferred from `securityContext` or `userAttributes`. Omitted or unusable falls back to the deployment's default time zone rather than failing the session. |
...
<Info>
The API returns streaming JSON responses. Each line contains a separate JSON object representing a chat message, a state update, or a usage frame.
</Info>

### Response Fields

- **`id`** (string): Unique message identifier. Sentinel values: `__cutoff__`, `__state__`, and `__usage__`
- **`role`** (string): Message sender: `"user"` or `"assistant"`. Absent on error lines and on the `__usage__` frame
- **`error`** (string): Present only on error lines — mid-stream failures arrive as a standalone `{ "error": "<message>" }` line rather than a non-200 status, and such a line carries no message fields
...
- **`sort`** (number): Message ordering sequence number. Absent on the `__usage__` frame
- **`state`** (object): Current streaming state information
- **`type`** (string): Present only on the `__usage__` frame, where it is always `"usage"`
- **`usage`** (object): Present only on the `__usage__` frame — `models` (string[]), `llmCalls`, `inputTokens`, `outputTokens`, `cacheWriteTokens`, `cacheReadTokens`, `totalTokens` (numbers)

The stream always ends with one additional frame, `id: "__usage__"` (see the Usage Message example below). Treat it like the `__cutoff__` and `__state__` frames: filter it out of anything rendered as a chat message.
...
```json
{
  ...
  "usage": {
    ...
    "inputTokens": 18412,
    "outputTokens": 2280,
    "cacheWriteTokens": 31006,
    "cacheReadTokens": 264118,
    "totalTokens": 315816
  }
}
```

This is always the last frame on the stream, sent whether the run completed, was aborted, or errored — when the request is rejected before any LLM call, the counts are all `0` and `models` is empty. `usage.models` lists the distinct models used, and for a [BYOM](/admin/ai/bring-your-own-model) model is normalized to strip infrastructure detail (e.g. a Bedrock inference-profile ARN is reduced to its model name) rather than the raw configured identifier. The frame reports tokens only — there is no cost field.
