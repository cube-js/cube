...
| `sessionSettings.timezone` | string | No | IANA time zone the agent runs *this chat turn's* queries in (e.g. `America/New_York`). Also accepted, at lower priority, nested as `sessionSettings.settings.timezone` — see [Request Body](#stream-chat-state-request-body) below. Falls back to [`CUBEJS_DEFAULT_TIMEZONE`](/reference/configuration/environment-variables#cubejs_default_timezone) if neither is set or valid. |
