...
| `sessionSettings.timezone` | string | No | IANA time zone the agent runs *this chat turn's* queries in (e.g. `America/New_York`): time dimensions are bucketed in it and relative filters ("today") are resolved against it. Also accepted, at lower priority, nested as `sessionSettings.settings.timezone` — matching the `settings.timezone` key path the [Generate Session API](/reference/embed-apis/generate-session#session-settings) uses for its own, separately-scoped iframe time-zone setting; setting one does not affect the other. Falls back to the deployment's default time zone (`CUBEJS_DEFAULT_TIMEZONE`) if neither is set or valid. |
...
- **`sessionSettings`** (object, required for API key authentication; ignored
  ...
  - **`timezone`** (string): IANA time zone the agent runs this chat turn's queries in. Also accepted, at lower priority, nested as `sessionSettings.settings.timezone` — matching the `settings.timezone` key path the Generate Session API uses for its own, separately-scoped iframe time-zone setting.
