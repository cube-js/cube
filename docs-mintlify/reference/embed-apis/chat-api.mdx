...
| `sessionSettings.timezone` | string | No | IANA time zone the agent runs *this chat turn's* queries in (e.g. `America/New_York`). Also accepted, at lower priority, nested as `sessionSettings.settings.timezone` — see [Request Body](#request-body) below. Falls back to [`CUBEJS_DEFAULT_TIMEZONE`](/reference/configuration/environment-variables#cubejs_default_timezone) if neither is set or valid. |
...
- **`sessionSettings`** (object, required for API key authentication; ignored
  ...
  - **`timezone`** (string): IANA time zone the agent runs this chat turn's queries in. Also accepted, at lower priority, nested as `sessionSettings.settings.timezone` — matching the `settings.timezone` key path the [Generate Session API](/reference/embed-apis/generate-session#session-settings) uses for its own, separately-scoped iframe time-zone setting.
...
### Request Body {#abort-request-body}
