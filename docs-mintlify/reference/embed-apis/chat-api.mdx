...
The stream always ends with one additional frame, `id: "__usage__"`, reporting the run's models and token usage (see the Usage Message example below). Treat it like the `__cutoff__` and `__state__` frames: filter it out of anything rendered as a chat message.
