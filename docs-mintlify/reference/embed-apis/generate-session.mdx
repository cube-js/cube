...
server side, rather than waiting out the token's own 24-hour expiry. Removing the
embedded iframe on logout is still required — revoking a session doesn't tear it down.
...
`Api-Key` authentication as Generate Session.
