...
every embed opened with this session. Each key is **tri-state**: omit it to
inherit what the layer below says — the account-wide setting under **Embed →
Settings** for the AI keys, the built-in default for the rest — or set
`true`/`false` to pin the behavior for this session, taking precedence over the
