...
| `branchName`               | string   | No          | Deployment branch this session queries. Omit it (or pass the deployment's production branch) to query production, which stays the default. Any other branch — one with a staging environment enabled, a shared branch, or a personal dev branch — points every Cube query the embedded dashboards, Creator Mode workspace, and chat make at that branch's data model instead. The branch must be served when the session is generated: a name that isn't a branch of the deployment returns `404`, and a branch nothing is currently running returns `400` — enable the branch's staging environment so it's always reachable. Content itself isn't branch-scoped: the same dashboards and workbooks render against whichever branch is selected. |
