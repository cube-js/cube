...
`settings` is an object of per-session overrides for embed behavior. Each key is
**tri-state**: omit it to inherit the account-wide setting, or set `true`/`false`
to force the behavior for every embed viewed with this session, taking precedence
over the account-wide setting.
...
|----------|------|----------|-------|
| `allowAi` | boolean | No | Master switch over every AI surface in the embed: the dashboard agent, the workbook chat, the AI summary widgets, and the standalone [embedded chat][ref-analytics-chat]. Omit to inherit the account-wide **Embed → Settings** default (allowed by default); `false` disables AI even if it's enabled account-wide. Unlike the other keys on this table, `allowAi` is enforced server-side — a session with `allowAi: false` cannot obtain AI credentials at all, so it can't spend AI tokens — and it outranks the `show*` keys below, which only hide UI entry points. |
...
| `showWorkbookChat` | boolean | No | Show or hide AI entry points in a [Creator Mode][ref-creator-mode] workbook for this session: the chat side panel and its toggle, the launchpad's "Ask Cube Agent" button, and "Fix in chat" on a failed report. Omit to inherit the account-wide **Embed → Settings** default (shown by default). Hides entry points only — use `allowAi` to withhold AI from the session entirely. |
| `showAiWidgets` | boolean | No | Show or hide AI summary widget authoring for this session: the "AI widget" entry in the dashboard editor's Add widgets menu, and the Generate/Regenerate controls on an existing widget. Omit to inherit the account-wide **Embed → Settings** default (enabled by default). An already-generated summary keeps rendering either way — this only stops it being re-spent. |
| `allowChatWorkspaceAuthoring` | boolean | No | Restricts this session's AI chat from creating or modifying persistent Cube Workspace content. Set to `false` for headless chat integrations that render answers in their own UI: ad-hoc analysis and inline tables/charts remain available, but the agent can't save or update explorations/reports, or create or publish workbooks and dashboards. Omit or `true` preserves the session user's role-derived authoring capabilities; it never grants access the user doesn't already have. |
