...
own. Its `sessionSettings.allowChatWorkspaceAuthoring` and
`sessionSettings.timezone` fields mean the same thing as `settings.allowChatWorkspaceAuthoring`
and `settings.timezone` here, just nested under a differently-named object.
</Note>
