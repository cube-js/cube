...
description: HTTP reference for minting short-lived embed sessions that authorize external viewers under signed embedding without sharing API keys, and how to revoke one.
---
...
It requires an admin key, but unlike Generate Session, that key's scope is not
checked: the request body carries only a `sessionId`, so a key scoped to one
deployment can revoke a session belonging to any deployment in the account.
