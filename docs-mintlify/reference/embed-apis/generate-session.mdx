...
session id carries no deployment once redeemed, so a key scoped to one deployment can
still revoke a session belonging to any deployment in your account.
