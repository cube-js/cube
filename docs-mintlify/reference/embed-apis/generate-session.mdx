...
It also accepts a `timezone`, both as its own `sessionSettings.timezone` field
and nested as `sessionSettings.settings.timezone` matching this object's shape.
That value only sets the zone the agent runs *that chat turn's* queries in — it
is resolved independently of, and does not affect, the `timezone` documented
above, which governs iframe rendering.
</Note>
