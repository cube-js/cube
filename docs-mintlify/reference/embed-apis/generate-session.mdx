...
The token it's exchanged for is usable for about 23 hours — see [signed
embedding][ref-signed-embedding] — unless you revoke it sooner; see [Revoke a
...
embedded iframe on logout — revoking a session doesn't tear it down on its own. Unlike
Generate Session, this accepts any admin key in the account regardless of its
deployment scope.

See [Revoke an Embed Session][ref-revoke-session] for the full request, response, and
