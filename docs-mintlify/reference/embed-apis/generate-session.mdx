...
to an admin user — but unlike Generate Session, an API key's scope is not checked:
the request body carries only a `sessionId`, so a key scoped to one deployment can
revoke a session belonging to any deployment in the account.
