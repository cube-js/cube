...
<Note>
The [Chat API](/reference/embed-apis/chat-api) session exchange also accepts this
`settings` object directly, plus top-level `allowChatWorkspaceAuthoring` and
`timezone` fields equivalent to the matching `settings` keys — when both a
top-level field and its `settings` equivalent are sent, the top-level one wins.
</Note>
