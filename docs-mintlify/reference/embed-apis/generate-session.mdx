...
The deployment-scope restriction is specific to Generate Session. [Revoke a session](#revoke-a-session) needs an admin key but no deployment scope — its request body carries only a `sessionId`, so there is no `deploymentId` to check against the key's scope.
