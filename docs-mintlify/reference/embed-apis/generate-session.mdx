...
The deployment-scope restriction is specific to Generate Session. [Revoke a session](#revoke-a-session) requires an admin key, but its scope is not checked: the request body carries only a `sessionId`, so a key scoped to one deployment can revoke a session belonging to any deployment in the account.
