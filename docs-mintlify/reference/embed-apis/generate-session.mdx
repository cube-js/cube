...
Because these values are signed into the session token, a viewer cannot change
them; use them for anything that differs per customer. See [Show and hide
...
| `timezone` | string | No | IANA time zone the agent runs its queries in for every embed opened with this session (e.g. `America/New_York`). Sits between the per-iframe `?timezone=` parameter and the account-wide default, and stays subject to the account's user-time-zone policy — it is not a way around it. A bare UTC offset or an unknown name falls through rather than failing the session. See [Time zones](/embedding/iframe/time-zones). |
