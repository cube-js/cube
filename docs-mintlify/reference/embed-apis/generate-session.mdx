...
Both restrictions are specific to Generate Session. [Revoke a session](#revoke-a-session) takes any admin key in the account — personal keys included, whatever the key's deployment scope — because it ends a session rather than minting one under somebody else's identity.
