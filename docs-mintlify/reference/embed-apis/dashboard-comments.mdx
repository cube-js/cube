...
Deleting is a soft delete, and the two cases differ. **A deleted reply is dropped from its thread.** **A deleted root is still listed** while a live reply survives under it, returned with an empty `body` and `removedAt` set — otherwise removing it would discard other people's replies along with it. A root with no surviving replies leaves the list entirely.
