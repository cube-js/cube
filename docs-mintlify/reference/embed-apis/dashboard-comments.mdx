...
const res = await fetch(`${BASE}/dashboard/${PUBLIC_ID}/comments`, {
  method: "POST",
  headers,
  body: JSON.stringify({ body: "Revenue looks off for the EU region." }),
});
// Check the status: an unchecked error body yields `root.id === undefined`,
// and step 2 would then post its reply as a second thread instead of failing
