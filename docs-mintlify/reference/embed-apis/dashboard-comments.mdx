...
**`first` defaults to 100 and is capped at 200.** Omit it and you get up to 100 threads. A larger value is clamped rather than rejected, so `first=1000` returns 200 threads with `hasNextPage: true` — page through the rest rather than asking for them at once. A `first` below 1 is a `400`. The same default and ceiling apply to [mentionable users](#mentionable-users).
