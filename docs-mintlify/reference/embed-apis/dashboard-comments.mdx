...
That is what makes the endpoints usable for your own comment UI. It also means **there is no server-side switch that stops comments being written**: read access to the dashboard is the whole permission, so any session you mint for a viewer carries the ability to comment with it. If comments must not be accepted at all, do not embed that dashboard.
...
Every endpoint requires a **signed embed session scoped to an embed tenant**. Mint one with [Generate Session][ref-generate-session] using `externalId`. `embedTenantName` is optional: omit it and the session scopes to the Cube tenant's own name, which reaches these endpoints; pass it to scope the conversation to a specific embed tenant. That name decides which conversation the session sees — see [A conversation lives inside a single embed tenant](#a-conversation-lives-inside-a-single-embed-tenant).
...
Threads are stored per embed tenant, and `GET /dashboard/{publicId}/comments` returns only the caller's own tenant's threads. Embed one published dashboard across several customers and each gets a separate, private conversation on it: a user in `acme` never sees a thread written in `globex`, and neither can reply to the other.

Expect an empty list on a dashboard your own tenant has not commented on yet, however busy it looks to someone else. The embed tenant comes from the session — see [Authentication](#authentication).
