...
Every endpoint requires a **signed embed session scoped to an embed tenant**. Mint one with [Generate Session][ref-generate-session] using `externalId`. The session is scoped to `embedTenantName`, or to the account's own tenant when you omit it, so either form reaches these endpoints.

A credential that names no embed tenant is rejected with `403 Forbidden`: a [Cube Cloud API key][ref-api-keys], a console session, or a session minted with `internalId`, which resolves its user against Cube Cloud's own accounts and so carries no embed tenant at all.
...
| `403` | The credential names no embed tenant; the caller cannot read the dashboard; or the comment belongs to another embed user. |
