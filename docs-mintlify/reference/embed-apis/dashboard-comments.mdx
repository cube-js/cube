...
Every endpoint requires a **signed embed session scoped to an embed tenant**. Mint one with [Generate Session][ref-generate-session] using `externalId`. `embedTenantName` is optional: omit it and the session scopes to the Cube tenant's own name, which reaches these endpoints; pass it to scope the conversation to a specific embed tenant. That name decides which conversation the session sees — see [A conversation lives inside a single embed tenant](#who-may-edit-delete-and-resolve).
...
Reading is wider than writing — but only inside one embed tenant. Every embed user of the caller's own embed tenant sees the whole conversation, including threads they did not start.

**A conversation lives inside a single embed tenant.** Threads are stored per embed tenant and `GET /dashboard/{publicId}/comments` returns only the caller's own tenant's threads. Embed one published dashboard across several customers and each gets a separate, private conversation on it: a user in `acme` never sees a thread written in `globex`, and neither can reply to the other. Expect an empty list on a dashboard your own tenant has not commented on yet, however busy it looks to someone else.
...
**The directory is scoped to the caller's own embed tenant, and to embed users only.** An embed user can mention their colleagues in the same embed tenant. They cannot mention a Cube user of the host account, and they never see another embed tenant's users. Pass `search` to filter by username or email; results are ordered by username. Paginate as for [threads](#thread-list), `hasNextPage` included — a `search` that matches nobody returns an empty page whose cursors are `null` or absent.
