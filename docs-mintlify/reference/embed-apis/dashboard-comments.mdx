...
Every endpoint requires a **signed embed session scoped to an embed tenant**. Mint one with [Generate Session][ref-generate-session] using `externalId`, and **pass `embedTenantName`** — commenting requires it. The field is optional on Generate Session at large, but a session minted without it carries no embed tenant and is rejected here with `403 Forbidden`.

Any other credential that names no embed tenant is refused the same way, including a [Cube Cloud API key][ref-api-keys] and a console session.
