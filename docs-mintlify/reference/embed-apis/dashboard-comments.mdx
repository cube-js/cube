...
| `removedAt` | string \| null | Always `null` on a comment the list returns — a deleted comment is dropped rather than returned as a tombstone. |
...
**The directory is scoped to the caller's own embed tenant, and to embed users only.** An embed user can mention their colleagues in the same embed tenant. They cannot mention a Cube user of the host account, and they never see another embed tenant's users. Pass `search` to filter by username or email; results are ordered by username. Pagination works as it does for threads: pass `first` and `after` with the previous page's `endCursor`.
