...
A view defines its own surface, so included members don't inherit the
[`public`][ref-dim-public] flag from the underlying cube: a member that is
`public: false` on the cube becomes queryable through the view. Use `excludes`
to keep such members out of the view.
...
[ref-dim-public]: /reference/data-modeling/dimensions#public
