...
`if` expressions support boolean operators in both YAML and JavaScript data
models. In YAML, use the Python operators `not`, `and`, and `or`. In
JavaScript, use the standard `!`, `&&`, and `||` operators. This allows
expressing more complex conditions, for example checking group membership with
null safety:
...
access_policy: [
  {
    group: `user_group`,
    conditions: [
      { if: securityContext.auth.groups && securityContext.auth.groups.includes(`region_group`) }
    ],
    // ...
  },
  {
...
Within `row_level`, you can use `allow_all: true` instead of `filters` to grant
unrestricted row-level access when a policy matches. `allow_all` and `filters`
are mutually exclusive — exactly one of them must be specified.

This is useful when layering multiple policies, for example, to grant one group
unrestricted access while another group receives a row filter:
...
When multiple matching policies apply to a query, an `allow_all` policy on a
cube grants unrestricted access for that cube, overriding row filters from
other matching policies. See [row-level access][ref-dap-rls] for details on
