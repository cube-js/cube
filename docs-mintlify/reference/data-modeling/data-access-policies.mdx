...
In JavaScript data models, you can also set `row_level` to an expression that
evaluates against the [security context][ref-sec-ctx] and returns a different
`row_level` configuration depending on its value. This lets you switch the
entire row-level configuration (e.g., between filtering by an attribute and
allowing all rows) based on user properties such as group membership.

In the following example, users in the `region_group` group are restricted to
rows matching their `allowedProductIds` user attribute, while all other users
in `user_group` are allowed access to all rows:
...
behavior, define multiple policies for the same group and gate them with
[`conditions`](#conditions), as shown in the YAML example above.
