...
Each `if` expression is evaluated as a boolean. In YAML data models, expressions
are written inside `{ ... }` and support standard boolean operators — `and`,
`or`, and `not` — along with parentheses for grouping, comparisons (`==`, `!=`,
`<`, `>`, `<=`, `>=`), and method calls on the [security
context][ref-sec-ctx] (e.g., `.includes(...)`). In JavaScript data models, `if`
takes a regular JavaScript expression.

```yaml
conditions:
  # AND across operands within a single `if`
  - if: "{ userAttributes.is_full_time_employee and userAttributes.tenure_years >= 2 }"

  # OR
  - if: "{ userAttributes.is_admin or userAttributes.is_owner }"

  # NOT and grouping
  - if: "{ not (security_context.groups and security_context.groups.includes('contractors')) }"
```

Multiple entries in the `conditions` list are combined with logical AND — the
policy applies only when every `if` evaluates to `true`. To express OR between
policies, define multiple policies for the same group with different
`conditions`.
...
access][ref-dap-rls] for a policy. It accepts the following sub-parameters:

- `filters` — a list of filters that should apply to result set rows.
- `allow_all` — when set to `true`, grants the policy access to all rows
without applying any row-level filters.
...
Use `allow_all: true` to grant a policy access to all rows without applying any
row-level filters. This is useful when you want to express “no row-level
restriction” explicitly, for example in combination with [`conditions`](#conditions)
or with conditional `row_level` expressions (see below).
...
#### Switching `row_level` based on the security context

In JavaScript data models, `row_level` may be set to an expression that
evaluates against the [security context][ref-sec-ctx] and returns a different
`row_level` configuration depending on its value, including `{ allow_all: true }`.
In YAML data models, `row_level` is a static structure — to achieve the same
result, define multiple policies for the same group and gate them with
[`conditions`](#conditions).

See [Conditional row-level access by group membership][ref-recipe-cond-rls] for
a worked example.
