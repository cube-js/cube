...
member granted by `member_level` is never masked, so a member you intend to
mask with [`member_masking`](#member_masking) must be left out of
`member_level` — use `excludes` for it.
