...
`if` expressions support boolean logic so you can compose checks against
the [security context][ref-sec-ctx] and user attributes:

- In YAML, expressions inside `{ ... }` are parsed as Python and support the
`and`, `or`, and `not` operators, parentheses for grouping, member access, and
method calls such as `.includes(...)`. Comparison operators (`==`, `>=`, `in`)
and inline `if`/`else` conditionals are **not** supported — use boolean user
attributes or method calls instead.
- In JavaScript, expressions use native operators: `&&`, `||`, `!`, and any
...
Use `allow_all: true` to make the row-level intent of a policy explicit without
listing any filters. It grants the policy access to all rows and is equivalent
to omitting `row_level` from the policy — useful when you want the policy to
clearly state that no row-level restriction applies, or as an admin override
that combines with more restrictive policies via [_OR_ semantics][ref-dap-rls].

A `row_level` block must contain either `filters` or `allow_all` — the two are
mutually exclusive. `allow_all` only accepts `true`; `allow_all: false` is not a
valid value. To deny a group all rows, don't grant it a matching policy: access
policies are deny-by-default, so any member not granted by a matching policy is
inaccessible.
...
on the [security context][ref-sec-ctx] or user attributes. Attach
[`conditions`](#conditions) to a policy so its `row_level` is skipped when the
conditions don't match, and define separate policies for the different filters
you want to apply. In JavaScript, you can additionally compute a filter's
`member` or `values` from `securityContext` at evaluation time (for example,
with a ternary), which lets a single policy vary its filter per request. When a
policy's `row_level` is skipped, other matching policies still apply; if no
policy matches, access is denied by default. See the [conditional row-level
access recipe][ref-recipe-conditional-row-level] for worked examples.
