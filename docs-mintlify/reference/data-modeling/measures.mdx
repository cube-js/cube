...
Set `type` to `to_date` to accumulate from the start of the period that contains each
data point, instead of over a window of fixed size. A `to_date` window also requires
`granularity`, which names that period: a default granularity such as `week`, `month`,
`quarter`, or `year`, or a [custom granularity][ref-custom-granularities]. Omitting it
fails validation.

<Warning>

A custom granularity here is only available in Tesseract, the [next-generation data
modeling engine][link-tesseract]. In versions before v1.7.0, it was not enabled by
default. Without Tesseract, the model still compiles and the query fails.
...
A window of type `to_date` takes none of the `offset`, `trailing`, or `leading`
parameters documented below; those apply to fixed-size windows, which are selected by
`type: fixed` — the default when `type` is omitted. To apply the same
period-to-date logic across many measures without writing each one by hand, see
[reusing period-to-date logic][ref-recipe-period-to-date].

The `year_to_date`, `quarter_to_date`, and `month_to_date` types are deprecated
equivalents of `to_date` with the matching `granularity`.

On the legacy planner, a `to_date` measure cannot be served from a pre-aggregation: only
`fixed` rolling windows are supported there, and a query that tries to match one fails
with `Only fixed rolling windows are supported by Cube Store`. Tesseract plans
pre-aggregations itself and does not apply this restriction.
...
[ref-recipe-period-to-date]: /recipes/data-modeling/reusable-period-to-date
