...
The `grain` parameter requires the [Tesseract SQL planner][ref-tesseract-env]
(`CUBEJS_TESSERACT_SQL_PLANNER=true`) and is only valid on **measures** — it is rejected on
dimensions. For a step-by-step explanation with worked result tables, see
[Controlling grain](/docs/data-modeling/measures#controlling-grain) in the measures guide, and
the [semi-additive measures recipe](/recipes/data-modeling/semi-additive-measures) for a worked
