...
data point, instead of over a window of fixed size. The `granularity` parameter names
that period: a default granularity such as `week`, `month`, `quarter`, or `year`, or a
[custom granularity][ref-custom-granularities].
...
A window of type `to_date` takes none of `trailing`, `leading`, or `offset`. To apply
the same period-to-date logic across many measures without writing each one by hand,
see [reusing period-to-date logic][ref-recipe-period-to-date].
