...
description: Pre-aggregations are materialized summaries of data that accelerate query performance by pre-computing results.
---
...
partitions — and each one is roughly 50 MB in size and contains up to ~1M
rows. The exact per-tier limits are still being finalized, but expect on the
order of 5, 10, and 20 physical partitions for S, M, and L tiers
respectively.
...
[ref-caching-preaggs-target]: /docs/pre-aggregations/getting-started-pre-aggregations#ensuring-pre-aggregations-are-targeted-by-queries
