...
widen the pushed-down range to cover the periods the shifted stages read — here
a fiscal prior year, bracketed at 371 and 364 days. The bracketing is the
load-bearing part; the date arithmetic below is BigQuery syntax, so adjust it
for your data source. Note that the values arrive as bare query parameters, so
they are cast before use — the same as the shard example above:
