...
<Update label="2026-08-27" description="v0.4.0" tags={["Added","Changed","Removed"]}>
  ...
  #### Regions and PrivateLinks — infrastructure provisioning

  - Full region lifecycle, gated to Cube super admins: `POST /api/v1/regions` (`RegionsPublicController.createRegion`), `GET`/`PUT`/`DELETE /api/v1/regions/{regionId}` (`getRegion`/`updateRegion`/`destroyRegion`), `POST /api/v1/regions/{regionId}/apply` (`applyRegion`), `GET /api/v1/regions/{regionId}/provisioning-status` (`getRegionProvisioningStatus`), and `GET /api/v1/regions/cloud-provider-catalog` (`listCloudProviderCatalog`). Creating or updating a region only records intent — nothing is built in the cloud until `apply` runs. New schemas: `ProvisionedRegionResponse`, `RegionCreateRequest`, `RegionUpdateRequest`, `RegionProvisioningRunResponse`, `RegionProvisioningStatusResponse`, `RegionCloudProviderCatalogListResponse`, `RegionCloudProviderCatalogResponse`, `RegionCloudProviderRegionResponse`.
  - PrivateLink management under a region: `GET`/`POST /api/v1/regions/{regionId}/private-links` (`RegionPrivateLinksPublicController.listPrivateLinks`/`createPrivateLink`) and `GET`/`PUT`/`DELETE /api/v1/regions/{regionId}/private-links/{privateLinkId}` (`getPrivateLink`/`updatePrivateLink`/`removePrivateLink`). Like region create/update, writes here only take effect in AWS on the next `apply`. New schemas: `RegionPrivateLinkResponse`, `RegionPrivateLinkCreateRequest`, `RegionPrivateLinkUpdateRequest`, `RegionPrivateLinkRemoveResponse`, `RegionPrivateLinksListResponse`.
  ...
  - `PUT .../branches/staging-environment` (`setBranchStagingEnvironment`) — enable or disable a branch's always-on staging environment (`<deploymentUrl>/dev-mode/{branchName}/cubejs-api/v1`). New schemas: `SetBranchStagingEnvironmentRequest`, `SetBranchStagingEnvironmentResponse`.
  ...
  - Report placements are now individually addressable and trackable: `ConnectReportToWorkbookInput`, `CreateReportInput` and `UpdateReportInput` each gained `placementId`, `sheetId`, `sheetName`, `anchorCell`, `workbookName`, and `host` (new `ConnectReportToWorkbookInputHost` / `CreateReportInputHost` / `UpdateReportInputHost` enums, `"GOOGLE_SHEETS" | "EXCEL"`). `ReportPlacement` gained the matching `id`, `host`, `sheetId`, `sheetName`, `anchorCell`, `workbookName`, plus `placedAt`/`placedByUserId` and `refreshedAt`/`refreshedByUserId` (new `ReportPlacementHost` enum). `ReportSnapshot` gained `isSourceReport` and `pythonCode`.
  ...
  - `name` / `userCount` added to `InheritedGroupPolicyDto`.
