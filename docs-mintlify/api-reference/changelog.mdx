...
<Update label="2026-09-22" description="v0.13.0" tags={["Added","Changed","Removed"]}>
  ### Added
...
<Update label="2026-09-20" description="v0.12.0" tags={["Added","Deprecated"]}>
  ### Added
  ...
  - `TenantSettings` / `TenantSettingsInput` gained `aiTokensAutoPurchaseEnabled`, `aiTokensAutoPurchaseThreshold` and `aiTokensAutoPurchaseAmount` (CUB-4690) — automatic AI-token top-up. `Threshold`/`Amount` are dollar amounts and are only meaningful while `Enabled`; they are retained when it is switched off so re-enabling restores what the customer had.
