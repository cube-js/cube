...
<Update label="2026-08-27" description="v0.4.0" tags={["Added","Changed","Removed"]}>
  ...
  These operations are Cube-staff-only (a tenant admin cannot call them) and, for that reason, are excluded from the public API reference at docs.cube.dev — they're documented here because the SDK still types and ships them.
