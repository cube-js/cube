...
<Update label="2026-08-27" description="v0.4.0" tags={["Added","Changed","Removed"]}>
  ...
  - `first`/`after` → `items` + `pageInfo` cursor pagination added to: deployment env variables (`GET .../env-vars`), deployment environments and environment tokens, GitHub branches/installations/repositories (`GitHubPublicController.*`), user attributes, groups, users, `GET /api/v1/regions/`, and report folders. The legacy `data`/`count`/`pagination` fields are kept on each response, marked `@deprecated`.
