...
`CUBEJS_DEV_MODE` defaults to `false` everywhere — under the official Docker images,
under `cubejs server`, and when you embed `@cubejs-backend/server-core` directly — so
leaving the flag unset is enough. `NODE_ENV` no longer has any say in it.
