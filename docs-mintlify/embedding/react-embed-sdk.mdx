...
The React Embed SDK uses the same session-based authentication as [signed embedding](/embedding/signed-embedding). Generate a session using the [Generate Session API](/api-reference/embed-apis/generate-session) and pass the `sessionId` to the provider.
