...
To isolate those resources per tenant as well, define
[`context_to_orchestrator_id`][ref-config-ctx-to-orch-id] and
[`pre_aggregations_schema`][ref-config-preagg-schema], as shown in the sections
below.
