...
| **Language** | Default UI language for embedded surfaces, below `settings.locale` and `?locale=`. | English |
| **Time Zone** | Time zone agent queries run in, below `settings.timezone`, `?timezone=`, and a host message. | Account-wide zone |
