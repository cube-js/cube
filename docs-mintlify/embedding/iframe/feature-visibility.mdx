...
the bar for you. All five govern the same header in Creator Mode's dashboard view
too, which renders the identical back button, title, Edit, and Duplicate controls —
a different header from the workbook-editing one `showWorkbookBackButton` controls,
which has no master switch of its own.
