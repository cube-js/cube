...
| --- | --- | --- | --- |
| [Account settings](#account-settings) | **Embed → Settings** in the Cube console | Every embed in the account | No |
| [Session settings](#session-settings) | The `settings` object of the [Generate Session API][ref-generate-session] | Every embed opened with that session — i.e. one viewer | No — they are signed into the session token |
...
- **Session settings are per-viewer and tamper-proof.** Your backend decides
  them when it mints the session, and they are signed into the session token, so
  a viewer cannot grant themselves something you withheld. Everything that
...
These four inherit an account-wide default from **Embed → Settings**, so a
session value is an override of what the account already says.

| Setting | Governs | Default |
| --- | --- | --- |
| `allowAi` | **Master switch** over every AI surface in the embed. Enforced on the server: a session with `allowAi: false` cannot obtain AI Chat credentials at all, so it cannot spend AI tokens. Outranks the three keys below — `false` here disables a surface whose own key is `true`. | Account setting, then allowed |
...
every iframe that session opens uses them. The per-iframe `?locale=` /
`?timezone=` parameters still win where a host sets one, and the account-wide
default applies where neither does.
...
| `timezone` | The IANA zone the agent runs its queries in (`America/New_York`). Still subject to the account's user-time-zone policy — a session setting is not a way around it — and a bare UTC offset or unknown name falls through. See [Time zones](/embedding/iframe/time-zones). |
...
| `allowChatWorkspaceAuthoring` | Whether AI Chat authenticated with this session may create or modify persistent workspace content. `false` keeps ad-hoc analysis and inline tables/charts, but the agent cannot save or update explorations and reports, create or modify workbooks, create or publish dashboards, or point users at those surfaces. Intended for headless chat integrations that render answers in their own UI. `true` never grants authoring the user's role does not already allow. |
...
| `showNewChat=false` | Hides the **New chat** button in the standalone chat embed. | Analytics Chat |
| `showChatInput=false` | Hides the chat's message input, for a read-only transcript. | Analytics Chat |
| `showChatHistory=false` | Hides the **Chat History** button. | Analytics Chat |
...
Every parameter falls back to what your account would have done anyway, so a typo
never buys something you did not ask for. Which direction that is depends on what
the parameter does:
...
- **A parameter that hides** (every `show…`) requires the exact literal string
  ...
  the control visible.
- **A parameter that selects a value** (`locale`, `timezone`) drops to the
  account default when the value is one Cube cannot use.
...
| **Language** | Default UI language for embedded surfaces, overridden per embed by `?locale=`. | English |
| **Time Zone** | Time zone agent queries run in, overridden per embed by `?timezone=` or a host message. | Tenant default |
...
`false` hides. Their original spellings — `hideNewChat=1`, `hideChatInput=1`,
`hideHistory=1`, each accepting `1` or `true` — still work so that existing embed
URLs keep working, and the `show…` form wins when both are present.
...
- **Three of them are enforced, not merely hidden.** `allowAi: false` stops the
  session obtaining AI credentials at all. `allowChatWorkspaceAuthoring: false`
  changes the tools and instructions the agent is given, on the server.
  `showMemberNames: false` and `showGroupByCube: false` additionally pin the data
  pane's state, so a user who had already switched cannot stay switched.
...
- **Export is client-side.** A widget's CSV is serialized in the browser from
  ...
  is offered, not whether the viewer's browser holds the rows. The tenant-wide
  **restrict data downloads** governance lock outranks it.
