...
Pass `settings` to the [Generate Session API][ref-generate-session]. Every key is
**tri-state**: omit it to inherit the layer below (the account setting where one
exists, otherwise the built-in default), or set `true` / `false` to pin the
behavior for every embed opened with that session.
...
| `locale=es-MX` | Sets the embed's UI language. See [Localization](/embedding/iframe/localization). | All |
| `timezone=America/New_York` | Sets the time zone agent queries run in. See [Time zones](/embedding/iframe/time-zones). | All |
...
URL parameters of its own, which predate the convention above: they are phrased
as `hide…` rather than `show…=false`, and they accept either `1` or `true`.
