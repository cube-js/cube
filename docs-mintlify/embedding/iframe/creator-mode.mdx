...
To enable it, pass `creatorMode: true` to the [Generate Session API][ref-generate-session]. Add `branchName` alongside it to have everything authored in the session query a staging or development branch's data model rather than production.
...
Creator Mode ships the full authoring experience, and most of it can be taken
back out for a given customer or a given placement:

- **Per session**, via `settings` on the [Generate Session API][ref-generate-session]:
  `showWorkbookShare` (the Share button and row actions), `showDashboardSettings`
  (the dashboard gear and its sidebar), `showMemberNames` and `showGroupByCube`
  (the data pane's more-actions items), `showWorkbookChat` and `showAiWidgets`
  (AI entry points), `allowChatWorkspaceAuthoring` (whether the agent may author
  workbooks, explorations, and dashboards at all), and `allowAi` (AI entirely).
- **Per iframe**, via URL parameters: `showWorkbookBackButton=false` on the
  workbook header, and the dashboard header controls — which render the same in
  Creator Mode as in a standalone dashboard embed.
- **Account-wide**, under **Embed → Settings**: the workspace header title and
  whether it shows at all, and whether embed users may see Semantic SQL and
  Generated SQL.

Export works the other way: it's hidden by default, and the iframe URL parameter
`allowExport=true` grants it, adding **Download as CSV** in the report builder
alongside the dashboard download actions it already controls.

[Show and hide features](/embedding/iframe/feature-visibility) documents all of
them, and the rule for which layer to use.

The comment panel on published dashboards is account-wide too, and off by
default. The setting hides the panel rather than closing the comment API.
[Dashboard Comments](/reference/embed-apis/dashboard-comments) documents it.
That page does not cover commenting inside a Creator Mode workbook:
that is the Cube app's own surface, with its own separate setting.

## AI

Creator Mode workbooks show the same AI entry points as the full Cube app — the chat side panel, the launchpad's "Ask Cube Agent" button, "Fix in chat" on a failed report, and AI summary widgets — unless you turn them off.

Three account-wide switches under **Embed → Settings** control this, each overridable per session via [Generate Session][ref-generate-session]:
