...
- **Per iframe**, via URL parameters: `showWorkbookBackButton=false` on the
  workbook header, the dashboard header controls — which render the same in
  Creator Mode as in a standalone dashboard embed — and `allowExport=true`,
  which also grants **Download as CSV** in the report builder.
