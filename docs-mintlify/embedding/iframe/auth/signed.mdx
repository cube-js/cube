...
- A session (or the token it was exchanged for) can also be ended server-side from your application's logout handler, using an admin-scoped key — see [Revoke an embed session](/api-reference/embed/revoke-an-embed-session)
...
To point an embed at a staging or development branch instead of production — to preview data model changes before they ship, for example — pass that branch's name as `branchName`. Every query the session makes, from dashboards, Creator Mode, and chat alike, runs against that branch's data model; the same saved dashboards and workbooks are rendered, since content isn't branch-scoped. The iframe URL is unchanged: the branch is fixed when the session is generated, not by a URL parameter.
