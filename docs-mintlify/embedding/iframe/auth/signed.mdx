...
- A session (or the token it was exchanged for) can also be ended server-side from your application's logout handler — see [Revoke an embed session](/api-reference/embed/revoke-an-embed-session)
