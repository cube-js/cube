...
while blocking another. (`showDashboardExportMenu=false`, below, can still
hide the dashboard-level menu while keeping per-widget export.) Only the
exact literal string `true` opts in: `allowExport=1`,
