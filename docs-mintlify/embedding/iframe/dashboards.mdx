...
Viewers of an embedded dashboard can discuss it in comment threads. Threads are
kept separate per embed tenant, so mint sessions with a distinct
`embedTenantName` per customer — sessions that omit it share one tenant, and
therefore one conversation. The comment panel appears once the account turns
on **Allow comments on embedded dashboards** under **Embed → Settings**, which
is off by default. That setting hides the panel rather than closing the API:
the comment endpoints answer either way.
[Dashboard Comments](/reference/embed-apis/dashboard-comments) documents the
setting, the API behind it, and who may edit, delete, and resolve.
