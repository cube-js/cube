...
`f_`, `tg_`, and `ms_` are read on both regular and published (embedded)
dashboards, and are applied only if a matching control for that member already
...
The reverse direction is published-only: when a viewer changes a control
there — or switches a tab — the new value is written back into the dashboard's
own URL, so the state a link carries and the state a viewer reaches by clicking
are the same format. See [Controls → Sharing the current
