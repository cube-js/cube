...
The whole dashboard can also be downloaded as a single PNG or PDF, from a
download menu in the dashboard header (Creator Mode) or floating over the
dashboard (published-dashboard embeds). `allowExport=true` grants that menu too —
it is the same opt-in, not a second one. To keep the per-widget downloads but
drop the dashboard-level menu (for example when your own chrome already offers
it), add `showDashboardExportMenu=false`.
...
actions appear. The tenant-wide **restrict data downloads** setting outranks it.
...
| `showDashboardExportMenu=false` | The whole-dashboard download menu (see [Allow chart export](#allow-csv-export)) |
...
carries the workbook name, tabs, Publish changes, and Share — is a separate
