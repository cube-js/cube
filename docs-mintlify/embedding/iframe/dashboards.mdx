...
The semantic view and member must match the internal names (not display
titles) configured on the widget — as must the member on the right-hand side
of `ms_`, which is a measure rather than a dimension when the field
switcher's **Field Type** is **Measure**. For
...
member outside the field switcher's alternatives.
