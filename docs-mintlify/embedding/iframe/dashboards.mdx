...
These parameters apply to the dashboard header only. The [Creator
Mode](/embedding/iframe/creator-mode) workbook header — which carries the
workbook name, tabs, Publish changes, and Share — has its own, separate
back-button switch:
...
`showWorkbookBackButton` is independent of `showDashboardBackButton` — the two
headers are separate surfaces, so a host that wants neither back button sets
both. The rest of the workbook header (name, tabs, Publish changes, Share) has
no equivalent switch, since those actions are part of the authoring flow. The
workbook actions menu still offers **View all**, so hiding the back button
never removes a way back to the workspace.
