...
| `showDashboardExportMenu=false` | The dashboard-level ⋮ download menu |
...
`showDashboardHeader=false` overrides the other five: the whole bar
...
permission. `showDashboardExportMenu` likewise only hides — the menu has to be
granted by [`allowExport`](#allow-csv-export) first. On the published-dashboard
embed there is no header bar and the menu floats over the dashboard, so
`showDashboardHeader` doesn't reach it; `allowExport` and
`showDashboardExportMenu` are the only switches that govern it there.
