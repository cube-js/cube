...
display titles) of the semantic view and member configured on the widget — as
must the member on the right-hand side of `ms_`, which is a measure rather than
a dimension when the field switcher's type is **Measure**. For
...
for time dimensions that expose them. A field switcher's parameter names
the member it *replaces* on the left and the member to show in its place on the
right; the right-hand side is a bare member name, without the view prefix, and
must be one of the switcher's configured options.
