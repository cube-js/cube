...
| Field | Type | Description |
| --- | --- | --- |
| `format` | `"csv" \| "xlsx" \| "png" \| "pdf"` | The file format produced. The whole-dashboard **Print version** reports `"pdf"`. |
...
<Note>
  ...
  whole-dashboard (PNG, PDF, print version) — only appear when the embed URL
  includes `allowExport=true` (see [Dashboards → Allow chart and dashboard
