...
description: "To see a complete list of environment variables for a specific data source, please check the relevant page on [Connecting to Data Sources][ref-config-db]."
---
...
option](/docs/configuration/reference/config#check_auth).
...
option](/docs/configuration/reference/config#cache_and_queue_driver).
...
option](/docs/configuration/reference/config#orchestrator_options).

If you'd like to adjust the concurrency for the refresh worker, use
[`CUBEJS_REFRESH_WORKER_CONCURRENCY`](/docs/configuration/reference/environment-variables#cubejs_refresh_worker_concurrency).
...
| Possible Values | Default in Development       | Default in Production        |
| --------------- | ---------------------------- | ---------------------------- |
| A valid number  | Same as [`CUBEJS_CONCURRENCY`](/docs/configuration/reference/environment-variables#cubejs_concurrency) | Same as [`CUBEJS_CONCURRENCY`](/docs/configuration/reference/environment-variables#cubejs_concurrency) |
...
configured via the [`CUBEJS_REFRESH_WORKER_CONCURRENCY`](/docs/configuration/reference/environment-variables#cubejs_refresh_worker_concurrency) environment variable, and
...
Previously, this environment variable was named [`CUBEJS_SCHEDULED_REFRESH_CONCURRENCY`](/docs/configuration/reference/environment-variables#cubejs_scheduled_refresh_concurrency).
...
[`CUBEJS_DB_EXPORT_BUCKET`](/docs/configuration/reference/environment-variables#cubejs_db_export_bucket)).
...
[MySQL](/docs/configuration/data-sources/mysql#connecting-via-a-unix-socket).
...
The cluster name to use when connecting to [Materialize](/docs/configuration/data-sources/materialize).
...
option](/docs/configuration/reference/config#context_to_api_scopes).

## `CUBEJS_DEV_MODE`

If `true`, enables [development mode](/docs/configuration#development-mode).
...
touch within [`CUBEJS_TOUCH_PRE_AGG_TIMEOUT`](/docs/configuration/reference/environment-variables#cubejs_touch_pre_agg_timeout).
...
option](/docs/configuration/reference/config#jwt).
...
[`CUBEJS_API_SECRET`](/docs/configuration/reference/environment-variables#cubejs_api_secret).
...
See also the [`logger` configuration option](/docs/configuration/reference/config#logger).
...
If Cube is configured to act as a refresh worker instance using [`CUBEJS_REFRESH_WORKER`](/docs/configuration/reference/environment-variables#cubejs_refresh_worker),
then [`CUBEJS_PRE_AGGREGATIONS_BUILDER`](/docs/configuration/reference/environment-variables#cubejs_pre_aggregations_builder) will be `true`.

</Info>

## `CUBEJS_PRE_AGGREGATIONS_SCHEMA`

The [database schema name](/docs/configuration/reference/config#pre_aggregations_schema)
...
option](/docs/configuration/reference/config#pre_aggregations_schema).
...
mode](/docs/caching/using-pre-aggregations#rollup-only-mode) for details.
...
option](/docs/configuration/reference/config#scheduled_refresh_time_zones).
...
option](/docs/configuration/reference/config#schema_path).
...
option](/docs/configuration/reference/config#check_sql_auth).
...
option](/docs/configuration/reference/config#can_switch_sql_user).

## `CUBEJS_ALLOW_UNGROUPED_WITHOUT_PRIMARY_KEY`

If `true`, disables the primary key inclusion check for
[ungrouped queries](/api-reference/queries#ungrouped-query).
...
option](/docs/configuration/reference/config#allow_ungrouped_without_primary_key).
...
clause](/api-reference/queries#order) for [SQL API][ref-sql-api] queries.
...
[`CUBEJS_DROP_PRE_AGG_WITHOUT_TOUCH`](/docs/configuration/reference/environment-variables#cubejs_drop_pre_agg_without_touch) to
learn more.
...
See also [`CUBEJS_LOG_LEVEL`](/docs/configuration/reference/environment-variables#cubejs_log_level).
...
[ref-config-multiple-ds-decorating-env]: /docs/configuration/advanced/multiple-data-sources#configuring-data-sources-with-environment-variables-decorated-environment-variables
[ref-config-sched-refresh-timer]: /docs/configuration/reference/config#scheduled_refresh_timer
[ref-rest-scopes]: /api-reference/rest-api#configuration-api-scopes
...
[ref-sql-api-streaming]: /api-reference/sql-api#streaming
[ref-row-limit]: /api-reference/queries#row-limit
[mysql-server-tz-support]: https://dev.mysql.com/doc/refman/8.4/en/time-zone-support.html
[ref-schema-ref-preagg-allownonstrict]: /docs/data-modeling/reference/pre-aggregations#allow_non_strict_date_range_match
...
[ref-context-to-app-id]: /docs/configuration/reference/config#context_to_app_id
...
[ref-mdx-api-locale]: /api-reference/mdx-api#measure-format
[ref-time-dimensions]: /docs/data-modeling/reference/dimensions#time
