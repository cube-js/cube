...
description: "Following [configuration options][ref-config-options] can be defined either using Python, in a cube.py file, or using JavaScript, in a cube.js file."
---
...
This configuration option can also be set using the [`CUBEJS_SCHEMA_PATH`](/docs/configuration/reference/environment-variables#cubejs_schema_path)
...
This configuration option can also be set using the [`CUBEJS_CACHE_AND_QUEUE_DRIVER`](/docs/configuration/reference/environment-variables#cubejs_cache_and_queue_driver)
...
can be configured **statically**. Use [`CUBEJS_DATASOURCES`](/docs/configuration/reference/environment-variables#cubejs_datasources) and decorated environment
...
system-level settings. Please use [`CUBEJS_ROLLUP_ONLY`](/docs/configuration/reference/environment-variables#cubejs_rollup_only), [`CUBEJS_DB_QUERY_TIMEOUT`](/docs/configuration/reference/environment-variables#cubejs_db_query_timeout), and
[`CUBEJS_CONCURRENCY`](/docs/configuration/reference/environment-variables#cubejs_concurrency) [environment variables][ref-environment-variables] instead.
...
| Option              | Description                                                                                                                                                                                                                        | Default Value |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| `concurrency`       | Maximum number of queries to be processed simultaneosly. For drivers with connection pool [`CUBEJS_DB_MAX_POOL`](/docs/configuration/reference/environment-variables#cubejs_db_max_pool) should be adjusted accordingly. Typically pool size should be at least twice of total concurrency among all queues. | `2`           |
...
This configuration option can also be set using the [`CUBEJS_PRE_AGGREGATIONS_SCHEMA`](/docs/configuration/reference/environment-variables#cubejs_pre_aggregations_schema)
environment variable.
...
Cube enables background refresh by default using the [`CUBEJS_REFRESH_WORKER`](/docs/configuration/reference/environment-variables#cubejs_refresh_worker)
environment variable.
...
[`CUBEJS_SCHEDULED_REFRESH_TIMEZONES`](/docs/configuration/reference/environment-variables#cubejs_scheduled_refresh_timezones) environment variable.
...
This configuration option can also be set using the
[`CUBEJS_ALLOW_UNGROUPED_WITHOUT_PRIMARY_KEY`](/docs/configuration/reference/environment-variables#cubejs_allow_ungrouped_without_primary_key) environment variable.
...
The base path for the [REST API](/api-reference/rest-api/reference).
...
was set via [`CUBEJS_DEFAULT_API_SCOPES`](/docs/configuration/reference/environment-variables#cubejs_default_api_scopes) is provided as the second argument.
...
| ------ | ----------- | -------------------- |
| `jwkUrl` | URL from which JSON Web Key Sets (JWKS) can be retrieved | Can also be set using [`CUBEJS_JWK_URL`](/docs/configuration/reference/environment-variables#cubejs_jwk_url) |
| `key` | JSON string that represents a cryptographic key. Similar to [`CUBEJS_API_SECRET`](/docs/configuration/reference/environment-variables#cubejs_api_secret) | Can also be set using [`CUBEJS_JWT_KEY`](/docs/configuration/reference/environment-variables#cubejs_jwt_key) |
| `algorithms` | [Any supported algorithm for decoding JWTs][gh-jsonwebtoken-algs] | Can also be set using [`CUBEJS_JWT_ALGS`](/docs/configuration/reference/environment-variables#cubejs_jwt_algs) |
| `issuer` | Issuer value which will be used to enforce the [`iss` claim from inbound JWTs][link-jwt-ref-iss] | Can also be set using [`CUBEJS_JWT_ISSUER`](/docs/configuration/reference/environment-variables#cubejs_jwt_issuer) |
| `audience` | Audience value which will be used to enforce the [`aud` claim from inbound JWTs][link-jwt-ref-aud] | Can also be set using [`CUBEJS_JWT_AUDIENCE`](/docs/configuration/reference/environment-variables#cubejs_jwt_audience) |
| `subject` | Subject value which will be used to enforce the [`sub` claim from inbound JWTs][link-jwt-ref-sub] | Can also be set using [`CUBEJS_JWT_SUBJECT`](/docs/configuration/reference/environment-variables#cubejs_jwt_subject) |
| `claimsNamespace` | Namespace within the decoded JWT under which any custom claims can be found | Can also be set using [`CUBEJS_JWT_CLAIMS_NAMESPACE`](/docs/configuration/reference/environment-variables#cubejs_jwt_claims_namespace) |

### `check_sql_auth`

Used in the [SQL API][ref-sql-api]. Default
implementation verifies user name and password from environment variables:
[`CUBEJS_SQL_USER`](/docs/configuration/reference/environment-variables#cubejs_sql_user), [`CUBEJS_SQL_PASSWORD`](/docs/configuration/reference/environment-variables#cubejs_sql_password), but in [development
...
[`CUBEJS_SQL_SUPER_USER`](/docs/configuration/reference/environment-variables#cubejs_sql_super_user) and returns `true` when it's equal to session's user.
...
See also the [`CUBEJS_LOG_LEVEL`](/docs/configuration/reference/environment-variables#cubejs_log_level) environment variable.
...
alternatively, by setting [`CUBEJS_TELEMETRY`](/docs/configuration/reference/environment-variables#cubejs_telemetry) environment variable to `false`.
...
[`CUBEJS_DB_TYPE`](/docs/configuration/reference/environment-variables#cubejs_db_type) to resolve the data source type.
...
[ref-pre-aggregations-refresh-key]: /docs/data-modeling/reference/pre-aggregations#refresh_key
[ref-schema-cube-ref-refresh-key]: /docs/data-modeling/reference/cube#refresh_key
[ref-schema-cube-ref-ctx-sec-ctx]: /docs/data-modeling/reference/context-variables#security_context
...
[ref-schema-ref-datasource]: /docs/data-modeling/reference/cube#data_source
...
[ref-environment-variables]: /docs/configuration/reference/environment-variables
[ref-rest-scopes]: /api-reference/rest-api#api-scopes
...
[ref-websockets]: /api-reference/rest-api/real-time-data-fetch
