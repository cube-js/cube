...
description: Give each tenant its own database while sharing one semantic model by combining contextToAppId, driverFactory, and JWT security context as in this recipe.
---
...
[`contextToAppId`](/docs/configuration/reference/config#contexttoappid) function to
provide distinct identifiers for each tenant. Also, implement the
[`driverFactory`](/docs/configuration/reference/config#driverfactory) function where
