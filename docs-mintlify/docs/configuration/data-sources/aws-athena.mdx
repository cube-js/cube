---
title: Amazon Athena
description: Learn how to connect Cube to Amazon Athena using IAM credentials, region and S3 output settings, workgroups, and related environment variables.
---
...
- You can also provide [`CUBEJS_AWS_KEY`](/docs/configuration/reference/environment-variables#cubejs_aws_key) and [`CUBEJS_AWS_SECRET`](/docs/configuration/reference/environment-variables#cubejs_aws_secret) as master credentials for the role assumption
...
| ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ | :------: |
| [`CUBEJS_AWS_KEY`](/docs/configuration/reference/environment-variables#cubejs_aws_key)                                | The AWS Access Key ID to use for database connections                                                                  | A valid AWS Access Key ID                        | ❌<sup>1</sup> |
| [`CUBEJS_AWS_SECRET`](/docs/configuration/reference/environment-variables#cubejs_aws_secret)                             | The AWS Secret Access Key to use for database connections                                                              | A valid AWS Secret Access Key                    | ❌<sup>1</sup> |
| [`CUBEJS_AWS_REGION`](/docs/configuration/reference/environment-variables#cubejs_aws_region)                             | The AWS region of the Cube deployment                                                                                  | [A valid AWS region][aws-docs-regions]           |    ✅    |
| `CUBEJS_AWS_S3_OUTPUT_LOCATION`                 | The S3 path to store query results made by the Cube deployment                                                         | A valid S3 path                                  |    ❌    |
| [`CUBEJS_AWS_ATHENA_WORKGROUP`](/docs/configuration/reference/environment-variables#cubejs_aws_athena_workgroup)                   | The name of the workgroup in which the query is being started                                                          | [A valid Athena Workgroup][aws-athena-workgroup] |    ❌    |
| [`CUBEJS_AWS_ATHENA_CATALOG`](/docs/configuration/reference/environment-variables#cubejs_aws_athena_catalog)                     | The name of the catalog to use by default                                                                              | [A valid Athena Catalog name][awsdatacatalog]    |    ❌    |
| [`CUBEJS_AWS_ATHENA_ASSUME_ROLE_ARN`](/docs/configuration/reference/environment-variables#cubejs_aws_athena_assume_role_arn)             | The ARN of the IAM role to assume for Athena access                                                                    | A valid IAM role ARN                             |    ❌    |
| [`CUBEJS_AWS_ATHENA_ASSUME_ROLE_EXTERNAL_ID`](/docs/configuration/reference/environment-variables#cubejs_aws_athena_assume_role_external_id)     | The external ID to use when assuming the IAM role (if required by the role's trust policy)                             | A string                                          |    ❌    |
| [`CUBEJS_DB_NAME`](/docs/configuration/reference/environment-variables#cubejs_db_name)                                | The name of the database to use by default                                                                             | A valid Athena Database name                     |    ❌    |
| [`CUBEJS_DB_SCHEMA`](/docs/configuration/reference/environment-variables#cubejs_db_schema)                              | The name of the schema to use as `information_schema` filter. Reduces count of tables loaded during schema generation. | A valid schema name                              |    ❌    |
...
<sup>1</sup> Either provide [`CUBEJS_AWS_KEY`](/docs/configuration/reference/environment-variables#cubejs_aws_key) and [`CUBEJS_AWS_SECRET`](/docs/configuration/reference/environment-variables#cubejs_aws_secret) for static credentials, or use [`CUBEJS_AWS_ATHENA_ASSUME_ROLE_ARN`](/docs/configuration/reference/environment-variables#cubejs_aws_athena_assume_role_arn) for role-based authentication. When using role assumption without static credentials, the driver will use the AWS SDK's default credential chain (IAM instance profile, EKS IRSA, etc.).
