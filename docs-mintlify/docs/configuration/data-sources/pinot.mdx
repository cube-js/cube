---
title: Apache Pinot
description: Wire Cube to an Apache Pinot broker, including required cluster features, env-only configuration when the UI omits Pinot, and broker host settings.
---
...
|---------------------------------|-------------------------------------------------------|---------------------|:--------:|
| [`CUBEJS_DB_HOST`](/docs/configuration/reference/environment-variables#cubejs_db_host)                | The host URL for your Pinot broker                    | A valid host URL    |    ✅    |
...
| [`CUBEJS_DB_USER`](/docs/configuration/reference/environment-variables#cubejs_db_user)                | The username used to connect to the broker            | A valid username    |    ❌    |
| [`CUBEJS_DB_PASS`](/docs/configuration/reference/environment-variables#cubejs_db_pass)                | The password used to connect to the broker            | A valid password    |    ❌    |
| [`CUBEJS_DB_NAME`](/docs/configuration/reference/environment-variables#cubejs_db_name)                | The database name for StarTree                        | A valid name        |    ❌    |
| [`CUBEJS_DB_PINOT_NULL_HANDLING`](/docs/configuration/reference/environment-variables#cubejs_db_pinot_null_handling) | If `true`, enables null handling. Default is `false`  | `true`, `false`     |    ❌    |
| [`CUBEJS_DB_PINOT_AUTH_TOKEN`](/docs/configuration/reference/environment-variables#cubejs_db_pinot_auth_token)    | The authentication token for StarTree                 | A valid token       |    ❌    |
