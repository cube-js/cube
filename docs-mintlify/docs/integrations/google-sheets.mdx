...
{/* TODO: screenshot — a pivoted exploration with the Total column on the sheet */}
...
of that exploration's placements in the current document at once — a
placement whose result is identical to what's already there is left
untouched instead of being rewritten; click its title to open it and change
the query, which applies to every placement.
...
while each row shows **Queued**, **Refreshing…**, **No changes** for a
placement left untouched as above, or **Not refreshed** with a reason if it
failed. The run keeps going if you navigate away and back; refreshes are
written one sheet at a time, so a spreadsheet with many sheets refreshes
...
If someone shares a spreadsheet with you that has a placement you don't have
access to, placing your cursor in it shows a note that names the owner so you
can request access; it never reveals the exploration's title or data.
...
When you open the add-on on a spreadsheet someone else shared with you, the
spreadsheet home lists every Cube-backed placement in the document, including
ones you don't have access to. A placement you can't open shows only its
location and who to contact for access — its exploration name and data stay
hidden.
...
[Chats History](/admin/monitoring/chats-history). These conversations stay
in the add-on — they don't appear in **History** in the
[Cube workspace](/docs/explore-analyze/analytics-chat#history).
