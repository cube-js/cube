...
Column and row totals are saved with the exploration and survive **Refresh**;
with auto-run off, toggling either switch waits for **Run** like any other
display change.
...
filters dropped and dimension filters kept, and totals taken over the
query's full result rather than the rows on the sheet. With both switches
on, the cell where they meet holds a grand total across the whole result,
from a third query with neither zone kept.
...
request. Deleting one this way is [narrower than from the Workspace
page][ref-deleting-folders].
...
— and, like other conversations, visible to deployment admins under
