...
row shows **Queued**, **Refreshing…**, **No changes** if the result is
identical to what's already there (the cells are left untouched), or **Not
refreshed** with a reason if it failed. The run keeps going if you navigate
away and back; refreshes are written one sheet at a time, so a workbook with
many sheets refreshes noticeably slower than a single placement.
