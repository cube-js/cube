...
row shows **Queued**, **Refreshing…**, **No changes** for a placement left
untouched as above, or **Not refreshed** with a reason if it failed. The run
keeps going if you navigate away and back; refreshes are written one sheet at
a time, so a workbook with many sheets refreshes noticeably slower than a
single placement.
...
If someone shares a workbook with you that has a placement you don't have
...
When you open the add-in on a workbook someone else shared with you, the
workbook home lists every Cube-backed placement in the document, including
...
Drag a saved exploration or an unsaved draft from the workbook's home onto
...
pane — every conversation you've had in the add-in on this deployment, not
just the current workbook's — and sync with your Cube account, so they're
...
in the add-in — they don't appear in **History** in the
