...
generates ready-to-run `setspn` and `ktpass` commands, stores the XMLA service account
credentials, and accepts the keytab upload.
...
format, e.g., `cube-pbi-svc-account@CUBE.DEV`. A NetBIOS format like
`CUBE\cube-pbi-svc-account` will not match.
...
is set in NetBIOS format instead of `user@REALM`.
