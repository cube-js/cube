...
The plugin connects to the same endpoint as the Cube connector. If you already connected
the connector, Claude Code uses one server, not two.
...
CLI is required for `cube-admin`, `cube-embed`, and most of `cube-deploy`, which the MCP
server does not cover.
...
| `cube-explore-content` | Browses workbooks, workbook reports, explorations, dashboards, folders, and scheduled notifications. | CLI |
...
| `cube-deploy` | Manages deployments, environments, environment variables, builds, and logs. | CLI |
