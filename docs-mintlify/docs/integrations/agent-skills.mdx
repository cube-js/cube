...
In Claude Code, the skills ship as the **Cube plugin**, together with the hosted
[Cube MCP server](/docs/integrations/mcp-server). In other coding agents, you install the
skills on their own and they drive the [Cube CLI](/reference/cli).
...
| **Cube agent skills** | Workflows for operating Cube — model, content, access, deployments | Claude Code as the Cube plugin; other coding agents over the `cube` CLI |
...
You don't need the Cube connector as well. If you already connected it, Claude Code
connects once, through the plugin, and `/mcp` lists the connector as hidden. A Cube server
you added yourself with `claude mcp add` takes precedence over the plugin's instead.
...
This installs the skills only. Install the Cube CLI below, and optionally
[add the Cube MCP server](/docs/integrations/mcp-server) to your agent.
...
CLI is required for `cube-admin`, `cube-embed`, and `cube-deploy`, and for finding saved
content with `cube-explore-content` — work the MCP server does not cover.
...
| `cube-explore-content` | Browses workbooks, workbook reports, explorations, dashboards, folders, and scheduled notifications. | CLI; MCP reads a known workbook or report |
...
production only when you ask to publish them. Every MCP call and CLI command runs as the
