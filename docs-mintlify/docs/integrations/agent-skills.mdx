...
skills on their own and connect the MCP server or the [Cube CLI](/reference/cli) yourself.
...
| **Cube agent skills** | Workflows for operating Cube — model, content, access, deployments | Claude Code as the Cube plugin; other coding agents over the MCP server or the `cube` CLI |
