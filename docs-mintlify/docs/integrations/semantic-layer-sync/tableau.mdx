...
Use environment variables to store your personal access token secret rather
than hardcoding it in your configuration. In Python, use
`os.environ['CUBEJS_TABLEAU_PAT_SECRET']` and in JavaScript, use
`process.env.CUBEJS_TABLEAU_PAT_SECRET`.
