...
- **Plain SQL string** (e.g., `sql: "SELECT id, status FROM raw.orders"`): Cube
  creates a helper Snowflake view named `CUBE_SV_SRC_<CUBENAME>` in a configurable
  schema (defaults to `PUBLIC`) and uses that view as the source for the semantic
  view.
- **Simple table reference** (e.g., `sql: MY_SCHEMA.MY_TABLE`): treated the same
  as `sql_table`.
- **Templated SQL** (e.g., Jinja or dbt expressions): not supported — the push
...
- The Snowflake role configured for your Cube data source (via [`CUBEJS_DB_SNOWFLAKE_ROLE`](/reference/configuration/environment-variables#cubejs_db_snowflake_role))
  ...
  `CREATE VIEW` on the schema where helper views are created (defaults to `PUBLIC`).
...
- **Cubes with templated `sql`.** If a cube's `sql` property uses template
  ...
  pushed. Cubes that use `sql_table` or a plain SQL string are supported — see
  [Cubes defined with `sql`](#cubes-defined-with-sql) for details.
