...
| Blocking | Dynamic JavaScript/TypeScript/Jinja models, unflattened `extends`, any Cube access policy on the view or a referenced cube, mixed data sources, a two-part `schema.table` source without `CUBEJS_DB_DATABRICKS_CATALOG`, ambiguous roots, non-equality/cyclic/one-to-many joins, unsupported expressions or types, and multi-stage/window/ranking calculations | Set `CUBEJS_DB_DATABRICKS_CATALOG` for a two-part source; otherwise simplify the model or keep that view in Cube. For `CUBE_VIEW_JOIN_NOT_REPRESENTABLE`, root the view at the many-side cube. |
