...
When saving settings through the API, set `deletionPolicy` to `retain`. The older
`delete-managed` value is deprecated but remains accepted for existing API clients. It does not
delete obsolete Metric Views; it currently behaves like `retain`. Choose `retain` for new settings.
