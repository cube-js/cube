...
- Ask your Databricks administrator for a **dedicated target catalog and schema** for the preview.
...
<Steps>
  <Step title="Configure publication">
    ...
    views, selected views, or a name pattern; optionally add a target-name prefix. A view that
    later leaves the scope keeps its Metric View in Databricks—there is no managed-delete option.
    Turn publication on and save. This alone does not start a write.
  </Step>
  <Step title="Preview the deployed model">
    ...
    issues. Preview requires deployment `SchemaRead` or `SchemaUpdate` access, reads the
    deployed model, and **does not write to Databricks**. Preview payloads and results are retained
    for at most seven days; run it again if an older result is gone.
  </Step>
...
The [publication settings API](/api-reference/databricks-metric-view-integration/get-databricks-metric-view-publication-settings)
and [publication preview API](/api-reference/databricks-metric-view-publication/start-a-databricks-metric-view-publication-preview)
support scripting configuration and read-only previews. Publication runs and cancellation also
have REST endpoints, but they are not yet in the public API reference during preview. Contact
[Cube support](/admin/account-billing/support) for API guidance if you need to automate them.
When saving settings through the API, use `deletionPolicy: retain`. The older
`delete-managed` value may still appear when reading a historical configuration, but new writes
reject it. If the API reference lists that value, treat it as legacy-only.
...
in Databricks. There is no automatic managed-delete action in this preview.
