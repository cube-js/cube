...
data source; URL-encode a different name in the path. Set `MAX_POLLS` to scale the wait for your
view count and warehouse start-up time (default: 180 polls, 10 seconds apart).
...
if [[ -z "$run" ]]; then
  echo "No run status received after $((max_polls * 10)) seconds: $CUBE_API_URL$status_url" >&2
  exit 1
fi
case "$(jq -r '.status' <<<"$run")" in
  QUEUED|RUNNING|CANCELLING)
    echo "No terminal run status observed after $((max_polls * 10)) seconds: $CUBE_API_URL$status_url" >&2
    exit 1 ;;
esac
