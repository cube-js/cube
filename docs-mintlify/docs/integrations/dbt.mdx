...
- The top-level `meta:` key works too. If a model carries both, `config.meta` wins.
...
- Use a boolean, or the strings `"true"` / `"false"` in any casing. Any other value is
  ignored.
...
and leaves its dimensions and measures as they are: a private cube can't be queried
directly, and a view that includes its members exposes them either way.
