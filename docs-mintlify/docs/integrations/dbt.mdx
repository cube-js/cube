...
Cube needs no access to your dbt repository on this path, and because no branch is
created, the "dbt sync is ready for review" notification does not fire — which matters
when the job runs on every pull request.
