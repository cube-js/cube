  ...
  generate:
    ...
    steps:
      ...
      - name: Commit the regenerated cubes
        run: |
          # `-A` rather than a hard-coded path: the CLI writes only under the
          # deployment's Output path, and naming it here would stage nothing on a
          # deployment configured with a different one — a green job over a
          # working copy that quietly drifted.
          git add -A
