...
to it. Data source connection variables (`CUBEJS_DB_*`) and
[`DBT_SANDBOX_VERSION`](#set-the-dbt-core-version) reach the sandbox automatically; any
...
The variables passed automatically, and a few names reserved by the sync itself
...
The sandbox runs **dbt Core 1.12.3** by default, together with the adapter for your
deployment's warehouse. To run a pull on a different version — to match the version your
dbt project is built against — set `DBT_SANDBOX_VERSION` on the deployment under
...
It reaches the sandbox automatically, so it isn't selected in the **Environment
variables** picker above.
...
- Cube doesn't restrict which version you pin. A version your warehouse adapter can't
  work with fails while the sandbox installs dependencies.
