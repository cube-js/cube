...
through MCP. It is not [Cube agent skills](/docs/integrations/agent-skills) — workflows
for coding agents, which the Cube plugin for Claude Code bundles together with this
server — or [Agent Skills in Cube](/docs/explore-analyze/skills), which are saved
workflows your data team authors for Analytics Chat.

<Note>

The MCP server is available on [all plans](https://cube.dev/pricing). <br />Users need the [Viewer][ref-roles] role or higher to interact with the MCP server. Which tools a
...
In Claude Code, you can instead install the [Cube plugin](/docs/integrations/agent-skills#claude-code),
which bundles this server with Cube agent skills.
