...
it, and folder actions run with no confirmation step; see [Deleting
folders][ref-deleting-folders]. A client that runs `chat` automatically because the
annotation says read-only can still cause an unconfirmed write through the agent's own
reply.
