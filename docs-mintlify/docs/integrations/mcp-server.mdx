...
- **Publishing to production needs an explicit ask.** `mergeToDefaultBranch` — the tool
  that makes a branch live — is marked destructive so a client always prompts for
  confirmation, and its own description tells the agent to call it only when you've
  asked to publish, never on its own initiative after a commit. See [Committing and
  publishing changes](#committing-and-publishing-changes).
