...
standalone exploration inside it. This tool's delete is narrower than deleting a folder
from the [Workspace page][ref-folders]: it only removes the folder itself — contents
move to the workspace root instead of being deleted — and refuses when the folder still
has sub-folders, rather than deleting the whole subtree.
...
isn't guaranteed to prompt first — the only guard is the tool's own description, which
tells the agent to set the flag only on your explicit say-so, never to retry a refusal on
its own.
...
| Tool | Description | Access |
| --- | --- | --- |
| `getBranchState` | Reports the branch this session works on, the branch the user is looking at in the Cube UI, the deploy branch, and whether the session's branch has uncommitted edits. | Read-only |
...
pull request. Use `switchUserBranch` to point the user's console at the result so they
can see it. Publish only when the user has asked to: `mergeToDefaultBranch` makes the
branch live for everyone and can't be undone from the response it returns. Call
`getBranchState` at any point to check which branch the user and the MCP session are
each on, and `discardDataModelChanges` to throw away an edit that isn't working out.
