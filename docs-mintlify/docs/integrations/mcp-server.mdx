...
Letting an AI client edit your semantic model is safe because of five constraints — three
...
- **Registration is permission-gated.** Every tool in Data model editing, Committing and
  ...
  any of them. A role scoped to **Edit data model on dev branches** still sees them
  registered, but `mergeToDefaultBranch` and `commitDataModelChanges` with
  `confirmPublishToDefaultBranch` still fail for that role, the same as any other write
  targeting the deploy branch.
