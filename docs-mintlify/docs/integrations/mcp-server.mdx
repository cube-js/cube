...
- **Publishing to production needs an explicit ask.** `mergeToDefaultBranch` — the tool
  ...
  dev branch created from the deploy branch commits straight onto it, live,
...
| Tool | Description | Access |
| --- | --- | --- |
| `getBranchState` | Reports the branch this session works on, the branch the user is looking at in the Cube console, the deploy branch, and whether the session's branch has uncommitted edits. | Read-only |
...
whose parent is the deploy branch unless the call also sets
