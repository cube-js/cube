...
prompts for `list` as it would for `delete`. Its delete destroys less than [deleting a
folder from the Workspace page][ref-deleting-folders] but checks less too — it skips the
content-permission check that path applies.
