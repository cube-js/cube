...
cause a write once it runs — `chat` is that case; see the warning below.
...
| `chat` | Asks a question of a Cube agent, optionally targeting a specific deployment and agent. Pass `branchName` to have the agent answer from a dev branch's model instead of the deployed one. | Read-only call — see warning below |
