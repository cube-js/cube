...
- **Find past conversations** – **History** lists chats relevant to where you open it: from inside a workbook, it shows that workbook's chats plus standalone ones; from a surface with no workbook (like the Semantic Model IDE), it shows every chat. Conversations started in the Google Sheets add-on or the Microsoft Excel add-in don't appear in History — they're listed under **Chat History** in the [Google Sheets](/docs/integrations/google-sheets#chat) or [Microsoft Excel](/docs/integrations/microsoft-excel#chat) pane instead
