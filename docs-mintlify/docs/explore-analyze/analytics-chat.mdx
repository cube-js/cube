...
- **Find past conversations** – **History** scopes to where you open it: a workbook shows its own chats plus any chats not tied to a workbook; a surface with no workbook, like the Semantic Model IDE, shows every chat you've had in the workspace. Conversations from the Google Sheets add-on or the Microsoft Excel add-in don't appear here — they're under **Chat History** in the [Google Sheets](/docs/integrations/google-sheets#chat) or [Microsoft Excel](/docs/integrations/microsoft-excel#chat) pane instead
