...
Pie charts do not use the X, Y, or Color channels. Their **Fields** tab is its own panel — **Rings**, **Measure** (which sets slice size), and **Tooltips** — where the innermost ring also sets the slice colors.
...
When a chart plots several measures at once, the **Fields** tab replaces the measure channel with a **Series** section listing one row per measure. Every series then gets independent color and style settings in [series configuration](/docs/explore-analyze/charts/configuration/series-configuration).
