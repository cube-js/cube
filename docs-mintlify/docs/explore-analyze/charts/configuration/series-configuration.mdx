...
Series configuration lets you override chart settings on a per-series basis. Access it from the **Fields** tab by expanding an individual series. With one measure that is the Y-axis section; with several, the Y channel is replaced by a **Series** section listing one row per measure — see [series mapping](/docs/explore-analyze/charts/configuration/series-mapping).
