...
- **One axis side.** A chart using both axis sides cannot be split, and a split chart offers one side only — the second side's option is disabled with a reason in each measure's [axis side](/docs/explore-analyze/charts/configuration/fields#axis-sides) toggle.
