...
A measure with no break-down is drawn as a single series in a color picked automatically: the next
palette color after every break-down's values, so it never collides with them.

Two measures broken down by the *same* dimension share that dimension's color scale. They keep one
legend row each, and a palette change on either applies to both.
