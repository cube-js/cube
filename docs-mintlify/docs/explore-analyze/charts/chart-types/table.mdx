...
- The three icon buttons are mutually exclusive with each other: picking one replaces the other. Clicking the active one again clears it, returning the column to a plain value.
...
- **[`format: link`](/docs/data-modeling/dimensions#format) on the dimension**, when the
...
{/* TODO screenshot: table with an inline link column and the Style tab's link button selected (hidden — replace this comment with <Frame><img src="..." /></Frame> when image is ready) */}
