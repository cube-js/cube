...
| `GET` | `/api/v1/deployments/{deploymentId}/scheduled-tasks/{taskId}/runs` | Lists the chat threads the task's runs produced, newest first |
...
and `timezone` make up its **Schedule**.
...
Each run starts a new chat thread with no memory of earlier runs, so write the
prompt as a complete request.
