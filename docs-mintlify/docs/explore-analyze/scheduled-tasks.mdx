...
A custom cron must be a standard five-field expression whose minute field
is a single number: `0 * * * *` (every hour) and `30 9 * * 1-5` (9:30 on
weekdays) work, while `*/15 * * * *` is rejected.
