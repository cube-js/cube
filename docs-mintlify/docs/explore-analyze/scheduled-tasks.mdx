...
Each user can have up to **20 active tasks**, where active means scheduled
and enabled. Manual and paused tasks don't count, so you can keep as many of
them as you like. At the limit, creating a scheduled task or resuming a
paused one fails until you pause or delete another task, or save the new one
as **Manual**.
...
Everyone who can use the deployment can see its tasks. Only a task's
creator, or an admin, can edit, pause, run, or delete it, because every run
executes as the creator (see [Reading the output](#reading-the-output)).
...
**Scheduled** page. A cron that fires more than once an hour, an unknown time
zone, or a 21st active task returns `400`. Changing, running, or deleting
another user's task returns `403` unless you are an admin, and every endpoint
returns `403` while Scheduled Tasks aren't enabled for your account.
