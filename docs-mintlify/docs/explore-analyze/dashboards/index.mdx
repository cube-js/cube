...
in-progress edits. (Publishing is a workbook-level operation, so it's also
available from source control — see [Dashboards as
code](/docs/explore-analyze/dashboards/dashboards-as-code).)
...
When someone opens a published dashboard in Cube Cloud, a version chip in the
header shows the live version (e.g. `v3`). Opening it reveals when that version
was published and a summary of what changed since the previous version — added,
updated, or removed widgets — so viewers can see which version they're looking at
and what changed in it. (The chip is in-app only; it doesn't appear on
[embedded dashboards](/embedding/iframe/dashboards).)
