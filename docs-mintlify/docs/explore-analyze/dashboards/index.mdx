...
Click **Comments** in a dashboard's header to open the comments panel and start a discussion about the dashboard. To comment on a specific chart instead, hover the widget, open its **⋮** menu, and choose **Add comment** — the panel's scope picker then lets you switch between **All threads** and that widget's own thread.

Type `@` in the composer to mention a teammate; they get a [notification][ref-notifications] linking back to the comment. React to any comment with one of six emoji, reply to build a thread, and **Resolve** it once it's addressed — resolved threads move to their own tab so the panel stays focused on what's open.

Turn commenting on for your account from **Admin → Settings → Dashboard Comments**, using the **Allow comments on dashboards** switch. This is separate from [comments on embedded dashboards](/embedding/iframe/feature-visibility#account-settings), which has its own toggle and holds separate conversations.
