...
A published dashboard's header shows a version chip (e.g. `v4`). Click it to
see who published that version, when, and a summary of what changed since the
previous published version — widgets added, updated, or removed, and whether
...
Open a published dashboard and click **Comment** to start a discussion where
it matters. Reply to a comment (one level deep) or react to it with an
emoji instead of replying. Mark a thread **Resolved** to close the
discussion — a resolved thread keeps its comments and reactions visible but
takes no further replies or reactions until it's reopened.

An admin must turn on the **Allow comments on dashboards** switch on the
**Admin → Settings** page before this appears. That's a separate setting
from [comments on embedded
dashboards](/embedding/iframe/feature-visibility#account-settings), which
has its own switch and isn't affected by this one.
