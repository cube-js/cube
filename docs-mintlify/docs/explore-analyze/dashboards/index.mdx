...
A dashboard you build in the workbook is a **draft** until you publish it. Click
**Publish** in the dashboard builder to publish the current draft as a new
**version** — stakeholders always see the latest published version, never your
in-progress edits.

The button next to **Publish** reflects the dashboard's state:

- Nothing is shown before the first publish.
- **`v{N}`** once the published version and your draft match — click it to open
  the [revision history](#revision-history).
- **Draft** once you have unpublished changes — click it to open the same
  revision history.
...
Open the revision history from the **`v{N}`** / **Draft** button to browse every
published version alongside the current draft. Each row shows the version number,
when it was published, and by whom. Select a version to preview it, then
**Restore** to make it your current draft — publish again to make it the live
version.

### What viewers see

On a published dashboard, a version chip in the header shows the live version
(e.g. `v3`). Opening it reveals when that version was published and a summary of
what changed since the previous version — added, updated, or removed widgets — so
viewers can tell at a glance whether the dashboard has moved on.
