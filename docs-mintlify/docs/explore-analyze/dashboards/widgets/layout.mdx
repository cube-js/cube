...
A new tabs container is added 6 cells wide and 9 tall. A board with fewer than 6 columns narrows the width to its own column count; inside a grid both the width and the height come down to the grid's own column and row counts. Neither narrowing goes below the 4×6 minimum, and the grid then needs a free block of the narrowed size — 6×9 in a grid at least that large — so it refuses the drop rather than inserting a smaller container. It can't be resized below 4×6 either, inside a grid as much as on the canvas; and if a tab holds a grid, it also can't shrink past what that grid's own contents need.
