...
Switching a tab writes it into the page URL as `?tab_<widget_id>=<tab_id>`, so a link you copy reopens on the tab you were reading; only the containers you actually switched are written into that link. You can also [set that parameter by hand](/embedding/iframe/dashboards#pre-set-dashboard-filters-via-url) to open a published or embedded dashboard on a particular tab. Exporting the dashboard as PNG or PDF, or a scheduled-refresh screenshot, renders every tab of the container as its own titled section, one below the other.
