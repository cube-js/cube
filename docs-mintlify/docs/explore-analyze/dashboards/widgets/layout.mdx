...
Click the grid's header to select it, then drag its resize grip to change its size, or open its gear menu for **Edit title**, **Resize**, and **Delete Grid** ([what that removes](#deleting-a-container)). Resizing only crops or restores empty rows and columns: a grid can't shrink below 2×2 or cut into an occupied cell. To name a grid, click into its header once it's selected and type a title.
...
- **Second drop** — refused with *This tab already holds a widget.*; the dragged widget returns to where it came from.
- **Anything else** — a tab takes no text, AI summary, control, spacer, or divider: the drop reverts with *Only a chart or a container can be placed in a tab.*
...
A tabs container can sit on the canvas, inside a grid, or inside a tab, but not inside a stack ([nesting rules](#nesting-containers)). A container nested inside a tab is sized by the tab body rather than by hand: a nested grid keeps **Edit title** and **Delete Grid** but drops **Resize**, and a nested stack or tabs container carries a delete button in its top-right corner instead of a menu.
