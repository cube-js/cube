...
A new grid is added as a square matching the board's column count — for example, `12 × 12` on a 12-column dashboard. Add a widget to it by dragging one in from the canvas, the **Add Widgets** menu, or another container; the widget keeps its size and lands on the cell you drop it on, as long as that cell is free.

Click the grid's header to select it, then drag its resize grip to change its size, or open its **⋯** menu for **Edit title**, **Resize**, and **Delete**. Resizing only crops or restores empty rows and columns: a grid can't shrink below 2×2 or cut into an occupied cell. To name a grid, click its header once it's selected — or choose **⋯ → Edit title** — and type a title.
