...
A new tabs container is added 6 cells wide and 9 tall. A board narrows the width alone, to its own column count when that is under 6, so the height is always 9; a grid narrows both, to its own column and row counts. Neither narrowing goes below the 4×6 minimum.

So a grid narrower than 4 columns or shorter than 6 rows can never take a tabs container at all, and a larger one takes it only where it has a free block of the narrowed size — 6×9 in a grid at least that big. It can't be resized below 4×6 either, inside a grid as much as on the canvas; and if a tab holds a grid, it also can't shrink past what that grid's own contents need.
...
One pairing is never allowed at any depth: a **grid** or a **tabs container** inside a stack — a stack has no cells of its own to give them. The reverse is fine, which is why a stack is the one container that goes anywhere.
