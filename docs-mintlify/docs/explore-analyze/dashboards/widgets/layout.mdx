...
Click the grid's header to select it, then drag its resize grip to change its size, or open its **⋯** menu for **Edit title**, **Resize**, and **Delete**. Resizing only crops or restores empty rows and columns: a grid can't shrink below 2×2 or cut into an occupied cell. To name a grid, click into its header once it's selected and type a title.
