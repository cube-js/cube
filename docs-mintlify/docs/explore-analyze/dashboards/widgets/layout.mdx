...
A new tabs container is added at 6×9 cells — narrowing to the dashboard's own column count when that is fewer than 6, though never below 4 — and can't be resized below 4×6. If a tab holds a grid, the container also can't shrink past what that grid's own contents need.
