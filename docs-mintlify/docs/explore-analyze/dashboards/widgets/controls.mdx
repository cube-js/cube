...
A faceted filter is narrowed by the siblings whose condition can be turned into a scope:

- A dimension of any type set to `is` or `is not`.
- A time dimension set to any of its date conditions except `is null` and `is not null`.

Every other condition narrows nothing and cannot be picked as a source — `contains`, `starts with` and `ends with` and their negations, the numeric comparisons (`greater than`, `less than` and their *or equal* forms), and `is null`, `is not null`, `is empty` and `is not empty`.
...
A date source scopes the value list to the rows the charts are showing, never to a wider window. A positive date condition scopes it to the window it names:
...
- `after`, `after or on`, `before` and `before or on` scope the list to everything on one side of the date, on the same bound the charts use — the list stays open-ended exactly where they are.
- A `between` range, including the **Past week** to **Past 2 years** presets, scopes the list to the range's own bounds. A range the charts return no rows for therefore leaves the value list empty too, instead of offering values the charts exclude.
