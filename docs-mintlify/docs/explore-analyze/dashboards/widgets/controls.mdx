...
A [field switcher](#field-switcher) never offers **Clear**, only **Reset to default**. Clearing one would land the viewer on [the member it replaces](#field-switcher-default-option), which the control already offers as an option — so the ✕ would duplicate a choice already in the dropdown.
