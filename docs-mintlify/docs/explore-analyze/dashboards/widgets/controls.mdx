...
| **Time** on any condition except `is null` and `is not null` | One per field in the UI; several only on `is` / `is not`, and only through a hand-written [URL parameter](#sharing-the-current-selection) |
...
Where the default comes from is set by the **Default value** dropdown in the filter's edit sidebar, below **Visibility**. It offers one source at a time:

| Default value | What the filter opens on |
|---|---|
| **Saved widget value** | A static default: the value (or values) you pick in the filter in the dashboard builder, saved on the widget. Every viewer sees the same default. This is the source of any filter you haven't set another one for. |
| **From user attribute** | The viewer's [user attribute](#user-attribute-default), resolved at load time, so each viewer sees their own personalized default. [Time granularity switchers](#time-granularity-user-attribute-default), [field switchers](#field-switcher-user-attribute-default) and [parent controls](#parent-user-attribute-default) offer this too — their **Default value** dropdown has only these first two options. |
| **First value of dimension**, **Last value of dimension** | A value of the filter's dimension, [looked up in the data](#defaults-resolved-from-the-data) every time the dashboard opens. |
| **Max value by measure**, **Min value by measure** | The value of the filter's dimension with the highest or lowest value of a measure you pick, [looked up in the data](#defaults-resolved-from-the-data) every time the dashboard opens. |
...
<Warning>
Saving the filter with a different **Default value** discards the value saved on the widget. Switching back to **Saved widget value** later does not restore it: the filter has no default until you pick a value in it again.
</Warning>

#### User attribute default

Set **Default value** to **From user attribute** in the filter's edit sidebar to pre-fill a filter from the viewer's [user attribute][ref-user-attributes]. When the dashboard loads, Cube looks up the attribute value for the current viewer and applies it as the filter's default.
...
<Steps>
 <Step title="Open the filter's settings">
   In the dashboard builder, open the filter widget's settings menu and choose **Edit Control**.
 </Step>
 <Step title="Choose From user attribute">
   Set the **Default value** dropdown to **From user attribute**. The sidebar explains the option and shows the attribute picker below it.
 </Step>
 <Step title="Pick the attribute">
   Select the [user attribute][ref-user-attributes] whose value should be used as the default. Only attributes defined in your account appear in the picker. Until you pick one, the sidebar reads *Pick an attribute — otherwise the saved value is kept.*, and saving without one keeps the filter on **Saved widget value**.
 </Step>
</Steps>
...
Empty, `null`, or unresolvable attribute values are skipped — the filter falls back to whatever static default it has, or no default if none is set. Saving the filter on **From user attribute** discards the value saved on the widget, as the warning above says, so a filter has this fallback only when you pick a value in it in the dashboard builder after that save.
...
#### Defaults resolved from the data

A saved value is fixed when you pick it, so it goes stale as the dimension's values change: a filter defaulting to the newest release keeps opening on that release after the next one ships. The four remaining **Default value** options look the value up in the data instead, every time the dashboard opens:

| Default value | Resolves to |
|---|---|
| **First value of dimension** | The first value of the filter's dimension, in the sort order you choose. |
| **Last value of dimension** | The last value of the filter's dimension, in the same sort order. |
| **Max value by measure** | The value of the filter's dimension whose row has the highest value of the measure you pick — on a **City** filter with **Count**, the city with the most rows. |
| **Min value by measure** | The value of the filter's dimension whose row has the lowest value of that measure. |

Rows where the dimension, or the measure you pick, is empty (`null`) are left out, so a default never resolves to an empty value while the dimension has others.

The first and last value are offered on string, number and time dimensions. Picking either shows a sentence — *The first value of Status, according to the selected sort order.* — and a sort order dropdown beneath it:

- **Natural**, the default, is the dimension's own order: alphabetical for a string, numeric for a number, chronological for a time dimension. The first value is the lowest, and the last value is the highest.
- **Database** is the order this filter's own value list shows its values in, and is offered on a string dimension only. That list holds the first 100 values, so **Last value of dimension** under **Database** is the last of those 100 — not the last value the dimension holds.

The max and min options show a measure dropdown instead, grouped by semantic view, with nothing picked and the note *Measures of views that share this dimension.* under it. It lists the measures of the filter's own view first, followed by those of every other view that includes the same dimension — one the data model declares as the same underlying cube member, the way a **Customers** view and an **Orders** view can both include the users cube's city. For a measure of another view, the ranking query runs in that view, over its own copy of the dimension, and the value it returns is applied to this filter. A view is not matched by name: one holding a same-named dimension of a *different* cube offers no measures. Where no view has any measures to offer, neither option appears in the **Default value** dropdown.

Until you pick a measure, the sidebar reads *Pick a measure — otherwise the saved value is kept.*, and saving without one keeps the filter on **Saved widget value**. Choosing a different dimension for the filter also puts a data-resolved default back on **Saved widget value**, so pick the option again for the new dimension.

<Warning>
A filter with a default resolved from the data cannot be changed by hand — in the dashboard builder or on the published dashboard, whatever its [visibility](#visibility). Its operator and value are greyed out, and the action that [clears or resets](#clearing-and-resetting) it is unavailable. To change the value, change the **Default value** in the filter's settings.
</Warning>

A muted note right after the filter's title, on the same line, names the source of its value, such as *First value of Status* or *Max by Count*. When the title leaves it too little room, the note is cut off with an ellipsis, and hovering it shows the whole note. On a published dashboard, the information icon at the end of the title line says the same: *This filter's value comes from First value of Status. Change it in the filter's settings.*

The value is resolved for each viewer and never saved on the dashboard, so a published dashboard keeps following the data without being republished. A value set elsewhere still wins: a filter value passed [in the URL](#sharing-the-current-selection) replaces the resolved one, and so does a [parent control](#parent) option that sets this filter to a value. The resolved value itself is never written into the URL.

A parent control option set to [**Reset to default**](#children) returns the filter to the value the data resolves to, as if the dashboard had just opened. One set to **Clear** does not:

<Warning>
Moving the parent to an option that sets or resets this filter gives it a value again. Until then, a parent option set to **Clear** leaves a filter with a default resolved from the data empty: the default is not resolved again, and because the filter cannot be changed by hand, the viewer cannot fill it either. Its charts stay unfiltered while that option is on.
</Warning>

A [faceted filter](#faceted-filters) resolves its default inside its facet scope, so the default is always a value its own value list offers, and it resolves again when a sibling filter changes. For a measure of another view, each sibling's condition is carried over to that view's matching dimension, and a condition with no match there leaves the filter with no value rather than resolving over rows its value list excludes. The warning icon described below names that case.

When the data gives no value, the filter applies none: the note after its title reads *No value applied*, and a warning icon at the end of the title line says why:

- The dimension returned no rows — for example, because the filter is [faceted](#faceted-filters) and a sibling narrows it to a period with none.
- The data could not be loaded — for example, because the query failed.
- The measure you picked has been removed from its view.
- The measure's view no longer includes the filter's dimension.
- A faceted filter's sibling condition has no matching dimension in the view of the measure you picked, so it cannot be carried over. The measure itself is fine; the sibling filter's dimension has no counterpart in that view.

In the dashboard builder, clicking the warning icon opens the filter's settings, where you can pick another measure or option.

### Faceted filters

When a dashboard carries more than one filter, you can mark one as **faceted**. A faceted filter scopes its value list to what its siblings are set to — selecting a value in one filter narrows the options shown in the faceted one, so viewers only see combinations that exist in the data. A sibling on the same semantic view narrows it directly; one on another semantic view narrows it once you [match it to a dimension of this one](#sources-on-another-semantic-view).
...
A faceted filter can take as a source any sibling, on its own semantic view or another, whatever condition that sibling is set to and whether or not it carries **Facet** itself — `is` and `is not`, the substring conditions, the comparisons on a **Number** dimension and every date condition are all read, and a **Boolean** dimension counts here too, its `is true` / `is false` setting `is`. That comes with qualifications:

- `is null`, `is not null`, `is empty` and `is not empty` carry no value and narrow as soon as they are set. Every other condition waits for a value, and until it has one it leaves the value list alone, just as it leaves the charts unfiltered.
- A condition and value that together leave nothing to scope by narrow nothing either, as the warning below sets out.
- Custom SQL scopes nothing. No operator menu offers it, so only a hand-written [URL parameter](#sharing-the-current-selection) can set a filter to it, and a source set that way leaves the value list alone.

To pick which siblings a filter listens to, turn the **Facet** switch on in its edit sidebar: the source picker appears with it, reading **All**. The picker lists every other filter on the dashboard and greys out the ones it cannot use — a sibling whose condition cannot narrow a value list — with a tooltip saying so. A filter on another semantic view is selectable, and picking one adds a row under the picker for [matching it to a dimension of this filter's view](#sources-on-another-semantic-view). Left at **All**, the filter listens to every sibling on its own semantic view that the picker leaves selectable; picking entries narrows it to those. A filter you picked that later stops qualifying stays in the list with a warning icon rather than disappearing from it.

**Cancel** leaves the filter on the sources it was last saved with — it discards every unsaved edit in the sidebar, not only the source list — while turning **Facet** back off clears the entries you picked, and the dimensions you matched them to, so switching it on again starts from **All**.

A date source never scopes the value list to less than the window the charts are showing, so the list never drops a value the charts still have rows for. Which window it scopes to depends on the condition; `is null` and `is not null` name no window and scope by presence instead, to the rows with no date and to the rows with one:
...
<Warning>
A source can end up with nothing to scope by, leaving the value list as wide as it was while the charts narrow. These are the cases where the charts match on something the value list has no way to ask for:
...
- `is not` on several dates — the scope can exclude the period one value names, not one period apiece.
- Several values on a substring condition, which only a [URL parameter](#sharing-the-current-selection) can give one — the charts match the values joined into a single term, not any of them, and the value list has no way to ask for that.
- `not contains`, `not starts with` or `not ends with` carrying `"caseSensitive": true`, which that parameter is the only way to set — the value list can be narrowed case-insensitively alone, which would drop values the charts still show.
</Warning>

A list that *is* being scoped does not necessarily look shorter — a long one fills the dropdown either way, and the values the window drops need not be the ones on screen. To check that a source is narrowing it, look for a value you know the window has no rows for and confirm it is gone from the list. Searching for that value is not a check: a filter's value box takes values that are not in its list, so it offers back whatever you type.

#### Sources on another semantic view
...
Nothing is stored until you **Save**, and clearing a match or unchecking the source drops it from the dashboard: picking that source again suggests a match afresh rather than restoring the one you cleared.

<Warning>
A source you picked but have not matched narrows nothing, leaving the value list as wide as it was — match it, or uncheck it. Its row says so while it is unmatched: *Until you pick a matching dimension, this source does not narrow the list.*
</Warning>
...
The default above is one granularity for everyone. To give each viewer their own, set **Default value** to **From user attribute** in the switcher's settings and pick a [user attribute][ref-user-attributes]. When the dashboard loads, Cube reads that attribute for the current viewer and opens the control on the granularity it names.
...
<Steps>
 ...
 <Step title="Choose From user attribute">
   Below **Visibility**, set the **Default value** dropdown to **From user attribute**.
 </Step>
 <Step title="Pick the attribute">
...
Like [filters](#user-attribute-default), [time granularity switchers](#time-granularity-user-attribute-default) and [parent controls](#parent-user-attribute-default), a field switcher can start each viewer on their own member. Set **Default value** to **From user attribute** in the control's settings and pick a [user attribute][ref-user-attributes]; when the dashboard loads, Cube reads that attribute for the current viewer and opens the control on the member it names.
...
The default above is one arrangement for everyone. To give each viewer their own, set **Default value** to **From user attribute** in the parent control's settings and pick a [user attribute][ref-user-attributes]. When the dashboard loads, Cube reads that attribute for the current viewer and opens the control on the option it names — and drives the children with it, exactly as if the viewer had picked that option themselves.
...
<Steps>
 ...
 <Step title="Choose From user attribute">
   Below the option tabs — next to **Visibility** — set the **Default value** dropdown to **From user attribute**.
 </Step>
 <Step title="Pick the attribute">
   Select the [user attribute][ref-user-attributes] to resolve. Only attributes defined in your account appear in the picker.
 </Step>
...
|---|---|---|
| **Clear** (✕) | The control has a value, and is neither a [field switcher](#field-switcher), which carries no ✕, nor a filter whose default is [resolved from the data](#defaults-resolved-from-the-data), which cannot be changed by hand. | Stops applying the control: a cleared filter leaves its charts unfiltered, and a cleared switcher hands each chart back to the granularity or member it was built with. Clearing a [parent control](#parent) mid-session only deselects it: the parent stops driving, and each child keeps the value it is showing rather than reverting. |
| **Reset to default** (↩) | The control has been changed, it has a value published in the last dashboard version, and the two differ — except on a filter whose default is [resolved from the data](#defaults-resolved-from-the-data), which cannot be changed by hand. | Puts the control back on that published value. |
...
| Custom SQL, which no operator menu offers | `custom` — a facet source set to it scopes nothing |
...
A substring condition takes `caseSensitive` the same way: with `true` the match is made with `LIKE` rather than `ILIKE`, so it respects case wherever the column's collation is case-sensitive, and without the key case is ignored — `f_orders.status={"type":"not_contains","value":"ship","caseSensitive":true}`. The value editor has no switch for it, so a filter matches case only where a link sets the key.

Outside `between`, a `value` may still be a list, and what a condition makes of one differs. `is` and `is not` read its entries as alternatives — `f_orders.status={"value":["shipped","processing"]}` — on a time dimension as much as a string or number one. A substring condition matches the entries joined with commas into one term, so `{"type":"contains","value":["ship","proc"]}` matches `%ship,proc%` and neither entry on its own. Every other condition reads exactly one value — the comparisons, and the `in the …` and `not in the …` family — and a list in their `value` leaves the widget unable to build its query at all.

What travels in the link, and what wins when a link value arrives:

- **Only what the viewer chose.** Values that came from the control's own configuration — a static default, a [default granularity](#default-granularity), a filter default [resolved from the data](#defaults-resolved-from-the-data) — are not written into the URL. Every viewer already gets those from the dashboard itself, and leaving them out means a link stays correct after the dashboard's defaults change.
...
|---|---|
| **Visible** (default) | Shown on the dashboard and viewers can change its value — except a filter whose default is [resolved from the data](#defaults-resolved-from-the-data), which no one can change by hand. |
