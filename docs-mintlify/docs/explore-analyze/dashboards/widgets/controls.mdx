...
<Info>
Add the child controls to the dashboard *before* the parent control. A tab lists only the controls that already exist, so a parent added to an empty dashboard has nothing to drive yet. Another parent control is never listed — a parent cannot be a child of one.
</Info>

### Options

Each option is a tab across the top of the editor. Add one with **+** at the end of the tab strip, rename a tab in place by editing its title, and delete one with the ✕ that appears on the tab. A parent control created without options opens on a single tab named **Default** — a real option, saved with the control and offered in the viewer's dropdown like any other, so rename it if that label isn't what viewers should see. A parent control can hold up to 50 options.
...
Open an option's tab and you see every filter, time granularity switcher and field switcher on the dashboard, each with a dropdown saying what that option does to it. You set one option's whole arrangement in one place, rather than configuring one child at a time.

Every option/child pair is in one of four states:
...
Click the indicator to open that control's own settings, where **Driven by parent** names the parent driving it.

### Driven by parent

Every filter, time granularity switcher and [field switcher](#field-switcher) carries a **Driven by parent** section in its own settings, so you can see what drives a control without opening the parent.

The dropdown offers the parent that currently drives it and **None**. Picking **None** detaches the control: the parent stops driving it, and it keeps whatever value it is showing. Pointing a control at a *different* parent is done in that parent's settings, which is where the per-option values live — as the section itself says: *Mapping is set in the parent control's settings.*
