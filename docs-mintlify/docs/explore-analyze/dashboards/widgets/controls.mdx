...
- `after`, `after or on`, `before` and `before or on` scope the list to everything on one side of the single date their value resolves to — for a whole-period preset, the day that period starts on, which is the date the charts compare against too.
...
<Warning>
Two shapes scope nothing at all, leaving the value list as wide as it was while the charts narrow: `is not` on a fixed date, on a single moment such as **3 months ago**, or on several values at once has no period to exclude, and `in the month`, `in the quarter` and `in the year` read one period from one value, so several values leave them with none.
</Warning>
