...
A faceted filter reads every condition its sources can be set to — all of the [operators above](#operators-by-dimension-type) — with one exception, `is not` on a time dimension set to a single moment, below.
