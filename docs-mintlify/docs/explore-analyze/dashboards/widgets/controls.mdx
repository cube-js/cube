...
Empty, `null`, or unresolvable attribute values are skipped — the filter falls back to whatever static default it has, or no default if none is set. Saving the switch to **From user attribute** discards the value saved on the widget, as the warning above says, so a filter has this fallback only when you pick a value in it in the dashboard builder after that save.
...
A line under the filter names the source of its value, such as *First value of Status* or *Max by Count*, and on a published dashboard the information icon beside the filter's title says the same: *This filter's value comes from First value of Status. Change it in the filter's settings.*
...
When the data gives no value, the filter applies none: the line under it reads *No value applied*, and a warning icon beside the filter's title says why:
