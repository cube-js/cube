...
- A time dimension set to any of its date conditions except `is null` and `is not null` — though `is not` and the `in the …` conditions each have a value shape they cannot narrow on.
...
<Warning>
Two conditions can end up with nothing to scope by, leaving the value list as wide as it was while the charts narrow:
...
- `in the month`, `in the quarter` and `in the year` on several values — each reads one period from one value.
</Warning>
