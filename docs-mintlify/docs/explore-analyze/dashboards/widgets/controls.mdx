...
A faceted filter can take as a source any sibling set to one of these conditions:
...
- A time dimension set to any of its date conditions except `is null` and `is not null`. `is not` is offered on any value but scopes nothing unless that value is a single whole period.
...
- `after`, `after or on`, `before` and `before or on` scope the list to everything on one side of the date. Every bound the value editor picks is a whole day, and on a whole day the scope uses the same bound the charts do, so the list stays open-ended exactly where they are. Cube reads a bound carrying a time of day as the start of that day, so such a bound — which only a hand-written URL parameter produces — is widened to that day's end rather than cutting the list early.
...
<Warning>
Watch for `is not` on a time dimension set to a fixed date, to a single moment such as **3 months ago**, or to several values at once: a point in time has no period to exclude, so the value list stays as wide as it was while the charts narrow.
</Warning>
