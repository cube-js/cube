...
Neither tab sets more than one value, and the [**Selection Mode**](#single-vs-multiple-selection) switch is not offered for a time dimension, so a time filter holds *several values* only when a hand-written [URL parameter](#sharing-the-current-selection) passes a list in its `value` — `f_orders.created_at={"type":"equals","value":["2026-01-01","2026-02-01"]}`.
