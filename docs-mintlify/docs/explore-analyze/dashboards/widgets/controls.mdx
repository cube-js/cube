...
A time filter's *value* is fixed or relative, picked on the value editor's **Fixed** and **Relative** tabs: a calendar date, a whole-period preset — **Today**, **Yesterday**, **This week**, **This month**, **This quarter**, **This year** — a single-moment preset such as **1 week ago**, **1 month ago**, **3 months ago** or **1 year ago**, or a custom *N periods ago* / *from now*. A `between` filter picks both bounds the same way, and its **Relative** tab offers whole windows from **Past week** to **Past 2 years**.
...
- `is` scopes the list to what its value names: the whole period for a whole-period preset, and the moment itself — what the charts match — for a fixed date or a single-moment preset. Several values scope the list to all of their windows.
...
- `is not` on a fixed date, on a single-moment preset such as **3 months ago**, or on several values at once — a point in time has no period to exclude.
