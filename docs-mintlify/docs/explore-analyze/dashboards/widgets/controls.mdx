...
- `is`, `in the month`, `in the quarter` and `in the year` scope the list to what their value names: the whole period for an [`in the …` condition or a whole-period preset](#operators-by-dimension-type), and the moment itself — what the charts match — for a fixed date or a single-moment preset. Several values scope the list to all of their windows.
...
- `between` scopes the list to the range's own whole-day bounds, whether those bounds are fixed or [relative](#operators-by-dimension-type). A range the charts return no rows for therefore leaves the value list empty too, instead of offering values the charts exclude.
...
<Warning>
`is not` on a fixed date, on a single moment such as **3 months ago**, or on several values at once has no period to exclude, so the value list stays as wide as it was while the charts narrow.
</Warning>
