...
| **Reset to default** (↩) | The control has been changed, it has a value published in the last dashboard version, and the two differ. Nothing to return to — a dashboard still being built, or one published while this control was empty — means no ↩ at all. | Puts the control back on that published value. |
...
A [field switcher](#field-switcher) offers only **Reset to default**. It has no ✕.

On a published dashboard, clearing or resetting changes only what that viewer sees, and writes nothing back to the dashboard. It doesn't outlast a reload, though: [neither action travels in the URL](#sharing-the-current-selection), so the control comes back on whatever it opens on for that viewer.
...
- **Neither a clear nor a reset.** [Clearing](#clearing-and-resetting) a control drops its parameter rather than recording the empty state, so an emptied control looks the same in the link as one nobody touched and the recipient reopens on the default the clear was there to remove. Reset drops the parameter too — which reproduces the reset state for a viewer whose control opens on the published value, but reopens on the attribute value for one seeded by a [user attribute](#user-attribute-default).
