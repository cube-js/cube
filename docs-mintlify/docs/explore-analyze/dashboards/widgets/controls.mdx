...
<Info>
A tab lists the controls that are on the dashboard, so a parent added to an empty one has nothing to drive yet. A control added later appears in the tabs by itself, but is not driven until you save the parent's settings again. Another parent control is never listed — a parent cannot be a child of one.
</Info>
...
| **Not driven** | No parent control drives this one — the parent has never been saved, every option keeps this control as-is, or it was detached with [**Driven by parent** → **None**](#driven-by-parent). |
