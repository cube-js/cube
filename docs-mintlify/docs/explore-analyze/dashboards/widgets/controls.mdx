...
- A time dimension set to any of its date conditions except `is null` and `is not null` — though `is not` and the `in the …` and `not in the …` conditions each have a value shape they cannot narrow on.
...
To pick which siblings a filter listens to, turn the **Facet** switch on in its edit sidebar: the source picker appears with it, reading **All**. Left at **All**, the filter listens to every sibling on the same semantic view; picking entries narrows it to those. A sibling whose condition cannot narrow is greyed out in that list, with a tooltip saying it is not available for this semantic view or filter type.

<Warning>
Press **Cancel** in the sidebar to restore the sources the filter was last saved with: turning **Facet** back off clears the list you picked, and switching it on again starts from **All**.
</Warning>

A date source never scopes the value list to less than the window the charts are showing, so the list never drops a value the charts still have rows for. Which window a source scopes to depends on the condition:

- `is` scopes the list to what its value names: the whole period for a whole-period preset, and the moment itself — what the charts match — for a fixed date or a single-moment relative value. Several values scope the list to all of their windows.
- `in the month`, `in the quarter` and `in the year` scope the list to the whole period their value falls in.
- `after`, `after or on`, `before` and `before or on` scope the list to everything on one side of the single whole day their value resolves to — for a whole-period preset, the day that period starts on, which is the day the charts compare against too.
- `between` scopes the list to the range's own whole-day bounds, whether those bounds are fixed or relative. A range the charts return no rows for therefore leaves the value list empty too, instead of offering values the charts exclude.
- The negated conditions — `not in the month`, `not in the quarter`, `not in the year`, and `is not` on a whole-period value — scope the list the other way, to everything *outside* that period.
