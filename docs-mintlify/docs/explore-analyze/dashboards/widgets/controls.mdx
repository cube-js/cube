...
A date source scopes the value list to the window the charts are showing, and never to a narrower one, so the list never drops a value the charts still have rows for. A positive date condition scopes it to the window it names — and, where `is` carries several values, to all of their windows at once:
...
- `after`, `after or on`, `before` and `before or on` scope the list to everything on one side of the date. Every bound the value editor picks is a whole day, and on a whole day the scope uses the same bound the charts do, so the list stays open-ended exactly where they are.
