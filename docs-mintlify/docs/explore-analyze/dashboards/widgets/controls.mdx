...
If a [parent control](#parent) drives this filter, the option the viewer is on decides it: one that [keeps the filter as-is](#children) lets the attribute seed it, and one set to **Clear** empties it and the attribute does *not* seed it back — the filter stays unfiltered until the viewer sets it or a value arrives [in the URL](#sharing-the-current-selection).
...
This reaches controls another parent already drove. [Only one parent drives a control](#one-parent-per-child), so saving a second parent takes them over and leaves the first driving nothing — any row that would take a control over says so first, naming the parent that owns it — that note appears on every row except the ones reading **Keep as-is**, so it covers the controls the save would claim without your touching them. On a dashboard with more than one parent control, map the one you are editing deliberately rather than saving it to dismiss the dialog.

Click the indicator to open that control's own settings, at its **Driven by parent** section.

### Driven by parent

Once a dashboard has a parent control, every filter, time granularity switcher and [field switcher](#field-switcher) carries a **Driven by parent** section in its own settings, so you can see what drives a control without opening the parent — it names the driving parent, or offers only **None** when nothing drives it yet. Picking **None** detaches a driven control: the parent stops driving it, and it keeps whatever value it is showing. That holds only until the parent's settings are next saved, which drives the control again. For a detachment that survives, set the control to **Keep as-is** on every one of the parent's options — that is a mapping of its own, and [the save](#mapping-status-on-child-controls) fills in only the controls that have none.
