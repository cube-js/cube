...
Outside `between`, a `value` may still be a list: `is` and `is not` read its entries as alternatives — `f_orders.status={"value":["shipped","processing"]}` — while a substring condition matches them joined into a single term.
