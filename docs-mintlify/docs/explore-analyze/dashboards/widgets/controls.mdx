...
A time filter's value is either a fixed date or a relative one, chosen in the filter's own value picker. The **Relative** tab lists presets — **Today**, **Yesterday**, **This month**, **3 months ago** and the like — and **Custom** builds any other relative value from a number, a unit and **ago** / **from now**. The operator is the same either way.
...
A source you picked that later stops qualifying — its dimension re-pointed at another semantic view, say — is flagged in that picker.
