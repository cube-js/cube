...
`after`, `after or on`, `before`, `before or on` and `between` scope on their bounds rather than on a period. The value editor picks a whole day for each, and on a whole day the scope takes the same bound the charts do — so a window the charts return no rows for leaves the value list empty too.

<Warning>
A source can end up with nothing to scope by, leaving the value list as wide as it was while the charts narrow:

- `is not` on a fixed date, on a single-moment relative value such as **3 months ago**, or on several values at once — a point in time has no period to exclude.
