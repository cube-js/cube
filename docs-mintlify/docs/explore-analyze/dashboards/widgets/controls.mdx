...
Filters can allow either a single value or multiple values. The **Selection Mode** switch in the filter's edit sidebar sets this, and it appears on a string filter using `is`, where several values are allowed unless you turn it off. Every other combination is fixed by its own value editor: a number filter using `is` or `is not` takes several values, a time filter one value per field, and the substring conditions a single text box.
