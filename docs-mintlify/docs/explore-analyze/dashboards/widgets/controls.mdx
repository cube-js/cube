...
A faceted filter can take as a source any sibling on the same semantic view set to one of these conditions:
...
- `is not` on a fixed date or on a single-moment relative value such as **3 months ago** — a point in time has no period to exclude — or on several values at once, which the charts exclude as a list of moments rather than as a period.
- `in the month`, `in the quarter`, `in the year` and their negations on several values — each reads one period from one value.
</Warning>
