...
<Note>
A viewer's choice reaches a **PNG or PDF export** as well as a [shared link](#sharing-the-current-selection), so the exported image shows the member that was on screen rather than the control's default.

The two are not written on the same terms, and the difference is what makes the image reliable. A link carries only what the viewer *chose*, so a member resolved from a [user attribute](#field-switcher-user-attribute-default) stays out of it and the recipient re-resolves their own. An export carries **every** switcher, that one included, and a member named in the parameters outranks any attribute when the page loads — so the server-rendered copy shows what the person who asked for it was looking at, whoever the render itself runs as.

A **scheduled export** has no live viewer, so it renders the member stored on the schedule. Set it with `memberSwitchers` on the notification, alongside `filters` and `timeGrains` — see the [notifications endpoints](/api-reference/management-api) in the management API.
</Note>
...
On the **Children** tab, pick a control from **Child control**, then give each of the parent's options a value for it. Each row renders *that child's own control* — a time granularity switcher's row shows its granularity picker, limited to the granularities that switcher allows; a filter's row shows that filter's operator and value inputs; a field switcher's row shows its member picker, limited to that switcher's **Alternatives**. So the values you can offer are exactly the ones a viewer could pick in the child itself.
...
| **Reset to default** | The child is put back to its own default. For a filter that means no filtering on that dimension; for a time granularity switcher, its default granularity; for a field switcher, the member it opens on. Turn on the row's **Reset to default** switch. |
...
On a published dashboard, the values a viewer picks in the controls are reflected in the URL, so the view they are looking at is bookmarkable and shareable. Copy the address bar, send it on, and the recipient opens the dashboard on the same filters, granularities and members.
...
When a dashboard opens with these parameters, they are applied on top of whatever defaults the controls carry. A parameter is ignored when nothing on the dashboard can honor it — there is no matching control for that member, the requested granularity isn't in the switcher's [allowed granularities](#allowed-granularities), or a field switcher's value isn't one of its configured [Alternatives](#choosing-what-it-switches).
