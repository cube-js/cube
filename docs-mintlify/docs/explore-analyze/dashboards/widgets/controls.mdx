...
| Custom SQL, which no operator menu offers | `custom` — its `value` is a SQL condition, used in the query as written |
...
A substring condition takes `caseSensitive` the same way: with `true` it matches the case of the value exactly, and without the key it ignores case — `f_orders.status={"type":"not_contains","value":"ship","caseSensitive":true}`. The value editor has no switch for it, so a filter matches case only where a link sets the key.
