...
A date source scopes the value list to the window the charts are showing, and never to a narrower one, so the list never drops a value the charts still have rows for. A positive date condition scopes it to the window it names:
