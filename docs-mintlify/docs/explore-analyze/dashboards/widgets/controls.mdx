...
A time filter's value is either a fixed date or a relative one — **Today**, **Yesterday**, **This month**, `3 months ago` and the like — chosen in the filter's own value picker. The operator is the same either way.
...
For example, on a sales dashboard with a **Country** filter and a **City** filter, marking the City filter as faceted means selecting `United States` in the Country filter limits the City filter to U.S. cities only. The mark is per filter and narrows in one direction: Country scopes City without carrying the mark itself, and marking Country faceted as well is what makes the two scope each other.
...
- **A filter with no value yet.** An empty filter narrows nothing, just as it leaves the charts unfiltered — and so does one a viewer has cleared.
...
A source on a time dimension narrows by whole days, so a condition carrying a time of day scopes the list to the days it covers. A suggestion list is never narrowed past what the charts show: it can offer a value the charts have no rows for, but it never hides one they do.

#### Choosing which filters narrow this one

By default a faceted filter is narrowed by every other filter on its semantic view. To follow only some of them, open the gear beside **Facet** and pick those filters in the picker it reveals — it reads **All** for as long as nothing is picked. Closing the gear again drops the picks and puts the filter back on **All**. A filter never narrows its own list, so it isn't offered as a source for itself.
