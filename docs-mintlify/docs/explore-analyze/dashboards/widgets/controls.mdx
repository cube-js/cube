...
| **Boolean** | No operator menu — the value picker itself offers `is true` and `is false` |
...
A time filter's *value* is fixed or relative, picked on the value editor's **Fixed** and **Relative** tabs: a calendar date, a **Today**, **Yesterday**, **This week**, **This month**, **This quarter**, **This year**, **1 week ago**, **1 month ago**, **3 months ago** or **1 year ago** preset, or a custom *N periods ago* / *from now*. A `between` filter picks both bounds the same way, and its **Relative** tab offers whole windows from **Past week** to **Past 2 years**.
...
A faceted filter can take as a source any sibling the picker offers, which is a sibling whose condition Cube can turn into a scope:

- A dimension of any type set to `is` or `is not`. A **Boolean** dimension has no operator menu, and the `is true` / `is false` its value picker offers counts as `is` here.
- A time dimension set to any of its date conditions except `is null` and `is not null`. One of those is offered and still scopes nothing: `is not` on a value that is not a whole period, since a point in time has no period to exclude.
...
To pick which siblings a filter listens to, turn the **Facet** switch on in its edit sidebar and press the gear beside the switch. A sibling whose condition cannot narrow is greyed out in that list, with a tooltip saying it is not available for this semantic view or filter type.

A date source scopes the value list to the window the charts are showing. Where a Cube condition can express the charts' own bound exactly it does, and where it cannot the scope errs wider rather than tighter, so the list never drops a value the charts still have rows for. A positive date condition scopes it to the window it names:

- A value that names a whole period — a **Today**, **Yesterday**, **This week**, **This month**, **This quarter** or **This year** preset on the **Relative** tab, or an `in the month`, `in the quarter` or `in the year` condition — scopes the list to that period.
- A fixed date, or a relative value that names a single moment such as **3 months ago**, scopes it to that moment, which is what the charts match.
- `after`, `after or on`, `before` and `before or on` scope the list to everything on one side of the date, on the same bound the charts use whenever that bound is a whole day — the list stays open-ended exactly where they are.
- A `between` range, including the **Past week** to **Past 2 years** presets, scopes the list to the range's own whole-day bounds. A range the charts return no rows for therefore leaves the value list empty too, instead of offering values the charts exclude.

<Warning>
Cube reads a `before` or `after` bound as the *start* of the day it names, so an upper bound that carries a time of day is rounded up to the end of that day before it becomes a scope — never down, which would hide values the charts still show. The value list can then keep a value whose only rows fall later that day while the charts exclude it. Only a hand-written [`f_` URL parameter](#sharing-the-current-selection) produces such a bound; the value editor picks whole days, which match the charts exactly.
</Warning>

A negated date condition scopes the list the other way, to everything *outside* the period it names — `not in the month`, `not in the quarter` and `not in the year`, and `is not` on one of the whole-period values above.

<Warning>
`is not` on a time dimension is offered whatever the value, but narrows only when that value is a whole period. On a fixed date, on a single moment such as **3 months ago**, or on several values at once, it narrows nothing — a point in time has no period to exclude — so the value list stays as wide as it was while the charts narrow.
</Warning>
