...
A faceted filter is narrowed by the sibling filters set to `is` or `is not`, and by a filter on a time dimension set to any of its date conditions but `is null` and `is not null`. The rest — a substring condition, a numeric comparison, `is null` — are greyed out in the source picker, which says so in a tooltip. One combination is offered and still narrows nothing: `is not` on a time dimension set to a single moment rather than a whole period, since excluding the whole day around one instant would hide values whose rows the charts still show.

A date source scopes the value list to the rows the charts are showing, never to a wider window:

- A value that names a whole period — a **Today**, **Yesterday**, **This week**, **This month**, **This quarter** or **This year** preset on the **Relative** tab, or a condition that picks a whole month, quarter or year — scopes the list to that period.
...
- A range, including the **Past week** to **Past 2 years** presets, scopes the list to the range's own bounds. A range the charts return no rows for therefore leaves the value list empty too, instead of offering values the charts exclude.
