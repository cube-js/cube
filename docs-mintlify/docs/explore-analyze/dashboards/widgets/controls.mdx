...
- A time dimension set to any of its date conditions except `is null` and `is not null` — though `is not` and the `in the …` and `not in the …` conditions narrow only when their value is a single whole period.
...
| `between` | The range's own whole-day bounds, fixed or relative |
| `not in the month`, `not in the quarter`, `not in the year`, `is not` on a whole-period value | Everything *outside* that period |

A `between` range the charts return no rows for therefore leaves the value list empty too, instead of offering values the charts exclude.
