...
| **Boolean** | `is true` / `is false`, in one value dropdown rather than an operator picker |
...
- **Time** and **Boolean** — at most one value, of the shape each operator takes [above](#operators-by-dimension-type).
