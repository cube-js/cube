...
When multiple filters target dimensions from the same semantic view, you can mark one as **faceted**. A faceted filter scopes its value list to what its siblings are set to — selecting a value in one filter narrows the options shown in the faceted ones, so viewers only see combinations that exist in the data.
