...
A time filter's *value* is fixed or relative, picked on the value editor's **Fixed** and **Relative** tabs: a calendar date, a whole-period preset — **Today**, **Yesterday**, **This week**, **This month**, **This quarter**, **This year** — or a single-moment relative value, offered as the **1 week ago**, **1 month ago**, **3 months ago** and **1 year ago** presets and typed with any other count in the custom *N periods ago* / *from now* editor. A `between` filter picks a calendar date for each bound on its **Fixed** tab, and one whole window — **Past week** to **Past 2 years** — on its **Relative** tab. Neither tab sets more than one value, and the [**Selection Mode**](#single-vs-multiple-selection) switch is not offered for a time dimension, so a time filter holds *several values* only when a hand-written [URL parameter](#sharing-the-current-selection) passes a list in its `value` — `f_orders.created_at={"type":"equals","value":["2026-01-01","2026-02-01"]}`.

### Single vs. multiple selection

Filters can allow either a single value or multiple values. The filter's edit sidebar sets this under **Selection Mode**, on a switch labelled **Enable multiple values**: it is offered on a string filter using `is`, where several values are allowed unless you turn it off, and what you set there also governs the same filter on `is not`. No other combination has a switch — each takes the shape its own value editor gives it: several values for a number filter using `is` or `is not`, one value for the comparisons on a **Number** dimension (`greater than`, `less than` and their *or equal* forms), one value per field for a time filter, a single text box for the substring conditions, the one value its `is true` / `is false` picker sets for a boolean filter, and no value at all for `is null`, `is not null`, `is empty` and `is not empty`.
