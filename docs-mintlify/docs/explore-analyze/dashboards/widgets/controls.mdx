...
|---|---|---|
| **Clear** (✕) | The control has a value, and is not a [field switcher](#field-switcher) — that one carries no ✕. | Empties the control and stops applying it, so its charts fall back to their own defaults. Clearing a [parent control](#parent) clears the [children](#children) it drives as well — fully, rather than returning them to their defaults. |
...
Only one shows in the control at a time: **Reset to default** takes the slot whenever it applies, so a control that has drifted from its published value offers the way back rather than the way to empty. The dropdown header is not limited that way — whichever of the two apply are there as text links, **Reset** to the left of **Clear** — and neither closes the dropdown. Both are greyed out along with the control when its [visibility](#visibility) is **Disabled**.

On a published dashboard, clearing or resetting changes only what that viewer sees, and writes nothing back to the dashboard. [Neither action travels in the URL](#sharing-the-current-selection), so a reload reopens the control on whatever it opens on for that viewer.
