...
- **A filter on another semantic view.** Narrowing a value list means querying this filter's own view, so only filters on that view qualify. The rest are greyed out in the [source picker](#choosing-which-filters-narrow-this-one). Within the view every condition the operator picker offers on a dimension can narrow, so a filter greys out only when the dimension it names has left the data model.
