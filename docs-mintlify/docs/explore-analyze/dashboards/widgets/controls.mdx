...
|---|---|---|
| **Clear** (✕) | The control has a value. | Empties the control and stops applying it, so its charts fall back to their own defaults. Clearing a [parent control](#parent) clears the [children](#children) it drives as well — fully, rather than returning them to their defaults. |
...
A [field switcher](#field-switcher) never offers **Clear**. Its selection is obligatory — with no member picked, charts fall back to [the member it replaces](#field-switcher-default-option) — so there is no empty state for a viewer to choose, and **Reset to default** is the only action it carries.
...
On a published dashboard, clearing or resetting changes only what that viewer sees for the rest of the session — nothing is written back to the dashboard.

In the [dashboard builder][ref-workbooks] both write the draft. **Reset to default** returns the control to the published value, leaving no unsaved change behind. **Clear** removes the control's saved default, which viewers see once you publish; clearing a [parent control](#parent) saves its children's cleared defaults in the same write.

Until a control has been published with a value — on a dashboard still being built, or one published while the control was empty — there is nothing to return to, and the ↩ doesn't appear at all.

A clear also doesn't survive the [shareable link](#sharing-the-current-selection). Clearing drops the control's parameter rather than recording the empty state, so an emptied control looks the same in the URL as one nobody touched, and a bookmark or shared link reopens it on the default the clear was there to remove. Reset drops the parameter too. That reproduces the same state for a viewer whose control opens on the published value, but not for one seeded by a [user attribute default](#user-attribute-default): there the reload reopens on the attribute value, not the published one the reset landed on.
