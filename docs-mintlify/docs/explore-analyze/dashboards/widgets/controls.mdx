...
- `is`, `in the month`, `in the quarter` and `in the year` scope the list to what their value names: the whole period for an `in the …` condition or a whole-period preset, and the moment itself — what the charts match — for a fixed date or a single-moment preset. Under `is`, several values scope the list to all of their windows.
