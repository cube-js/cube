...
| **Boolean** | `is`, against a `true` / `false` value. With a single condition to choose from, a boolean filter shows no operator picker. |
...
Turn it on with **Enable facet** in the filter's settings. Faceting changes only the values this filter *suggests*; which rows the charts show is decided by the filters themselves, exactly as it is without it.

#### What a source filter contributes

Every condition a filter can be set to narrows a faceted filter — all of the [operators above](#operators-by-dimension-type): the substring conditions, the numeric comparisons, `between`, the date and period conditions, and `is null` / `is not null` / `is empty` / `is not empty` alongside `is` and `is not`.

Two cases contribute nothing:

- **A filter with no value yet.** A source is read on the same rule the charts read it by, so a filter nobody has filled in — or one a viewer has cleared — leaves the suggestions alone, just as it leaves the charts unfiltered.
...
A source you picked that later stops qualifying — its dimension re-pointed at another semantic view, say — is flagged in that picker, rather than quietly dropping out of the arrangement.
