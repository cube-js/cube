...
The user attribute default only seeds the filter's *initial* value. Viewers can still change the filter unless its [visibility](#visibility) is set to **Disabled**, in which case the resolved attribute value is locked against their own edits. A [parent control](#parent) still drives a Disabled child — that is the point of hiding or disabling children whose individual values aren't worth showing. Values passed via URL parameters also take precedence over user attribute defaults, so deep links continue to work.
...
| **Reset to default** | The child is put back to the value it opens on *for that viewer*. When the child takes its default from a **User attribute default** — a [filter](#user-attribute-default)'s, a [time granularity switcher](#time-granularity-user-attribute-default)'s, or a [field switcher](#field-switcher-user-attribute-default)'s — that's the attribute value; otherwise it's the child's own static default: the filter's saved value, a time granularity switcher's [default granularity](#default-granularity), a field switcher's [default option](#field-switcher-default-option) — and no filtering on that dimension for a filter that has no saved value. |
...
A control can be driven by only one parent control at a time. Mapping a child that another parent already drives **moves** it rather than sharing it — the row names the parent that currently owns it, on any row this parent would drive.
...
Saving the parent's settings writes a mapping for each control that doesn't have one yet, the tab's [displayed default](#children) included — so a parent that still has only its first option drives *every* control on the dashboard the moment you save it, each reading **Fully driven** before you have mapped anything by hand. Adding the second option moves them the other way: it leaves untouched controls as-is, so those same controls read **Partly driven** until that option drives them too.

This reaches controls another parent already drove. [Only one parent drives a control](#one-parent-per-child), so saving a second parent takes them over, leaving the first driving only the controls you set to **Keep as-is** on every one of the second parent's options — those get no mapping written, so they are never claimed. The [owner note](#one-parent-per-child) reaches those rows too: it appears on every row except the ones reading **Keep as-is**, and an untouched row on the first option reads **Reset to default**, so you are told before a save claims a control you never touched. On a dashboard with more than one parent control, map the one you are editing deliberately rather than saving it to dismiss the dialog.

Click the indicator to open that control's own settings, at its **Driven by parent** section.

### Driven by parent

Once a dashboard has a parent control, every filter, time granularity switcher and [field switcher](#field-switcher) carries a **Driven by parent** section in its own settings, so you can see what drives a control without opening the parent — it names the driving parent, or offers only **None** when nothing drives it yet. Picking **None** detaches a driven control: the parent stops driving it, and it keeps whatever value it is showing.
...
If you never pick an option, the parent opens with nothing selected and the children use their own defaults — that is the load-time case; [clearing the parent](#clearing-and-resetting) later leaves the children where they are. Deleting the option that was serving as the default clears it, and the parent goes back to opening on nothing.
