...
- A dimension of any type set to `is` or `is not`, including the `is true` / `is false` a **Boolean** dimension's value picker offers.
- A time dimension set to any of its date conditions except `is null` and `is not null` — though `is not` does not narrow on every value it accepts.
...
A date source scopes the value list to the window the charts are showing, and never to a narrower one, so the list never drops a value the charts still have rows for. Which window a positive date condition names depends on the condition:

- `is` scopes the list to what its value names — a whole period for a **Today**, **Yesterday**, **This week**, **This month**, **This quarter** or **This year** preset on the **Relative** tab, and the moment itself for a fixed date or a relative value such as **3 months ago**, which is what the charts match. Several values at once scope the list to all of their windows.
- `in the month`, `in the quarter` and `in the year` scope the list to the whole period they name.
- `after`, `after or on`, `before` and `before or on` scope the list to everything on one side of the single date their value resolves to — for a whole-period preset, the day that period starts on, which is the date the charts compare against too. Every bound the value editor picks is a whole day, and on a whole day the scope uses the same bound the charts do, so the list stays open-ended exactly where they are.
