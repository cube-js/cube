...
A [field switcher](#field-switcher) is reset-only. It always has a member selected, and clearing it would leave its charts with no member to query, so it offers **Reset to default** and no ✕.
...
On a published dashboard, clearing or resetting changes only what that viewer sees for the rest of the session — nothing is written back to the dashboard. In the [dashboard builder][ref-workbooks], **Reset to default** also restores the draft to the published value, so the control comes back to a saved state rather than leaving an unsaved change behind. Until a control has been published with a value — on a dashboard still being built, or one published while the control was empty — there is nothing to return to, and the ↩ doesn't appear at all.
