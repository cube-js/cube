...
`is`, `is not`, `after`, `after or on`, `before` and `before or on` each take a single value, either a fixed date or a relative one, chosen in the filter's own value picker. The **Relative** tab lists presets of two kinds: period values — **Today**, **Yesterday**, **This week**, **This month**, **This quarter**, **This year** — each standing for the whole day, week, month, quarter or year it names, and single moments — **1 week ago**, **1 month ago**, **3 months ago**, **1 year ago**. **Custom** builds any other single moment from a number, a unit and **ago** / **from now**. The operator is the same either way. `between` takes a fixed or relative *range*, with six presets of its own: **Past week**, **Past month**, **Past 3 months**, **Past 6 months**, **Past year** and **Past 2 years**. The `in the …` operators and their `not in the …` negations take neither — their value is one whole period, chosen in a month, quarter or year picker of their own and shown as **July 2025**, **Q3 2025** or **2025**, with no fixed-or-relative choice and no presets.

A boolean filter takes a `true` / `false` value and shows no operator picker, since `is` is the only condition to choose from.

### Single vs. multiple selection

Filters can allow either a single value or multiple values. Configure this when adding or editing the filter — multi-select is the default for string dimensions, while time and number dimensions default to a single value. A boolean filter is always single-value.
...
A faceted filter reads every condition its sources can be set to — all of the [operators above](#operators-by-dimension-type) — except `is not` on a time dimension set to a single moment.
...
- **`is not` on a time dimension, set to a single moment.** A period value — **Today**, **Yesterday**, **This week**, **This month**, **This quarter**, **This year** — narrows as expected. Any other value names one moment rather than a period — a fixed date, or a relative value such as **3 months ago** — and leaves the list alone rather than dropping the whole day it falls in.

A source on a time dimension narrows by whole days, rounded outward, so a condition carrying a time of day scopes the list to every day it touches. A suggestion list is never narrowed past what the charts show: it can offer a value the charts have no rows for, but it never hides one they do.
