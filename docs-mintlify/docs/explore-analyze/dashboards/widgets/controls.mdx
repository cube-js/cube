...
A faceted filter is narrowed by the sibling filters set to `is` or `is not`, and by a filter on a time dimension set to any of its date conditions but `is null` and `is not null`. Every other condition — the substring conditions, the numeric comparisons (`greater than`, `less than` and their *or equal* forms), and `is null`, `is not null`, `is empty` and `is not empty` — narrows nothing and cannot be picked as a source. You meet that in the filter's edit sidebar: turn the **Facet** switch on, press the gear beside it to open the list of source filters, and a sibling whose condition cannot narrow is greyed out there, with a tooltip saying it is not available for this semantic view or filter type. One combination is offered and still narrows nothing: `is not` on a time dimension whose value is not a whole period — a single moment, or several of them — since a point in time has no period to exclude.

A date source scopes the value list to the rows the charts are showing, never to a wider window:
