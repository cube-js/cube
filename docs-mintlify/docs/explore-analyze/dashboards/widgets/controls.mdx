...
Filters can allow either a single value or multiple values. The filter's edit sidebar sets this under **Selection Mode**, on a switch labelled **Enable multiple values** — offered on a string filter using `is` or `is not`, and on no other combination. Every other filter takes the shape its own value editor gives it:
...
| **Time** on any condition except `is null` and `is not null` | One per field |
...
A `between` filter's `value` is a two-entry array, and both bounds are inclusive unless you set `startInclusive` or `endInclusive` to `false`.
