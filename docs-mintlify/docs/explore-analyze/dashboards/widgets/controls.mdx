...
- `after`, `after or on`, `before` and `before or on` scope the list to everything on one side of the date, on the same whole-day bound the charts use, so the list stays open-ended exactly where they are.
