...
A reset that resolved its value from *your* [user attribute](#user-attribute-default) saves the child empty whatever it had, so your own value is never pinned onto everyone — each viewer re-seeds from their own attribute.
