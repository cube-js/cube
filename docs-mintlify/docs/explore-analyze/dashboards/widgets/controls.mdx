...
A [field switcher](#field-switcher) never offers **Clear** — there is no ✕, so **Reset to default** is the only action it can carry.

On a published dashboard, clearing or resetting changes only what that viewer sees, and writes nothing back to the dashboard. [Neither action travels in the URL](#sharing-the-current-selection), so a reload reopens the control on whatever it opens on for that viewer: a clear is always undone, while a reset survives unless the control is seeded by a [user attribute default](#user-attribute-default).
