...
In the [dashboard builder][ref-workbooks], open the **Add Controls** menu in the toolbar and choose **Parent**, then click **Configure Parent** to set it up. The editor gives each option its own tab, and lists every other control on the dashboard inside it — so one tab is one complete arrangement, and you can read off what an option does to the whole row at a glance.

<Info>
Add the child controls to the dashboard *before* the parent control. A tab lists only the controls that already exist, so a parent added to an empty dashboard has nothing to drive yet.
</Info>

### Options

Each option is a tab across the top of the editor. Add one with **+** at the end of the tab strip, rename a tab in place by editing its title, and delete one with the ✕ that appears on the tab. A parent control created without options opens on a single tab named **Default**. A parent control can hold up to 50 options.
...
Open an option's tab and you see every other control on the dashboard, each with a dropdown saying what that option does to it. You set one option's whole arrangement in one place, rather than configuring one child at a time.
