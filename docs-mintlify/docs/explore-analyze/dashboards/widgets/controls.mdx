...
A faceted filter reads every condition its sources can be set to — all of the [operators above](#operators-by-dimension-type), with one exception: the last of the three cases below.

A source contributes nothing in three cases:
...
- **`is not` on a time dimension, set to a single moment.** A period value — **Today**, **Yesterday**, **This week**, **This month**, **This quarter**, **This year** — narrows as expected, because the charts exclude the whole period too. A fixed date, or a relative value such as **3 months ago** that names one moment rather than a period, does not: the charts exclude only the rows at that instant, and a list that dropped the whole day would hide values those charts still show.
...
<Warning>
Closing the gear empties the picks, and that includes a list saved earlier: the gear opens on a saved list, so closing it there drops it and the next **Save** puts the filter back on **All** for every viewer. Leaving the settings with **Cancel** restores the saved list, so that is the way out if you have already closed the gear — otherwise leave it open while you are reviewing a saved list.
</Warning>
