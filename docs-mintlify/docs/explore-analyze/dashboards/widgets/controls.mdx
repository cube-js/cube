...
| `between` | The range's own bounds, fixed or relative — whole days as the value editor picks them, and on a whole day the same bound the charts use, so a range they return no rows for leaves the value list empty too |
