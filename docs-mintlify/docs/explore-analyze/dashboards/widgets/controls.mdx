...
<Info>
A tab lists the controls that are on the dashboard, so a parent added to an empty one has nothing to drive yet; a control added later simply appears in the tabs, on **Keep as-is** in each one after the first and on **Reset to default** in that one, like every other control there. Another parent control is never listed — a parent cannot be a child of one.
</Info>

### Options

Each option is a tab across the top of the editor. Add one with **+** at the end of the tab strip, rename a tab in place by editing its title, and delete one with the ✕ that appears on it when you hover — except the first tab, which anchors the dropdown and cannot be deleted. A parent control created without options opens on a single tab named **Default** — a real option, saved with the control and offered in the viewer's dropdown like any other, so rename it if that label isn't what viewers should see. A parent control can hold up to 50 options.
...
Each control on the tab carries a dropdown saying what that option does to it. Every option/child pair is in one of four states:
...
| **Not driven** | No parent control drives this one. |

**Set to**, **Clear** and **Reset to default** all count as driving — each is something the option does to the control. Only **Keep as-is** doesn't, which is why a control every option keeps as-is reads **Not driven**, and why a control left untouched on a two-option parent reads **Partly driven**: the first tab's **Reset to default** drives it, the second tab's **Keep as-is** does not.

Saving the parent's settings writes the first tab's untouched cells as the **Reset to default** they show, so a parent that still has only its first tab drives *every* control on the dashboard the moment you save it — each one reads **Fully driven**, before you have mapped anything by hand. Add the options you mean to offer, and each later tab leaves the controls it doesn't set as-is.

Click the indicator to open that control's own settings, at its **Driven by parent** section.

### Driven by parent

Once a dashboard has a parent control, every filter, time granularity switcher and [field switcher](#field-switcher) carries a **Driven by parent** section in its own settings, naming what drives it without your having to open the parent.

The dropdown offers the parent that currently drives the control, and **None**. Picking **None** detaches it: the parent stops driving it, and it keeps whatever value it is showing.

Mapping is the one thing you cannot do here — the values belong to the parent's options, so a control nothing drives yet shows only **None**. To map it, or to hand it to a different parent, open that parent's settings and set the control's row on each option's tab.
...
|---|---|---|
| **Clear** (✕) | The control has a value, and is not a [field switcher](#field-switcher) — that one carries no ✕. | Stops applying the control: a cleared filter leaves its charts unfiltered, and a cleared switcher hands each chart back to the granularity or member it was built with. Clearing a [parent control](#parent) only deselects it: the parent stops driving, and each child keeps the value it is showing. |
...
- **Neither a clear nor a reset.** [Clearing or resetting](#clearing-and-resetting) a control drops its parameter rather than recording the result, so the control is indistinguishable in the link from one nobody touched and the recipient opens it on whatever it opens on for them.
