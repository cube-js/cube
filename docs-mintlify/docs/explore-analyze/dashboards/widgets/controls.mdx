...
A date source never scopes the value list to less than the window the charts are showing, so the list never drops a value the charts still have rows for. Every bound the value editor picks is a whole day, and on a whole day the scope takes the same bound the charts do — so a `between` range they return no rows for leaves the value list empty too. Which window it scopes to depends on the condition:
...
| `after`, `after or on`, `before`, `before or on` | Everything on one side of the single whole day the value resolves to — for a whole-period preset, the day that period starts on |
| `between` | The range itself, between its own bounds — whole days, fixed or relative, as the value editor picks them |
