...
- **Neither a clear nor a reset.** [Clearing or resetting](#clearing-and-resetting) a control drops its parameter rather than recording the result, so the control is indistinguishable in the link from one nobody touched and the recipient opens it on whatever it opens on for them. This is also the one [parent mapping](#children) that doesn't travel: where **Set to**, **Reset to default** and **Keep as-is** leave the child holding a value the link can carry, an option that **Clear**s a child leaves it empty — so the recipient's copy of that control opens on its own default rather than cleared.
