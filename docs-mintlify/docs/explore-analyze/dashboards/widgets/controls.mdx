...
<Note>
A field switcher's selection travels wherever the other controls' values do. A [shared link](#sharing-the-current-selection) and an exported PNG or PDF both carry the member the viewer was looking at, so the image matches the screen it came from. A scheduled export has no live viewer, so it renders the member stored on the schedule — set it through the `memberSwitchers` field on the notification, alongside `filters` and `timeGrains`.
</Note>
