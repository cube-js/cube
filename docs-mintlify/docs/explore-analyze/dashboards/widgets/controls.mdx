...
<Info>
A tab lists the controls that are on the dashboard, so a parent added to an empty one has nothing to drive yet; a control added later simply appears in every tab on **Keep as-is**. Another parent control is never listed — a parent cannot be a child of one.
</Info>
