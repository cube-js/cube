...
- `not contains`, `not starts with` or `not ends with` marked case-sensitive, again only reachable through that parameter — the value list can be narrowed case-insensitively alone, which would drop values the charts still show.
</Warning>
