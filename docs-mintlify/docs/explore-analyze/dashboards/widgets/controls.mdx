...
A faceted filter can take as a source any sibling on the same semantic view, whatever condition that sibling is set to and whether or not it carries **Facet** itself: `is` and `is not`, the substring conditions, the comparisons on a **Number** dimension and every date condition all narrow the value list, and a **Boolean** dimension counts here too, its `is true` / `is false` setting `is`. The four conditions that carry no value — `is null`, `is not null`, `is empty` and `is not empty` — narrow as soon as they are set; every other condition waits for a value, and until it has one it leaves the value list alone, just as it leaves the charts unfiltered. Custom SQL is the exception no source can be set to: no operator menu offers it, and a filter a hand-written [URL parameter](#sharing-the-current-selection) gives it scopes nothing.
