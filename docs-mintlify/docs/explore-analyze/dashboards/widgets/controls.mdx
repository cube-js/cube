...
A mapping is written when you save the parent's settings, [defaults included](#children) — so a parent still on its first tab drives *every* control on the dashboard the moment you save it, each reading **Fully driven** before you have mapped anything by hand.

Click the indicator to open that control's own settings, at its **Driven by parent** section — which names the parent for a driven control, and offers only **None** for one nothing drives yet.

### Driven by parent

Once a dashboard has a parent control, every filter, time granularity switcher and [field switcher](#field-switcher) carries a **Driven by parent** section in its own settings, so you can see what drives a control without opening the parent. Picking **None** there detaches it: the parent stops driving it, and it keeps whatever value it is showing.
