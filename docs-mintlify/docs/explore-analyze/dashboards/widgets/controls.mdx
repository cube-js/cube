...
Every condition a filter can be set to narrows a faceted filter — all of the [operators above](#operators-by-dimension-type).
...
- **A filter on another semantic view.** Narrowing a value list means querying this filter's own view, so only filters on that view qualify. The rest are greyed out in the source picker below.
...
By default a faceted filter is narrowed by every other filter on its semantic view. To follow only some of them, open the gear beside **Facet** and pick those filters in the picker it reveals — it reads **All** for as long as nothing is picked. Closing the gear discards the list — including one saved earlier, which the gear opens on — so saving after that puts the filter back on **All**. A filter never narrows its own list, so it isn't offered as a source for itself.
