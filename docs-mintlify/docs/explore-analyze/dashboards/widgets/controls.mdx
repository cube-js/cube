...
| `between` | The range's own whole-day bounds, fixed or relative — the same bounds the charts use, so a range they return no rows for leaves the value list empty too |
