...
- **Time** — one value per operator: a date for `is`, `is not`, `after`, `after or on`, `before` and `before or on`, a range for `between`, a whole period for the `in the …` operators.
- **Boolean** — the single `true` / `false` described above.
