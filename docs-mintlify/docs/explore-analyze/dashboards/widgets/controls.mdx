...
A control can be driven by only one parent control at a time. Mapping a child that another parent already drives **moves** it rather than sharing it — the row says so as soon as you map it, naming the parent that currently owns it.
