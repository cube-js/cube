...
Where a time operator takes a single value, that value is either a fixed date or a relative one, chosen in the filter's own value picker. The **Relative** tab lists presets — **Today**, **Yesterday**, **This month**, **3 months ago** and the like — and **Custom** builds any other relative value from a number, a unit and **ago** / **from now**. The operator is the same either way. `between` takes a fixed or relative *range* and offers its own presets, **Past week** through **Past 2 years**; `is null` and `is not null` take no value at all.
