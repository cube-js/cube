...
- A time dimension set to any of its date conditions except `is null` and `is not null` — though `is not` and the `in the …` and `not in the …` conditions each have values they cannot narrow on, which the warning below lists.
...
| `between` | The range's own bounds, fixed or relative. The value editor picks whole days, and on a whole day the scope takes the same bound the charts do — so a range the charts return no rows for leaves the value list empty too |
