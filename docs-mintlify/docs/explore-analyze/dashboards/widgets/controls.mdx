...
|---|---|---|
| **Clear** (✕) | The control has a value | Empties the control and stops applying it, so its charts fall back to their own defaults. Clearing a [parent control](#parent) also clears the [children](#children) it drives |
| **Reset to default** (↩) | The control's value differs from the one published in the last dashboard version | Puts the control back on that published value |

Only one shows at a time: **Reset to default** takes the slot whenever it applies, so a control that has drifted from its published value offers the way back rather than the way to empty. Both are also text links in the control's dropdown header — **Reset** to the left of **Clear** — and neither closes the dropdown.
...
<Note>
This is the control's *own* published value, which is not always what a [parent control](#parent) means by its **Reset to default** [child mapping](#children): that one returns the child to the value it opens on *for that viewer*, which is the viewer's [user attribute default](#user-attribute-default) when one resolves.
</Note>

On a published dashboard, clearing or resetting changes only what that viewer sees for the rest of the session — nothing is written back to the dashboard. In the [dashboard builder][ref-workbooks], **Reset to default** also restores the draft to the published value, so the control comes back to a saved state rather than leaving an unsaved change behind.
