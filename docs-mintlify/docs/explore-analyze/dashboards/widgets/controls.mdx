...
In the [dashboard builder][ref-workbooks], open the **Add Controls** menu in the toolbar and choose **Parent**, then click **Configure Parent** to set it up. The editor is a strip of tabs — one per option — with the mapping for the selected option listed below it.

### Options

Each tab is one entry in the viewer-facing dropdown. The first tab, **Default**, is the option the control opens on; double-click to rename it, but it can't be deleted, so the dropdown always keeps at least one entry. Click **+ Add view** to add another option — up to 50 in total — and rename it the same way. Remove any option but Default with the **✕** on its tab.
...
### Setting values per option

<Info>
Add the child controls to the dashboard *before* the parent control. A tab lists only controls that already exist, so a parent added to an empty dashboard has nothing to map yet.
</Info>

Select a tab to edit that option's mapping. Below the tabs, the editor lists **every** filter, time granularity switcher and [field switcher](#field-switcher) on the dashboard, each with a dropdown of up to four states:

| State | What happens when the viewer picks that option |
|---|---|
| **Set to** | Pick a value below the dropdown — a time granularity switcher's row shows its granularity picker, limited to the granularities that switcher allows; a filter's row shows that filter's operator and value inputs; a field switcher's row shows its member picker, limited to the members that switcher offers — its **Alternatives**, plus the replaced member. So the values you can offer are exactly the ones a viewer could pick in the child itself. |
| **Clear** | The child is left with no value — no filtering on that dimension for a filter, the chart's own default granularity for a time granularity switcher. Not offered for field switchers, which always resolve to some member. |
...
On the **Default** tab, a control you haven't set explicitly defaults to **Reset to default** — so picking Default resets every control on the board unless you set specific ones to **Keep as-is**. On every other tab, an unset control defaults to **Keep as-is** instead, so that option only touches what you explicitly configure for it.

While the parent's settings are open, the children with a value on any option are highlighted on the canvas, so you can see the scope of the mapping at a glance.

### One parent per child

A control can be driven by only one parent control at a time. Setting any value other than **Keep as-is** for a child that another parent already drives **takes it over** rather than sharing it — a tag next to that row names the parent that currently owns it, and the takeover happens when you save.
...
Click the indicator to open that control's own settings. There, a **Driven by parent** field names the parent driving it (or **None**) — pick **None** to detach it. To change what an option sets for this control, edit the mapping in the owning parent's own settings; the per-option values live there, not on the child.
...
<Steps>
 ...
 <Step title="Enable User attribute default">
   At the bottom of the settings, above **Visibility**, turn on the **User attribute default** switch.
 </Step>
 <Step title="Pick the attribute">
   Select the [user attribute][ref-user-attributes] to resolve. Only attributes defined in your account appear in the picker.
 </Step>
...
|---|---|---|
| **Clear** (✕) | The control has a value, and is not a [field switcher](#field-switcher) — that one carries no ✕. | Stops applying the control: a cleared filter leaves its charts unfiltered, and a cleared switcher hands each chart back to the granularity or member it was built with. Clearing a [parent control](#parent) clears the [children it drives](#setting-values-per-option) as well — fully, rather than returning them to their defaults. |
...
<Note>
This is the control's *own* published value, which is not always what a [parent control](#parent) means by its **Reset to default** [child mapping](#setting-values-per-option): despite the shared label, that one returns the child to the value it opens on *for that viewer*.
</Note>
