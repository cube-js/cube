...
<Note>
A viewer's choice travels in a [shared link](#sharing-the-current-selection), but not in a scheduled export or a screenshot — those render the control's default member.
</Note>
