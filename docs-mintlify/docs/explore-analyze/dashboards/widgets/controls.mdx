...
| **Reset to default** | The child is put back to the value it opens on *for that viewer*. When the child takes its default from a **User attribute default** — a [filter](#user-attribute-default)'s, a [time granularity switcher](#time-granularity-user-attribute-default)'s, or a [field switcher](#field-switcher-user-attribute-default)'s — that's the attribute value; otherwise it's the child's own static default: the filter's, a time granularity switcher's [default granularity](#default-granularity), a field switcher's [default option](#field-switcher-default-option) — and where the child has none, the reset leaves it alone: no filtering on that dimension for a filter, and each chart on its own saved granularity for a time granularity switcher. |
...
A control can be driven by only one parent control at a time. Setting a child another parent already drives to **Set to**, **Clear** or **Reset to default** **moves** it rather than sharing it — the row names the parent that currently owns it, on any row this parent would drive. **Keep as-is** doesn't move it — it's the one state that [doesn't drive](#mapping-status-on-child-controls).
...
One exception follows from the same rule: the save only fills in controls with no mapping, and a control you set to **Keep as-is** on every one of the *second* parent's options has one. The save leaves it alone, and **Keep as-is** is not a claim, so the first parent keeps driving it.

The [owner note](#one-parent-per-child) reaches the rows a save would claim: it appears on every row except the ones reading **Keep as-is**, and an untouched row on the first option reads **Reset to default**, so you are told before a save claims a control you never touched. On a dashboard with more than one parent control, map the one you are editing deliberately rather than saving it to dismiss the dialog.
...
Picking in the builder also applies that option to the children and saves the result, so a published dashboard opens in a consistent state. What each state saves differs: **Set to** writes its value as the child's default, **Clear** writes the child empty — which discards the default it had — and **Reset to default** writes the child's own default back. A reset that resolved the value from *your* [user attribute](#user-attribute-default) saves no value at all, so picking in the builder never pins your own attribute value onto every viewer. **Keep as-is** writes nothing, so that child keeps whatever default it already had and is not part of the arrangement you picked.
