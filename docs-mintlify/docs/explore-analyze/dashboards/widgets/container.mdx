...
A widget dropped into a container keeps the size it had. If it doesn't fit where you drop it — because that spot is near an edge or already occupied — it shrinks and settles into the nearest free space rather than overlapping another widget. You can resize and rearrange widgets inside a container afterwards like any other widget.
