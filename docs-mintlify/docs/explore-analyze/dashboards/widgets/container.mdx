...
A widget dropped into a container keeps the size it had, shrinking to fit only when there isn't enough free room at that spot. Dropping a widget onto a cell that's already taken swaps the two. You can resize and rearrange widgets inside a container afterwards like any other widget.
