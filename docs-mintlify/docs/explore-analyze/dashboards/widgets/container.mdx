...
<Steps>
  <Step title="Add the container">
    Open the **Add Widgets** menu in the toolbar and choose **Grid**. The container is added with an empty inner grid, and its settings panel opens so you can size the grid right away. You can instead drag the **Grid** entry to drop the container exactly where you want it.
  </Step>
  ...
  <Step title="Add widgets into it">
    Fill the container with charts, controls, text, and other widgets — see [Adding widgets to a container](#adding-widgets-to-a-container) below for the ways to do that.
  </Step>
</Steps>
...
- **Move a widget in** — add the widget to the dashboard first (open **Add Widgets** or **Add Controls** and click the type you want, or pick a report from the **Charts** picker), then drag it into the container. Clicking a toolbar item always adds the widget to the dashboard itself, not to a container.
- **Straight from the toolbar** — drag a widget from the **Add Widgets** or **Add Controls** menu, or a report from the **Charts** picker, and drop it directly onto the container in one step, skipping the dashboard.
