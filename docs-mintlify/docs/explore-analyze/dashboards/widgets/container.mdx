...
A widget dropped into a container keeps the size it had, shrinking to fit only when the spot you drop it on doesn't have room. Dragging a widget **already inside** the container onto another child **swaps the two** — they trade places, each shrinking to fit the other's spot if needed. A widget brought in **from outside** never displaces a child: if you drop it on a cell that's already taken, it lands in the nearest free space instead. You can resize and rearrange widgets inside a container afterwards like any other widget.
