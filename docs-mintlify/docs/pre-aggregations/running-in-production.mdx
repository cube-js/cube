...
Cube Store supports Azure Blob Storage as persistent storage. Credentials are
resolved in the following order:

- `CUBESTORE_AZURE_ACCESS_KEY` (storage account key)
- `CUBESTORE_AZURE_SAS_TOKEN` (shared access signature)
- Workload identity, using the standard `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`,
  and `AZURE_FEDERATED_TOKEN_FILE` environment variables (for example, [Azure
  AD Workload Identity](https://learn.microsoft.com/en-us/azure/aks/workload-identity-overview)
  on AKS)
- Managed identity of the host, retrieved from the Azure Instance Metadata
  Service (IMDS)

This means that on Azure you can skip defining `CUBESTORE_AZURE_ACCESS_KEY`
and `CUBESTORE_AZURE_SAS_TOKEN` and rely on workload or managed identity
instead. Token renewal is handled automatically.
