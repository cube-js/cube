...
[`CUBEJS_PRE_AGGREGATIONS_DB_SQL_PREAMBLE`](/reference/configuration/environment-variables#cubejs_db_sql_preamble)
is the one variable that does inherit: when it is unset, builds run the regular
[`CUBEJS_DB_SQL_PREAMBLE`](/reference/configuration/environment-variables#cubejs_db_sql_preamble).
A preamble usually defines a function the data model's SQL calls, and a build that
silently ran without it would fail on the missing function rather than fall back to
anything sensible. Set it to an empty value to run builds with no preamble.

It also configures session setup rather than a connection target, so — like the
variables below — setting it alone does not switch pre-aggregations onto a dedicated
connection.
