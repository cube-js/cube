...
Sheets chat pane, or an MCP client to delete a folder is narrower than deleting it
here: it only removes the folder itself — contents move to the workspace root instead
of being deleted — and it refuses a folder that still has subfolders, rather than
