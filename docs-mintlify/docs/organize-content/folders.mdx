...
subfolders, rather than deleting the whole subtree. It's not narrower in who can do
it: this path only checks your access to the folder itself, not to what's inside, so
it doesn't apply the content-permission check above — access to a folder is enough to
scatter its contents to the workspace root, even contents you don't otherwise have
access to.
