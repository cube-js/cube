...
Sheets chat pane, or an MCP client to delete a folder is narrower in what it destroys
than deleting it here: it only removes the folder itself — contents move to the
workspace root instead of being deleted — and it refuses a folder that still has
subfolders, rather than deleting the whole subtree.

<Warning>

It's not narrower in who can do it. This path only checks that you have **Full
access** to the folder itself — it never applies the content-permission check
above. Full access to a folder alone is enough to scatter its contents to the
workspace root, even contents you don't otherwise have access to.
