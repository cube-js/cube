...
facts are stitched together at the grain you group by. Joining on
`DATE_TRUNC('month', …)` while grouping by `DATE_TRUNC('day', …)` is not merged
(it would silently stitch at day grain, diverging from the month-grain join) and
