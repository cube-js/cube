...
```yamltitle="model/macros/common_dimensions.jinja"
{%- macro dimension(column_name, type='string', primary_key=False) -%}
      - name: {{ column_name }}
        sql: {{ column_name }}
        type: {{ type }}
...
```yamltitle="model/cubes/orders.yml"
...
[literal string][ref-yaml-literal] (`|-`) rather than inline. A SQL expression
is arbitrary text, and inline it has to avoid everything YAML reads as syntax:
`{CUBE}.amount` starts a flow mapping, `amount # note` truncates at the
comment, and wrapping the whole thing in double quotes only moves the problem
to expressions that contain one, such as `{CUBE}."amount"`. The literal form
takes all of them, and — unlike the [folded form][ref-yaml-folded] (`>-`),
which folds newlines into spaces — it keeps line breaks, so a leading `--`
comment stays on its own line instead of swallowing the expression that
follows it.
