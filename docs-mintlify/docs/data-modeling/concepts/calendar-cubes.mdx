...
A granularity defined with `sql` must be named after one of the default granularities:
`second`, `minute`, `hour`, `day`, `week`, `month`, `quarter`, or `year`. So, a 4-5-4
retail month is defined as an override of `month` rather than under a name of its own.
[Custom granularities][ref-granularities] with names of their own are defined with
`interval` instead.
