...
Cube Core's development mode — which is on only when
[`CUBEJS_DEV_MODE=true`](/reference/configuration/environment-variables#cubejs_dev_mode)
— and which is an **authentication bypass**. It switches off JWT verification on the
data APIs, and Playground's endpoints are served with no authentication at all, so
