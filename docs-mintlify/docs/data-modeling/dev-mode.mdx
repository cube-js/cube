...
description: Outlines development mode in Cube Cloud—branch-scoped APIs, save and commit flows, and safe iteration before promoting changes to production.
---
...
An [embed session][ref-generate-session] can point at your branch as well: pass its
name as `branchName` and the embedded content queries your development environment
instead of production. That only holds while you're in development mode on the
branch — for a branch an embedded app should reach at any time, enable its [staging
environment][ref-environments-staging] instead.
...
Development environments share the production environment's Cube Store and
pre-aggregations schema. Pre-aggregations you haven't changed are served from
the production tables. Pre-aggregations you have changed are built separately
when queried, and production never serves those tables. Building large
pre-aggregations from development mode can remove older unused production
tables, so test builds on a separate deployment instead. See [Deployment
environments → Pre-aggregations][ref-environments-preaggs] for details.
