...
properties of the security context — such as membership in another group — by
combining boolean expressions in [`conditions`][ref-ref-dap-conditions] with
[`allow_all: true`][ref-ref-dap-row-level] for the unconstrained branch. See
[Conditional row-level access by group membership][ref-recipe-cond-rls] for a
worked example covering both YAML and JavaScript.
...
[ref-recipe-cond-rls]: /recipes/access-control/conditional-row-level-access