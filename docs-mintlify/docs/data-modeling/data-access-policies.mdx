...
### Switch row-level access by group membership

You can switch row-level access for the same group based on additional
properties of the security context, such as membership in another group.
The simplest approach is to define multiple policies and gate them with
[`conditions`][ref-ref-dap-conditions]. Use [`allow_all: true`][ref-ref-dap-row-level]
to express “no row-level restriction” for the unconstrained branch:
...
views:
  - name: region_test_view
    ...
    access_policy:
      # Users in `user_group` who are also in `region_group` are
      # restricted to rows matching their `allowedProductIds` attribute
      - group: user_group
        conditions:
      ...
      # All other users in `user_group` are allowed access to all rows
      - group: user_group
        conditions:
  ...
  access_policy: [
    {
      ...
      // In JavaScript, you can also switch the entire `row_level` structure
      // based on the security context, without using `conditions`
