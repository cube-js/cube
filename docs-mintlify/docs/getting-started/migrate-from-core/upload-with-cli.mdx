...
`cubejs-cli` is deprecated. You can upload the project with the new
[Cube CLI][ref-cli] instead: run `cube login`, find your deployment id with
`cube deployments list`, then run `cube deploy <deployment-id> --directory .`
