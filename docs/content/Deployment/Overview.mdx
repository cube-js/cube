...
API instance needs to be configured via environment variables, the `cube.js`
configuration file and has access to the data model files.

## Refresh Worker

A Refresh Worker updates pre-aggregations and invalidates the in-memory cache in
the background. They also keep the refresh keys up-to-date for all defined data
models and pre-aggregations. Please note that the in-memory cache is just
