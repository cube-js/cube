...
API instance needs to be configured via environment variables, the `cube.js` configuration file and
has access to the data model files.
...
data models and pre-aggregations. Please note that the in-memory cache is just
