...
As shown in the diagram below, a typical production Cube cluster consists of
...
Below you can find an example Docker Compose configuration for a Cube
cluster:
...
API instances process incoming API requests and query either Cube Store
for pre-aggregated data or connected data sources for raw data. It is possible to
...
[Cube Docker image][dh-cubejs] can be used for creating Refresh Workers; to
...
More information on Cube Store architecture can be found in [this presentation](https://docs.google.com/presentation/d/1oQ-koloag0UcL-bUHOpBXK4txpqiGl41rxhgDVrw7gw/).

By default, Cube Store listens on the port `3030` for queries coming from
Cube. The port could be changed by setting `CUBESTORE_HTTP_PORT` environment
variable. In a case of using custom port, please make sure to change
`CUBEJS_CUBESTORE_PORT` environment variable for Cube API Instances and
Refresh Worker.
...
Cube uses Redis, an in-memory data structure store, for query caching and
queue.

Set the `CUBEJS_REDIS_URL` environment variable to allow Cube API Instances
