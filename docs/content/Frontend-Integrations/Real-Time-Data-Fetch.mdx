...
[`refresh_key` refresh rules](caching#refresh-keys). In order to provide a desired
refresh rate, `refresh_key` should reflect the rate of change of the underlying
data set; the querying time should also be much less than the desired refresh
rate. Please use the [`every`](/schema/reference/cube#refresh-key)
