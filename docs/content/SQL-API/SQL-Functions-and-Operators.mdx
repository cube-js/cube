...
In Cube query rewrite, `CASE` expression is not suppored. You can either have move that logic into Cube data modeling or leverage post-processing with outer query.

The example below shows how to use `CASE` expression in post-processing with outer query. 
