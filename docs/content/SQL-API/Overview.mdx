...
To enable the SQL API in Cube Cloud, click&nbsp;<Btn>Deploy SQL API</Btn> from the&nbsp;<Btn>Overview</Btn> page, then click <Btn>How to connect your BI tool</Btn>. You
should then see the following screen:
...
Any segments defined in a data model can also be used in Cube SQL queries. Looking
at the data model below, we have one segment `isCompleted`:
