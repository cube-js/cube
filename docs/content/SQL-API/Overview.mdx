...
The SQL API allows querying Cube with Postgres-compatible SQL dialect.
It enables the use of BI applications, Python notebooks, reverse ETL tools,
and other downstream tools on top of Cube.
...
When using [Semantic Layer Sync][ref-sls], configure `CUBEJS_SQL_HOST` and
`CUBEJS_SQL_DB_NAME` so Bi tools know which host and port should be used to
