...
In SQL API every cube is represented as a table. Measures, dimensions, and segments in this table are columns.
We call these tables **cube tables**.

Consider “Orders” cube in your data model, the following query is performing select from **cube table** "Orders".
...
Cube SQL API transforms query fragments that perfoming select from **cube tables** into [Cube internal query format](/query-format). 
We call that process **Cube query rewrite**.

The above SQL query will be transformed or rewritten into the following Cube query.
...
Because of this transformation, not all functions and expressions are supported in query fragmemnts performing select from cube tables. 
...
You can leverage nested queries in cases like this. You can wrap your select from cube table into another select statement to perform calculation with expressions like `CASE`.
This outer select is not part of the SQL query that being transformed into Cube query and therefore you can use more SQL functions, operators and expressions.
You can rewrite the above query and wrap your select from cube table into our select. 
...
That query would work because `CASE` expression is supported in selects not from cube tables.


When querying cube tables it is important to understand fundamentals of Cube query rewrite as well as **pushdown** process.
**Pushdown** is a process of pushing processing of particuluar part of the query down to inner select from cube table.
The following sections provides overview of Cube query rewrite and pushdown. 
Please refer to the reference to see whether specific function, operator or expression is supported in Cube query rewrite.
...
Because of the default limit in Cube queries (50k rows), there is a possibility of a wrong result if there are more than 50k rows. 
Given that queries to Cube are usually aggregated, it is rare that they may return more than 50k rows, but keep that limitation in mind when designing your queries. 
