...
In Airflow, pipelines are represented by directed acyclic graphs (DAGs),
Python function decorated with a `@dag` decorator. DAGs include calls to
tasks, implemented as instances of the `Operator` class. Operators can
perform various tasks: poll for some precondition, perform
