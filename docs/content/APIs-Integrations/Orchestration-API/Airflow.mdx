...
menuTitle: Integration with Airflow
permalink: /orchestration-api/airflow
...
Airflow can push changes from upstream data sources to Cube.

<!-- https://github.com/cube-js/airflow-provider-cube -->

## Operators

In Airflow, each pipeline is represented as a directed acyclic graph (or
DAG) of tasks. Tasks are implemented using the Python programming language,
and they are instances of the `Operator` class. Operators can perform various
tasks: poll for some precondition, perform extract-load-transform (ETL) jobs,
...
### <--{"id" : "Operators"}--> CubeQueryOperator
...
### <--{"id" : "Operators"}--> CubeBuildOperator

`CubeBuildOperator` is used to trigger pre-aggregation builds and check their
status via the [`/v1/pre-aggregations/jobs`][ref-ref-jobs-endpoint] endpoint
of the [Orchestration API][ref-orchestration-api].
...
### <--{"id" : "Configuration"}--> DAGs

Create a new DAG named `cube_query.py` in the `dags` subdirectory with the
following contents. As you can see, the `CubeQueryOperator` accepts a Cube
query via the `query` option.
...
[`localhost:8080`](http://localhost:8080) (use `admin`/`admin` to
authenticate):
