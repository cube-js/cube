...
GraphQL API is enabled by default and secured using
[API scopes][ref-api-scopes] and [CORS][ref-cors]. To disallow access to
GraphQL API, disable the `graphql` scope, e.g., by setting the
`CUBEJS_DEFAULT_API_SCOPES` environment variable to `meta,data`.
...
[ref-cors]: /http-api/rest#configuration-cors
