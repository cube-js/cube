...
```
mysql> SELECT MEASURE(count), status FROM Orders GROUP BY status;
+-------+------------+
| count | status     |
+-------+------------+
|  3354 | processing |
...
In the following example we group orders by created month and also by
...
Measures are already aggregated in Cube there is no need to apply
aggregate functions to them in SQL API.

```
mysql> SELECT MEASURE(count) FROM Orders;
+-------+
| count |
+-------+
| 10000 |
