...
```
query GetOrders($byCategory: Boolean) {
  cube(limit: 100, offset: 50, timezone: "America/Los_Angeles") {
    orders(orderBy: { createdAt: asc, count: desc }, where: {status: {equals: "completed"}}) {
      count
...
| ------------- | ----------------------------------- | ------------------------------------------ |
| `loadOptions` | [`CubeQueryArgs`](#cube-query-args) | Options that apply to the entire query     |
| `cubeOptions` | [`CubeArgs`](#cube-args)            | Options that apply only to a specific cube |
