...
Superset][apache-superset], [Microsoft PowerBI][ms-powerbi] etc.
...
Cube SQL uses the same JWTs as Cube, so we first need to create a JWT.
If the project uses `CUBEJS_API_SECRET`, then we can generate a JWT
token by running the following inside the project directory:

<InfoBox>

If custom JWK authentication is configured, then a JWT must be retrieved from the external identity provider.
