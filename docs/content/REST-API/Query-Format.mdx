...
- `dateRange`: An array of dates with the following format `YYYY-MM-DD` or in
  ...
  You can also pass a string with a [relative date range][ref-relative-date-range],
...
You can also use a string with a relative date range in the `dateRange` property,
for example:
...
Cube uses the [Chrono][chrono-website] library to parse relative dates.
Please refer to its documentation for more examples.
