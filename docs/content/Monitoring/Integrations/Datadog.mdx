...
permalink: /monitoring/datadog
category: Monitoring
menuOrder: 10
---

[Datadog][datadog] is a popular fully managed observability service. This
guide demonstrates how to set up Cube Cloud to export logs to Datadog.
...
throughout the world. Use the `site` option to configure the sink
appropriately. When miscofigured, Vector agent outputs the following error:
`Client request was forbidden`.

Commit the configuration for Vector, it should take effect in a minute.
Then, navigate to&nbsp;<Btn>Logs</Btn> in Datadog and watch the logs coming:
