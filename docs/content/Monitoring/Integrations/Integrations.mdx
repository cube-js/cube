...
Monitoring integrations are available in Cube Cloud on [Enterprise](https://cube.dev/pricing) tier. [Contact us](https://cube.dev/contact) for details.
...
configration file. This file is used to keep sinks configration.
...
- [AWS S3][vector-docs-sinks-s3], [Google Cloud Storage][vector-docs-sinks-gcs], and [Azure Blob Storage][vector-docs-sinks-azureblob]
- [Datadog][vector-docs-sinks-datadog]
...
- [`prometheus_remote_write`][vector-docs-sinks-prometheus] (compatible with [Grafana Cloud][grafana-cloud])
- [`prometheus_exporter`][vector-docs-sinks-prometheus-exporter] (compatible with [Mimir][mimir])

Note that `prometheus_exporter` should be explicitly enabled. To do so,
navigate to&nbsp;<Btn>Settings → Monitoring Integrations</Btn> and
