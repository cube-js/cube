...
First, you need to create pre-aggregations that will contain your batch data. In the following example, we call it **batch.** Please note, it must have `timeDimension`, Cube will use it to union batch data with source data.
...
Please make sure that lambda pre-aggreation definition goes first when defining your pre-aggregations.
...
Batch data is coming from one pre-aggregation and real-time data coming from another, [streaming pre-aggregation][streaming-pre-agg].
