...
the <EnvVar>CUBEJS_WEB_SOCKETS</EnvVar> environment variable.
...
<EnvVar>CUBEJS_DEFAULT_API_SCOPES</EnvVar> environment variable. For example, to disallow
access to the GraphQL API for everyone, set <EnvVar>CUBEJS_DEFAULT_API_SCOPES</EnvVar> to
`meta,data`.
...
<EnvVar>CUBEJS_DEFAULT_API_SCOPES</EnvVar> to `data,graphql` and use the following
...
[ref-ref-metadata]: /product/apis-integrations/core-data-apis/rest-api/reference#metadata-api
