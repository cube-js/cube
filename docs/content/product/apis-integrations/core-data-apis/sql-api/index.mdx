...
**SQL API is disabled by default.** To enable the SQL API, set <EnvVar>CUBEJS_PG_SQL_PORT</EnvVar>
...
| Port       | Set via <EnvVar>CUBEJS_PG_SQL_PORT</EnvVar>   |
| User name  | Set via <EnvVar>CUBEJS_SQL_USER</EnvVar>      |
| Password   | Set via <EnvVar>CUBEJS_SQL_PASSWORD</EnvVar>  |
| Database   | Any valid string, e.g., `cube` |

You can also use
[`checkSqlAuth`][ref-config-checksqlauth],
[`canSwitchSqlUser`][ref-config-canswitchsqluser], and <EnvVar>CUBEJS_SQL_SUPER_USER</EnvVar>
...
<EnvVar>CUBEJS_PG_SQL_PORT</EnvVar>, <EnvVar>CUBEJS_SQL_USER</EnvVar>, and <EnvVar>CUBEJS_SQL_PASSWORD</EnvVar> environment
