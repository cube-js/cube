...
cubes:
  ...
  - name: orders
    ...
    joins:
      ...
      - name: dates
        relationship: many_to_one
        sql: "{orders}.created_date = {dates.date}"
  ...
  - name: returns
    ...
    joins:
      ...
      - name: dates
        relationship: many_to_one
        sql: "{returns}.created_date = {dates.date}"
...
cube(`dates`, {
  ...
  dimensions: {
    date: { type: `time`, sql: `date`, primary_key: true }
  }
})
...
cube(`orders`, {
  ...
  dimensions: {
    id: { type: `number`, sql: `id`, primary_key: true },
    customer_id: { type: `number`, sql: `customer_id` },
...
cube(`returns`, {
  ...
  measures: {
    count: { type: `count` },
    total_refund: { type: `sum`, sql: `refund_amount` }
  }
})
```
...
The results of the subqueries are joined together using a `FULL JOIN` on all
common dimension columns — in this case, `name`, `city`, and `date`. This ensures
that all rows from both fact tables are represented, even when a customer has
orders but no returns, or vice versa.
...
cubes:
  - name: refunds
    ...
    joins:
      ...
      - name: dates
        relationship: many_to_one
        sql: "{refunds}.created_date = {dates.date}"
