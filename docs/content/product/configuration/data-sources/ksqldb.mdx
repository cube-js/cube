...
Cube automatically omits the `GROUP BY` clause when at least one of the
dimensions included in the pre-aggregation has `primary_key: true`. In
that case, the generated query becomes a simple
`SELECT ... FROM ...` without grouping, and measures are passed through
as raw expressions rather than aggregated. This is what makes the
pre-aggregation eligible for the read-only streaming path.

If none of the dimensions has `primary_key: true`, Cube generates a
`GROUP BY` clause, which means the query cannot be used for streaming
ingestion. You must include at least one primary key dimension in the
streaming pre-aggregation to ensure it stays ungrouped.
