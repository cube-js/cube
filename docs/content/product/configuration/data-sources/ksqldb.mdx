...
the pre-aggregation) is fully supported. Cube automatically generates
the appropriate `PARSE_TIMESTAMP(FORMAT_TIMESTAMP(CONVERT_TZ(...)))`
expression chain to truncate timestamps to the configured granularity.
Cube Store evaluates these expressions natively on each micro-batch
during ingestion — there is no dependency on ksqlDB for this
processing.
