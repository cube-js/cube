...
### <--{"id" : "Options Reference"}--> semanticLayerSync

This function provides the configuration for [Semantic Layer Sync][ref-sls].

Called once when a refresh worker starts. Can be an `async` function.
Returns an array with options for specific BI tools:

```typescript
semanticLayerSync: () =>
  SemanticLayerSyncConfig[] |
  Promise<SemanticLayerSyncConfig[]>;

type SemanticLayerSyncConfig =
  PresetSemanticLayerSyncConfig |
  SupersetSemanticLayerSyncConfig |
  MetabaseSemanticLayerSyncConfig;

// Semantic Layer Sync with Preset
interface PresetSemanticLayerSyncConfig {
  name: string;
  active?: boolean;
  type: "preset";
  config: {
...
interface SupersetSemanticLayerSyncConfig {
  name: string;
  active?: boolean;
  type: "superset";
  config: {
...
interface MetabaseSemanticLayerSyncConfig {
  name: string;
  active?: boolean;
  type: "metabase";
  config: {
