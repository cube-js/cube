...
### <--{"id" : "Options Reference"}--> contextToApiScopes

This function is used to select accessible [API scopes][ref-rest-scopes],
used to allow or disallow access to REST API endpoints, based on the
[security context][ref-sec-ctx].

Can be an `async` function. Security context is provided as the first
argument. An array of scopes that was set via `CUBEJS_DEFAULT_API_SCOPES` is
