...
Path to data model files. The default value is `/schema`.
...
key for various in-memory structures like data model compilation results, connection
pool, etc.
...
Schema version can be used to tell Cube that the data model should be recompiled in case
it depends on dynamic definitions fetched from some external database
...
the data model will be recompiled. It can be used in both multi-tenant and single
tenant environments.
...
global environment variables for your data model, please see [Execution environment
...
Maximum number of compiled data models to persist with in-memory cache. Defaults to
...
Maximum length of time in ms to keep compiled data models in memory. Default keeps
data models in memory indefinitely.

### <--{"id" : "Options Reference"}--> updateCompilerCacheKeepAlive

Providing `updateCompilerCacheKeepAlive: true` keeps frequently used data models in
...
development mode. It doesn't track any credentials, data model contents or queries
...
a data model. The default value is `false`, and it is means the compiler would use
...
[`FileRepository`][gh-cube-filerepo], all data model files must be within the same
directory.
