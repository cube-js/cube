...
Enter your Credentials for Snowflake.
...
**Account**. Your snowflake account identifier. You can find it in your Snowflake URL 

```
<account_locator>.<cloud_region_id>.<cloud>.snowflakecomputing.com
``` 
You need to take the `account_locator` part.

**Region**. Your Snowflake account region. You can take it from the URL. 

```
<account_locator>.<cloud_region_id>.<cloud>.snowflakecomputing.com
or
<account_locator>.<cloud_region_id>.snowflakecomputing.com
```

It might have the `cloud` part. If your URL has cloud the part use it like this `us-east-2.aws` if not just use `us-east-2`.
...
Cube can generate basic data model from your data warehouse schema. It helps to get started with Cube data modeling faster.
Select all four tables in our `ECOM` schema and click through the data model generation wizard. We'll inspect these generated files in the next section and will start making changes to them.
