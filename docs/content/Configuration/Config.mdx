...
[REST API](/backend/rest/reference/api) base path. The default value is
`/cubejs-api`.
...
This option allows to customize the repository for Cube data schema files. It
...
Used in [SQL API][ref-sql-api], and can be an `async` function. This function
should verify username and return an object with password and security context.
If the password returned from this function matches the one provided in the
connection string, the user will be authenticated with the given security
context.

The default implementation verifies the username and password from the
`CUBEJS_SQL_USER` and `CUBEJS_SQL_PASSWORD` environment variables, but in
[development mode][ref-development-mode], it ignores validation.
...
Cube to know how to generate a context without the required input.
