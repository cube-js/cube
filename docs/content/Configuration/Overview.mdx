  ...
  queryRewrite: async (query, { securityContext }) => {
    if (securityContext.accountId) {
      query.filters.push({
        member: 'Accounts.id',
        operator: 'equals',
        values: [securityContext.accountId],
      });
    }
    return query;
  },
...
[link-config] /config [link-config-cors-opts] /config#http [link-dev-playground]
/dev-tools/dev-playground [link-env-vars] /reference/environment-variables
