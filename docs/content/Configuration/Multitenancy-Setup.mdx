...
context to every query. We should include them into our token generation code.
...
`userId`, then use [`driverFactory`][ref-config-driverfactory] to dynamically
select database, again based on `appId` and `userId`:

<WarningBox>

Cube.js App IDs and Orchestrator IDs are used as caching keys for various
in-memory structures like schema compilation results, connection pool, etc.
Missing definitions for either of the above can result in unexpected caching
issues such as a schema of one tenant being used for another one.
