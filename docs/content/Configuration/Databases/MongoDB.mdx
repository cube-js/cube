...
If you are connecting to a local `mongobi` connector, which is pointing to a local `mongodb` instance,
If MongoBI Connector and MongoDB are both running locally, then the above should work.
To connect to a remote MongoDB instance, first configure `mongosqld` appropriately. See
[here for an example config file][mongobi-with-remote-db].
