...
An `INNER JOIN` can be replicated with Cube.js; when making a Cube.js query, add
a filter for `IS NOT NULL` on the required column.
...
[ref-schema-dimension-types]: /docs/schema/reference/types-and-formats#types
