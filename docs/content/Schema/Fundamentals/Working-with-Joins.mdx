...
we explicitly _removed_ the `one_to_many` relationship from the `Customer` cube; not
...
The following example uses the `one_to_many` relationship on the `PostTopics` cube;
...
done by declaring a [`one_to_many` relationship][ref-schema-ref-joins-relationship]
