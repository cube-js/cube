...
no matching customer. Looking back at the `has_many` relationship and its'
...
with a `belongs_to` relationship on the `Orders` cube:
...
we explicitly _removed_ the `has_many` relationship from the `Customer` cube;
...
The following example uses the `has_many` relationship on the `PostTopics` cube;
...
done by declaring a [`has_many` relationship][ref-schema-ref-joins-relationship]
