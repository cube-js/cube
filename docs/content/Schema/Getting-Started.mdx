...
<InfoBox>
  We're excited to expand the beta of YAML and Python support in Cube data modeling. <br />
  Please <a href="https://forms.gle/4CZRRKK8nD4PvtxZA"> reach out to us </a> if you’d like to sign up for early access.
</InfoBox>

A Cube Data Schema is used to model raw data into meaningful business
...
In Cube, [cubes][ref-schema-cube] are used to organize entities and
...
`count`. We can group `paying_count` by `city` and `companyName` simply by adding
