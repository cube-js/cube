...
Learn how Cube's data modeling makes it easier to build maintainable semantic layers with best practices & key improvements
at [our webinar on May 31st, 2023](https://cube.dev/events/data-modeling-update).
...
such as `users`, `orders`, `products`, etc. In the `sql_table` parameter of the cube
...
table for the cube (with the `sql_table` property), we create a `count` measure in the
`measures` block. The `count` [type][ref-schema-types-formats] and sql `id`
