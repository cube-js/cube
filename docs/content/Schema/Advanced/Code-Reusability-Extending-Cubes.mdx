...
It’s important to use the `${CUBE}` reference when referencing properties on
the cube. Not specifying the cube name or using `${BaseEvents}` would not work
when the cube is extended.
