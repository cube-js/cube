...
The `shown` property is used to manage the visibility of a cube. Some data may
be available only to specific roles within an organization or to specific
customers. In the example below, we’re making the `Orders` cube only visible to
