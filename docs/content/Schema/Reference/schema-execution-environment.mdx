...
The Cube Schema Compiler uses [Node.js VM][nodejs-vm] to execute schema compiler
...
code in a safe manner. Cube Schema JavaScript is standard JavaScript supported
...
Node.js modules. Besides that, the data schema `require()` can resolve Cube
...
in order to provide API for schema configuration which aren't normally
accessible outside of Cube schema.
