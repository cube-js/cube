...
We **strongly** recommend persisting results of `original_sql` only back to the
source database.
...
`rollupJoin` is ephemeral pre-aggregation that relies on referenced rollups
 when queries are executed.
Setting [`scheduled_refresh`](#parameters-scheduled-refresh) to `true` is
unnecessary for `rollupJoin` and will result in an error.
Appropriate freshness controls should be set on referenced rollups instead.
...
### <--{"id" : "Parameters"}--> partition_granularity
...
[buildRange][self-buildrange] divided by `partition_granularity`. Number of
...
[`refresh_key`][ref-cube-refreshkey] documentation.
...
### <--{"id" : "Parameters"}--> use_original_sql_pre_aggregations
...
cube(`Orders`, {
  ...
  pre_aggregations: {
    ...
    completed_orders: {
      measures: [CUBE.count],
      time_dimension: CUBE.completedAt,
      granularity: `day`,
      use_original_sql_pre_aggregations: true,
    },
  },
...
### <--{"id" : "Parameters"}--> scheduled_refresh
...
### <--{"id" : "Parameters"}--> build_range_start and build_range_end

<WarningBox>

Currently `build_range_start` and `build_range_end` do **not** have any effect
on pre-aggregations without `partition_granularity`. This behavior may be
changed in future versions of Cube.
...
which can lead to an empty result set, depending on the query. If you want to query data outside of the build range,
consider using a [Lambda pre-aggregations][self-rolluplambda].
...
### <--{"id" : "Parameters"}--> union_with_source_data
...
is of type `rollupLambda`, and then set `union_with_source_data` to `true`:
