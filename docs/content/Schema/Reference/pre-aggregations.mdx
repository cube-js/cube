...
Similar to how Cube's caching layer uses `refreshKey` queries [for
cubes][ref-schema-ref-cube-refreshkey], pre-aggregations can also use
`refreshKey` queries to determine if they need to be refreshed. By default, it
is set to `every: '1 hour'`.
...
[ref-schema-ref-cube-refreshkey]: /schema/reference/cube#parameters-refresh-key
