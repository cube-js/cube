...
there is a need to control the join path or to rename measures and dimensions
from the underlying cubes. In the example below, we create a view
`CompletedOrders` which brings in dimensions from the `Users` cube through
nested joins:
