...
Views **can not** have their own members. Instead, use the `cubes` or `includes` properties
...
The other required parameter inside `cubes` block is `includes`. You can simply list measures, dimensions or segments you'd like to include. 
In case you need to rename some of them you can provide `name` and `alias` parameters. 
Alternatively, you can use `excludes` parameter in conjuction with includes all defition `includes: *`.

Optionally, if you'd like to prefix exposed measures, dimensions or segments with cube name, you can use `prefix: true` parameter. 
It will prefix exposed measures and dimensions with cube name, e.g. `users_city`. You can use `alias` parameter to rename cube for the prefixing purpose.
