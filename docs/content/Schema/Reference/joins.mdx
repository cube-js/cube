...
The name of a join should match the [name of the joined
cube][ref-schema-cube-naming]. For example when a `products` cube is being
joined to an `orders` cube, the join would be defined as follows:
...
The types of relationships listed above are available since v0.32.19.
Previously, they were known under the following aliases:

- `one_to_one` as `has_one` and `hasOne`
- `one_to_many` as `has_many` and `hasMany`
- `many_to_one` as `belongs_to` and `belongsTo`
