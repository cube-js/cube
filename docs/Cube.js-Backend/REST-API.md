...
REST API is used to communicate Cube.js backend.
All requests are using **basePath** described in [Backend Server Core](/@cubejs-backend-server-core). By default it's `/cubejs-api`.

### Authentication

Cubejs API uses [HTTP Basic Authentication](https://en.wikipedia.org/wiki/Basic_access_authentication). You need to pass your API Token in Authorization Header. Setting API Tokens is described in [Security](/security) section. In Development environment a temporary API Token is generated on every launch.

### Example request

```bash
curl -H "Authorization: exampleApiToken" https://example.com/cubejs-api/v1/sql
```

### Continue wait

Sometimes the request can take too much time to send a response. Possible reasons:
 ...
 * There are many queries requested and Cube.js backend queues them to save database from overloading.
 ...
 When this happens, REST API responds with ```{ "error": "Continue wait" }``` and 200 status code. To get the data requested, simply retry your request a bit later.
...
| 500 | Error message | Cube.js server error. Check error message for details. |
...
Get SQL Code which the database will be queried
...
* `sql` - Data used to build database queries
