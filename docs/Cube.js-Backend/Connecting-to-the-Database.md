...
Use `CUBEJS_DB_SSL=true` to enable SSL if you have force_ssl enabled for your RDS. Download the new certificate [here](https://s3.amazonaws.com/rds-downloads/rds-ca-2019-root.pem) provide the filename of the downloaded file to `CUBEJS_DB_SSL_CA` `CUBEJS_DB_SSL_CERT`, `CUBEJS_DB_SSL_CIPHERS`, `CUBEJS_DB_SSL_PASSPHRASE` can be left blank. More info on AWS RDS SSL can be found [here](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.SSL.html)
