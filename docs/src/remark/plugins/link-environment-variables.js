...
const isCubeEnvVar = (value) => {
  return !value.includes('=') && !value.endsWith('_*') && (value.startsWith('CUBEJS_') || value.startsWith('CUBESTORE_'));
}
...
module.exports = function retextSentenceSpacing() {
  return (tree) => {
      ...
      for (const index of indexes) {
        ...
        if (value && isString(value) && isCubeEnvVar(value)) {
          ...
          const newNode = {
            ...
            title: null,
            type: 'link',
            url: '/reference/environment-variables#' + value.toLowerCase().replace(/_/g, '-')
          }
