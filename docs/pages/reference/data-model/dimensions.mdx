...
Alternatively, instead of `offset`, you can specify how the time interval is
shifted forward or backward in time with the `origin` parameter. It accepts an
ISO 8601-compliant [date time string][link-date-time-string], e.g., `2024-01-02`
or `2024-01-02T12:00:00.000Z`. When `origin` is provided, time intervals will be
shifted in a way that one of them will match the provided origin.
