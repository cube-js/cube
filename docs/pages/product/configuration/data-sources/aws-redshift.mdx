...
By default this schema is named `prod_pre_aggregations` but it can be customized using the environment variable `CUBEJS_PRE_AGGREGATIONS_SCHEMA` ([reference][env-vars-pre-agg-schema]).
...
[env-vars-pre-agg-schema]: https://cube.dev/docs/reference/configuration/environment-variables#cubejs_pre_aggregations_schema
