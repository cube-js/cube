...
the `CUBEJS_DB_SSL` environment variable to `true`.

To use a custom (or self-signed) certificate chain, set the `NODE_EXTRA_CA_CERTS`
environment variable to `/path/to/certificate/bundle.cert`.

Please note that the configuration settings on the 
[Enable SSL Connections to the Database][ref-recipe-enable-ssl] page are not
supported by the Microsoft SQL Server driver.
