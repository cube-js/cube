...
| `CUBEJS_DB_EXPORT_BUCKET`      | The name of a bucket in cloud storage. Can include a path to organize files in subdirectories (e.g., `gs://bucket/path` or `bucket/path`)                                                      | A valid bucket name from cloud storage, optionally with a path          |    ❌    |
...
You can organize pre-aggregation files in subdirectories within your export bucket
by including a path in the `CUBEJS_DB_EXPORT_BUCKET` variable. This is useful for
multi-tenant scenarios where each tenant needs isolated storage within a shared bucket:

```dotenv
# Single-level subdirectory
CUBEJS_DB_EXPORT_BUCKET=gs://my-bucket/tenant-1

# Multi-level subdirectory
CUBEJS_DB_EXPORT_BUCKET=gs://my-bucket/data/exports/tenant-1
```

You can also use bucket names without the `gs://` scheme:

```dotenv
CUBEJS_DB_EXPORT_BUCKET=my-bucket/tenant-1
```

With subdirectories configured, pre-aggregation files will be created at:
- `gs://my-bucket/tenant-1/schema.table-*.csv.gz` instead of
- `gs://my-bucket/schema.table-*.csv.gz`

This allows you to configure IAM permissions at the subdirectory level, granting
each tenant access only to their specific folder within the shared bucket.
