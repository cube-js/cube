...
Persistent storage can be [encrypted](#data-at-rest-encryption) to ensure
...
Cube Store provides data-at-rest protection by utilizing the [modular encryption
mechanism][link-parquet-encryption] of Parquet files in its [persistent
storage](#persistent-storage). Pre-aggregation data is secured using the
[AES cipher][link-aes] with 256-bit keys. Data encyption and decryption are
completely seamless to Cube Store operations.
