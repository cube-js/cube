...
For optimal _querying performance_, partitions should be small enough so that the Cube Store workers can process them in <100 milliseconds. 
...
However, Cube Store workers can handle up to 4 partitions per worker conucrrently. Since Cube Store workers max out at 16, this means you should avoid having more than 64 partitions. 
