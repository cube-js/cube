...
| AI API | | Depends on a [chosen tier](#deployment-tiers) and config         |
| AI Assistant | | Depends on a [chosen tier](#deployment-tiers) and config         |
...
AI API and AI Assistant consume CCUs per request apart from [Enterprise and above][cube-pricing] product tiers where customers can provide their own suitable LLM if wanted and then will be exempt from this charge:
