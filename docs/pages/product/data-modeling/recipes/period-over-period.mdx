...
Tesseract, the [next-generation data modeling engine][link-tesseract], provides a more
efficient way to calculate period-over-period changes via [time-shift measures][link-time-shift].
