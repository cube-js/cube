...
Cube Remote MCP (Model Context Protocol) lets MCP-compatible AI clients connect to Cube over HTTPS using OAuth. It is hosted by AI Engineer, so you don’t need to run a local MCP server or manage API keys.
...
- **Endpoint:** `https://<ai-engineer-host>/api/mcp`
- **OAuth discovery:** `https://<ai-engineer-host>/.well-known/oauth`
...
- AI Engineer URL configured

### 1) Confirm AI Engineer URL

Remote MCP uses your AI Engineer host. If the URL isn’t configured, the Remote MCP page will show “Remote MCP configuration is unavailable.”

### 2) Enable MCP in OAuth Integrations

Remote MCP relies on a single OAuth Integration that has MCP enabled. Configure or update the integration in <Btn>Admin → Team & Security → OAuth Integrations</Btn>, then enable MCP for it.

### 3) Set Remote MCP Defaults
...
3. Use:
   ...
   - **Integration URL:** `https://<ai-engineer-host>/api/mcp`
...
```json
{
  "mcpServers": {
    "cube-mcp-server": {
      "command": "npx",
      "args": ["-y", "mcp-remote", "--transport", "http", "https://<ai-engineer-host>/api/mcp"]
    }
  }
}
```
...
```bash
claude mcp add --transport http cube-mcp-server https://<ai-engineer-host>/api/mcp
```
...
```bash
codex mcp add cube-mcp-server --url https://<ai-engineer-host>/api/mcp
```
...
url = "https://<ai-engineer-host>/api/mcp"
```
...
```bash
npx mcp-remote --transport http https://<ai-engineer-host>/api/mcp
```
...
- **Remote MCP configuration is unavailable**: Configure the AI Engineer URL.
...
The previous local MCP server runs on the client and talks directly to the Chat API using an API key. It is kept for self-managed or legacy setups, but new integrations should use Remote MCP.

### Required parameters

- **`CUBE_CHAT_API_URL`**: Full Chat API URL from <Btn>Admin → Agents → Click on Agent</Btn> (Chat API URL field).
...
### Example config (legacy local MCP)
...
Third-party users with custom identifiers, allowing for dynamic user attributes, groups, and row-level security configuration.
