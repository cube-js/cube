...
By default, the SQL API executes only [regular queries][ref-regular-queries] and
[queries with post-processing][ref-queries-wpp]. You can also
[configure][ref-sql-api-conf-query-plan] it to execute [queries with
push-down][ref-queries-wpd]. This page explains their format and details if they
...
## Post-processing and push-down

By default, the SQL API executes only [regular queries][ref-regular-queries] and
[queries with post-processing][ref-queries-wpp]. You can also
[configure][ref-sql-api-conf-query-plan] it to execute [queries with
push-down][ref-queries-wpd].
...
[reference](/reference/sql-api) to see whether a specific expression or function
is supported.
...
<WarningBox heading={`🐣  Preview`}>
  ...
  Query push-down is currently in public preview, and the API and behavior may
  change in future versions.
  
  Query push-down is disabled by default and is controlled by
  `CUBESQL_SQL_PUSH_DOWN` environment variable.
  In future versions, `CUBESQL_SQL_PUSH_DOWN` will be enabled by default.

</WarningBox>

Query push-down provides a safe net for queries that can't be rewritten
...
<WarningBox>
  ...
  This behavior is enabled whenever query push-down is enabled.
...
Without query push-down, queries that Cube runs against your database will always be aggregated,
regardless of whether you use aggregated (with `GROUP BY`) or non-aggregated
queries with the SQL API.
Whenever you enable query push-down, queries which do not contain `GROUP BY` clause will be executed as ungrouped queries.
...
With query push-down disabled, Cube will still use `GROUP BY` to execute such a query.
...
Whenever query push-down is enabled, such query would run as ungrouped query.
...
If query push-down is enabled, calculated `number`, `string` or `time` measures queried by SQL API can't use aggregation function definitions with it's `sql` paremeter.
...
Without query push-down, Cube supports most simple equality operators like
...
Without query push-down, Cube tries to push down all `ORDER BY` statements into
...
[ref-queries-wpd]: /product/apis-integrations/queries#query-with-push-down
