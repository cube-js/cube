...
_Configure AI agents, spaces, rules, and certified queries using YAML configuration files._
...
The configuration is validated when your deployment starts. Common validation errors include:

### Missing Required Fields

```
Agents config validation failed for config.yml:
Validation error: Required at "agents.0.name"
```
...
```
Agents config validation failed for config.yml:
Validation error: Invalid enum value. Expected 'always' | 'agent_requested', received 'sometimes' at "spaces.0.rules.0.type"
```

### Duplicate Names

```
Agents config: duplicate space name "sales-space" in spaces.yml
```

### Invalid Cross-References

```
Agents config: agent "sales-assistant" references non-existent space "unknown-space"
```

### YAML Syntax Errors

```
Agents config: failed to parse YAML in config.yml: unexpected end of the stream
```

## Migration from UI Configuration

If you have existing agents configured through the Cube Cloud UI, you can export their configuration and convert it to YAML format. Contact your Cube representative for assistance with migration.

## Best Practices

1. **Use descriptive names**: Choose clear, descriptive names for spaces, agents, and rules
2. **Document with descriptions**: Add descriptions to help team members understand the purpose of each configuration
3. **Organize logically**: Use subdirectories to organize related configuration files
4. **Version control**: Commit configuration changes with meaningful commit messages
5. **Test in development**: Validate configuration changes in a development environment before deploying to production
6. **Keep rules focused**: Write specific, actionable rules rather than vague guidelines
