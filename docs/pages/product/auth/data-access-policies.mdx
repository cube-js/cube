...
Be careful with the [_any role_ shorthand][ref-ref-dap-role] (`role: "*"`) in data access
policies that don't restrict access on member __and__ row level. Given that policies for
multiple roles are [combined together](#policy-evaluation) using the _OR_ semantics,
a policy that allows access for _any role_ without restrictions on a certain level will
...
`orders` cube, regardless of their roles, because of the first policy applies to _any
role_ and is combined with the second one using the _OR_ semantics.
