...
Cube supports authenticating users through Google Workspace, which is
useful when you want your users to access Cube using single sign on. This
...
Single sign-on with Google Workspace is available in Cube on
...
First, we'll enable SAML 2.0 authentication in Cube. To do this, log in to
Cube and
...
Next, we'll create a [SAML app integration for Cube in Google
...
4. Take note of the <Btn>SSO URL</Btn>, <Btn>Entity ID</Btn> and
   ...
   SAML integration in Cube. Click <Btn>Continue</Btn> to go to the next screen.
...
| Name      | Description                                                         |
| --------- | ------------------------------------------------------------------- |
| ACS URL   | Use the <Btn>Single Sign On URL</Btn> value from Cube         |
| Entity ID | Use the <Btn>Service Provider Entity ID</Btn> value from Cube |
...
SAML integration in Google into Cube.
...
[single sign-on URL provided by Cube](#enable-saml-in-cube-cloud)
(typically `<YOUR_CUBE_CLOUD_URL>/sso/saml`) to log in to Cube.
