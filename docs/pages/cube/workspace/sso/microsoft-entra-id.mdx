...
Cube supports authenticating users through [Microsoft Entra
ID][ext-ms-entra-id] (formerly Azure Active Directory), which is
useful when you want your users to access Cube using single sign-on.

This guide will walk you through the steps of configuring SAML authentication
in Cube with Entra ID. You **must** have sufficient permissions in your
Azure account to create a new Enterprise Application and configure SAML
integration.

<SuccessBox>

Single sign-on with Microsoft Entra ID is available in Cube on
...
First, we'll enable SAML 2.0 authentication in Cube:
...
<Screenshot
  alt="Cube Team Authentication and SSO tab"
...
configuration page](#enable-saml-in-cube-cloud) in Cube:
...
To map a role attribute from Entra ID to an identically-named role defined in Cube, add the claim URI corresponding to role to the Role field in Cube, similar to above. Note that Admin status cannot be set via SSO.

You can map the user's display name from Entra ID to Cube in the same manner.

Save settings on the Cube side.
...
and verify that the SAML integration now works for your Cube account:
