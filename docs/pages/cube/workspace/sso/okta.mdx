...
Cube supports authenticating users through Okta, which is useful when you
want your users to access Cube using single sign on. This guide will walk
you through the steps of configuring SAML authentication in Cube with
...
Single sign-on with Okta is available in Cube on
...
Next, we'll create a [SAML app integration for Cube in
...
| Name                        | Description                                                 |
| --------------------------- | ----------------------------------------------------------- |
| Single sign on URL          | Use the <Btn>Single Sign On URL</Btn> value from Cube |
| Audience URI (SP Entity ID) | Use the <Btn>Audience</Btn> value from Cube           |
...
SAML integration in Okta into Cube.
