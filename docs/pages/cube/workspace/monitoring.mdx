...
Cube allows exporting logs and metrics to external monitoring tools so you
can leverage your existing monitoring stack and retain logs and metrics for the
long term.

<SuccessBox>

Monitoring integrations are available in Cube on
...
Under the hood, Cube uses [Vector][vector], an open-source tool for
...
Cube deployment should export their logs:
...
| `account_name` | Name of the Cube account. |
...
[ref-prod-env]: /cube/workspace/environments#production-environment
...
[ref-query-history-export-recipe]: /cube/workspace/recipes/query-history-export