...
As an account administrator, you can manage how your team and users access Cube.

You can authenticate using email and password, a GitHub account, or a Google account.
Cube also provides single sign-on (SSO) via identity providers supporting
[SAML 2.0](#saml-20), e.g., Okta, Google Workspace, Azure AD, etc.

Finally, Cube provides the [LDAP integration](#ldap-integration), enabling
...
Authentication is available in Cube on [all product tiers](https://cube.dev/pricing).<br/>
...
of your Cube account, and switch to the <Btn>Authentication & SSO</Btn> tab:
...
Cube will be accessing your LDAP server from the IP addresses shown under
...
Use the tooltips in Cube to get more information about each setting.

#### User properties mapping

You have to configure how user data in an LDAP catalog maps to user properties in Cube.
...
You can configure how user data in an LDAP catalog maps to roles in Cube.
...
* If the value matches an existing role in Cube, then the user assumes this role.
