...
_Real-time streaming API for AI agent chat interactions._

Stream real-time conversations with AI agents for analytics and data exploration.
...
1. **Generate a session** using your [API key](/cube/embed/api-keys). This endpoint will automatically create (insert) or update the external user based on the `externalId` provided:
...
<InfoBox>
User attributes must first be configured in your Cube admin panel before they can be used. See the [User Attributes Administration Guide](/cube/administration/user-attributes) for setup instructions.
</InfoBox>
