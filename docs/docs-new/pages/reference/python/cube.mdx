...
Should be called with two arguments, a `name` and a `value` of the variable.
...
This method registers a function so that it's callable from a Jinja template
as a Python function.

In case this method is used as a decorator with a single `name` argument,
...
In case this method is called with two arguments, a `name` and a function,
...
This method registers a function so that it's callable from a Jinja template
as a [Jinja filter][link-jinja-filters].
...
[ref-model-syntax]: /product/data-modeling/fundamentals/syntax#model-syntax