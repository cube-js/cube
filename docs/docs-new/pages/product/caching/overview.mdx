...
With the following `refresh_key`, Cube will only refresh the data if the value of
`MAX(created_at)` changes. By default, Cube will check this `refresh_key`
every 10 seconds:
...
the [`refresh_key`][ref-schema-ref-cube-refresh-key] property in a cube. Often, a
