...
# Use the 'context.function' decorator to expose a Python function to Jinja
@context.function
def dbt_models():
  return list(model for model in dbt.models if model.name not in excluded_models)

@context.function
def dbt_model(name):
  return dbt.models(name)
```
...
```yml
{% set model = dbt_model('users_copy') %}
