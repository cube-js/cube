...
To enable the SQL API, set `CUBEJS_PG_SQL_PORT` to a port number you'd like to
connect to with a Postgres-compatible tool.
...
the <Btn>Overview</Btn> page, then click <Btn>How to connect your BI tool</Btn>.
...
[Cube's internal query format](/product/api-rest/query-format). This process is
called **Cube query rewrite**.
...
[ref-sql-api-auth]: /product/api-sql/security
