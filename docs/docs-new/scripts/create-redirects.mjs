...
async function createRedirects() {
  ...
  const result = await Promise.all(mdxFiles.flatMap(async (filePath) => {
  ...
  const filteredUrls = result.flatMap(f => f ? [f] : []).flat();
  ...
  await writeFile(path.resolve('redirects.json'), JSON.stringify(finalRedirects, null, 2));

  const message = [
    '\n------------------------------',
    `Finished generating ${finalRedirects.length} redirects:\n`,
    '------------------------------\n',
  ];
